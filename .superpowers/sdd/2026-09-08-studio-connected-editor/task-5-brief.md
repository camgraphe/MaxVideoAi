...
- Projects conserve actuellement une palette locale bleu/indigo distincte de l’app qualifiée. Le contrôle réel via le menu app confirme que son thème sombre fonctionne : ne pas réinventer le header, la navigation ou le thème global. Aligner les seuls tokens locaux Projects/dialogue sur les règles et accents acceptés. Pour les captures clair/sombre, vérifier le thème DOM réellement appliqué : émuler `prefers-color-scheme` seul ne suffit pas, la préférence app par défaut est claire.
- Création de montage : la capture réelle `output/playwright/studio-connected/mobile-ordered-creation.png` montre une liste ordonnée scrollable dans un dialogue lui-même scrollable, avec le haut de la première carte coupé après focus du trim suivant. Réduire cette imbrication sur mobile (une surface de défilement cohérente), sans sacrifier les douze clips, l’accès au submit ni la géométrie contenue. Le parcours clavier et le rejeu idempotent après réponse perdue sont déjà des contrats racine ; les conserver.
