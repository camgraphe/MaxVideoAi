...
Base vérifiée : `cf525e7df`. Le commit contenant cette section porte uniquement les correctifs de composition ; son SHA exact est communiqué dans la livraison et son delta est `cf525e7df..HEAD`. Les modifications Task3 suspendues, les six clés `viewer.projectMedia` et les fixtures QA du parent restent intactes et non stagées.

- Chat : `useWorkspaceGenerationActions` produit un écho local explicitement marqué « Simulation — aucun modèle appelé » en Mock, sans fetch. Live ne modifie ni brouillon ni historique, même si le client présente un devis prêt fictif. Les modèles, certificats, paramètres et anciens messages restent conservés.
- `useWorkspaceRenderNodes`, ChatNode et ChatNodeInspector projettent le mode courant : Simuler en Mock ; Send désactivé et raison tarifaire explicite en Live. Trois clés Chat ajoutées à la copie Studio et aux dictionnaires EN/FR/ES, sans faux prix zéro.
- La route Chat délègue à `studio-chat-handler`, qui conserve l'authentification puis retourne 503 `STUDIO_CHAT_LIVE_UNAVAILABLE`, avant lecture du payload ou import d'un provider. Faux devis, mode Mock envoyé à l'API et payload historique ne peuvent autoriser un appel. Le propriétaire fournisseur existant reste conservé pour une intégration ultérieure qualifiée.
- Le véritable helper `renderer.renderDimensions` gère 1440p : 2560×1440 en 16:9, 1440×1440 en carré, 810×1440 en 9:16. Autres hauteurs/proportions inchangées ; aucun worker ou encodage lancé.
- Le repository export dérive l'ID global par SHA-256 du tuple JSON `[userId, clé exacte]`. La réservation existante emploie le même helper ; aucun second journal ou wallet. Le transport et les services partagent la validation 8–200 caractères, sans trim/troncature. Le lookup compte/clé et l'UPSERT existants rejouent les IDs historiques sans migration.

TDD observé avant correction : Mock et Live tentaient chacun un fetch ; le bouton Mock ne proposait pas Simulate ; la route atteignait l'erreur fournisseur 500 au lieu du refus 503 ; 1440p retournait 1920×1080 ; les clés perdaient leur ponctuation/suffixe et les espaces étaient normalisés ; PostgreSQL signalait une collision réelle de clé primaire. Après correction :

```sh
PATH="/opt/homebrew/opt/postgresql@17/bin:$PATH" pnpm dlx node@22 frontend/node_modules/tsx/dist/cli.mjs --tsconfig frontend/tsconfig.json --test tests/studio-chat-composition.test.ts tests/studio-chat-route-composition.test.ts tests/timeline-export-composition.test.ts tests/timeline-export-composition-postgres.test.ts tests/maxvideoai-editor-generation-blocks.test.ts tests/maxvideoai-editor-v1-pricing.test.ts
PLAYWRIGHT_EDITOR_BASE_URL=http://127.0.0.1:3032 PLAYWRIGHT_EDITOR_SKIP_WEB_SERVER=1 pnpm dlx node@22 node_modules/@playwright/test/cli.js test -c playwright.editor.config.ts tests/e2e/editor/editor-chat-composition.spec.ts --output=.superpowers/studio-visuals/composition-fix-results
pnpm dlx node@22 frontend/node_modules/typescript/bin/tsc --noEmit -p frontend/tsconfig.json
npm --prefix frontend run lint
...
Résultats : **93/93 tests ciblés et contrats**, dont **10 nouvelles régressions**, **1/1 e2e**, TypeScript **0 erreur**, lint **0 erreur / 4 warnings** (fichiers bibliothèque média non modifiés par ce lot), exposition publique et diff-check propres. Le test de handler injecte seulement l'identité authentifiée et configure des clés fournisseur factices avec tout fetch intercepté ; aucune authentification de production ni requête fournisseur. Le test PostgreSQL17 a réellement créé/détruit un cluster local jetable et prouvé deux comptes/une clé, deux clés ponctuées, rejeu exact et ancien ID `tlx_historicalkey`. Il est explicitement ignoré si les binaires PostgreSQL locaux manquent ailleurs.

La commande complète `test:editor` (mêmes globs sous Node22, PATH PostgreSQL ci-dessus) a été exécutée : **412/418 passent, 6 échouent**, aucun skip. Les six échecs concernent les edits Task3 en attente, laissés hors commit : `MaxVideoAI editor workspace is an isolated authenticated app route`, `MaxVideoAI editor owns graph, node, generation, and capability contracts`, `MaxVideoAI editor library assets map to media node records`, `studio editor asset library hook owns pagination and project media kind filters`, `studio asset picker defers multi-select import until its primary action`, `Studio localization dictionaries expose required copy in every locale` (six nouvelles clés médias non encore traduites). Aucun succès global revendiqué ; revue du delta isolé puis reprise Task3 par le parent.

Limites : aucune qualification provider, worker/export MP4 ou nouvelle facturation Chat ; pas de nouveau serveur, secret réel, DB distante, API payée, catalogue ou contrat média. Le tarif export1440p n'est pas changé : `frontend/src/server/timeline-exports/pricing.ts::resolutionMultiplier` traite 2160/4k puis1080 et retourne1 autrement, donc1440p reçoit1 contre1.5 pour1080p (constat de source, propriétaire export/pricing, à qualifier séparément). Les skills revue/TDD/vérification ont imposé la reproduction RED, les frontières existantes et la déclaration explicite de ces limites.
