...
import { fetchModelSlugs, fetchExampleSlugs, fetchWorkflowSlugs, type SitemapRow } from '@/lib/sitemapData';

const BASE_URL = (process.env.NEXT_PUBLIC_SITE_URL ?? 'https://maxvideoai.com').replace(/\/+$/, '');

const STATIC_PATHS = ['/', '/models', '/workflows', '/examples', '/legal', '/legal/privacy', '/legal/terms'] as const;
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...staticEntries,
    ...mapRowsToEntries('models', models, fallbackLastModified),
    ...mapRowsToEntries('examples', examples, fallbackLastModified),
    ...mapRowsToEntries('workflows', workflows, fallbackLastModified),
  ];
}
