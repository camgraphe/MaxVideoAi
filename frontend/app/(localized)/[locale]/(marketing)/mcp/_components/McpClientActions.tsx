...
function McpClientAction({ action }: { action: McpClientActionCopy }) {
...
export function McpClientActions({ actions }: { actions: McpClientActionCopy[] }) {
  ...
  return (
    <div className="grid gap-3 border-hairline bg-bg text-text-primary dark:border-white/[0.08] dark:bg-bg sm:grid-cols-2">
      {clients.map((action) => <McpClientAction key={action.client} action={action} />)}
    </div>
  );
}
