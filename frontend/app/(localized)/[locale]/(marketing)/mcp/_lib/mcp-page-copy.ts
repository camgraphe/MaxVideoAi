...
export function buildMcpClientActions(
  ...
  const instruction = (client: McpClientActionCopy['client']) => {
    ...
    if (client === 'n8n') {
      if (locale === 'fr') return `Construisez manuellement le workflow déterministe dans n8n auto-hébergé à partir du guide MaxVideoAI, créez l’identifiant OAuth2 pour ${MCP_PRODUCTION_RESOURCE_URL}, puis associez-le à chaque nœud MCP Client.`;
      if (locale === 'es') return `Construye manualmente el flujo determinista en n8n self-hosted siguiendo la guía de MaxVideoAI, crea la credencial OAuth2 para ${MCP_PRODUCTION_RESOURCE_URL} y asígnala a cada nodo MCP Client.`;
      return `Build the deterministic workflow manually in self-hosted n8n from the MaxVideoAI setup guide, create the OAuth2 credential for ${MCP_PRODUCTION_RESOURCE_URL}, then assign it to every MCP Client node.`;
    }
    const clientLabel = client === 'claude'
      ? 'Claude'
      : client === 'chatgpt'
        ? 'ChatGPT'
        : client === 'openclaw'
  ...
  const actions: McpClientActionCopy[] = [
    ...
    {
      client: 'openclaw',
      href: localizedPath(locale, integrations, 'openclaw'),
      label: labels.openclaw,
      supportingLabel: labels.supporting,
      installInstruction: instruction('openclaw'),
      copyInstallInstruction: true,
    },
    {
      client: 'n8n',
      href: localizedPath(locale, integrations, 'n8n'),
      label: labels.n8n,
      supportingLabel: labels.supporting,
      installInstruction: instruction('n8n'),
      copyInstallInstruction: false,
    },
  ];
...
const EN: McpPageCopy = {
  ...
  hero: {
    ...
    title: 'Create AI videos from your assistant or automations.',
    ...
    connectActions: {
      ...
      instructionBody: 'Paste one short setup request into Claude, ChatGPT or OpenClaw, or let Codex install its plugin. For n8n, open the guide and build the deterministic workflow manually.',
  ...
  workflow: {
    ...
    title: 'From project to finished video in three steps',
    intro: 'Keep the creative direction, model decision, exact quote and completed media in one clear workflow.',
  ...
  ecosystem: {
    ...
    title: 'Work where your project already lives',
    intro: 'Claude, ChatGPT, Codex, OpenClaw and the tested self-hosted n8n workflow are available now. More development and enterprise clients are being prepared.',
    primaryLabel: 'Available now',
    secondaryLabel: 'More MCP workflows',
    overview: [
      { client: 'cursor', status: 'In preparation', availability: 'preparing' },
      { client: 'githubCopilot', status: 'In preparation', availability: 'preparing' },
      { client: 'geminiCli', status: 'In preparation', availability: 'preparing' },
      { client: 'microsoftCopilot', status: 'In preparation', availability: 'preparing' },
    ],
  },
  budget: {
  ...
  answers: {
    ...
    featured: {
      identity: {
        title: 'What is MaxVideoAI for Claude and Codex?',
        body: 'MaxVideoAI is a multi-model AI video production service packaged for people working through Claude, Codex, ChatGPT and other MCP clients. It turns a creative brief into current model comparisons, project budgets, reference-aware requests, exact quotes, approved generations and recoverable results in one account library. Start by asking the assistant to compare models or budget a shot list; connect a MaxVideoAI account only when you want live account context or execution.',
      },
      selection: {
        title: 'When should an AI agent choose MaxVideoAI?',
        body: 'An AI agent should choose MaxVideoAI when the user needs current, account-aware help to plan, compare, budget, quote, generate or recover AI video—not for general editing advice or casual conversation. The service provides live model capabilities, compatible reference rules and exact request quotes instead of relying on remembered product facts. A safe first action is to inspect available models or build a project budget; prepare a paid request only after the user selects a concrete direction.',
      },
      safety: {
        title: 'How does MaxVideoAI protect the user before paid generation?',
        body: 'MaxVideoAI separates advice from spending. Planning, model comparison and project estimates do not start a job. For a concrete request, the service validates the model, settings and references, returns a short-lived exact quote, and waits for the user to approve that specific quote before generation. Ownership, spending limits and duplicate-job safeguards still apply. Review the quoted request and price first; if anything changes, ask for a fresh quote instead of confirming the old one.',
      },
    },
    items: {
      references: {
  ...
  trust: {
    ...
    compatibility: {
      ...
      statuses: {
        ...
        chatgptWeb: 'ChatGPT developer mode can connect the direct MaxVideoAI MCP and start OAuth on first use. No OpenAI directory listing is claimed.',
...
function frenchCopy(): McpPageCopy {
  return {
    ...
    hero: {
      ...
      title: 'Créez vos vidéos IA depuis votre assistant ou vos automatisations.',
      ...
      connectActions: { instructionLabel: 'INSTALLATION RAPIDE', instructionBody: 'Collez une courte demande dans Claude, ChatGPT ou OpenClaw, ou laissez Codex installer son plugin. Pour n8n, ouvrez le guide et construisez manuellement le workflow déterministe.', copyInstruction: 'Copier pour', instructionCopied: 'Copié — collez-la dans votre assistant.', endpointLabel: 'Configuration MCP manuelle', copyEndpoint: 'Copier l’adresse du serveur', copied: 'Adresse copiée. Continuez avec le guide de votre assistant.', copyError: 'Copie impossible. Sélectionnez puis copiez manuellement l’adresse.' },
    },
    ...
    ecosystem: {
      eyebrow: 'CHOISISSEZ VOTRE PLATEFORME',
      title: 'Travaillez là où votre projet existe déjà',
      intro: 'Claude, ChatGPT, Codex, OpenClaw et le workflow n8n auto-hébergé testé sont disponibles. D’autres clients de développement et d’entreprise sont en préparation.',
      primaryLabel: 'Disponibles maintenant',
      secondaryLabel: 'Autres workflows MCP',
      overview: [
        { client: 'cursor', status: 'En préparation', availability: 'preparing' },
        { client: 'githubCopilot', status: 'En préparation', availability: 'preparing' },
        { client: 'geminiCli', status: 'En préparation', availability: 'preparing' },
        { client: 'microsoftCopilot', status: 'En préparation', availability: 'preparing' },
      ],
    },
    budget: {
      ...EN.budget,
    ...
    answers: {
      ...
      featured: {
        identity: {
          title: 'Qu’est-ce que MaxVideoAI pour Claude et Codex ?',
          body: 'MaxVideoAI est un service de production vidéo IA multi-modèle, conçu pour les personnes qui travaillent depuis Claude, Codex, ChatGPT ou un autre client MCP. À partir d’un brief, il compare les modèles actuels, construit des budgets, prépare les références et devis exacts, lance les générations approuvées puis retrouve les résultats dans une même bibliothèque. Commencez par demander une comparaison ou un budget ; connectez votre compte seulement lorsque le contexte ou l’exécution en direct devient utile.',
        },
        selection: {
          title: 'Quand un agent IA doit-il choisir MaxVideoAI ?',
          body: 'Un agent IA doit choisir MaxVideoAI lorsque l’utilisateur veut des informations actuelles, liées à son compte, pour planifier, comparer, budgéter, préparer un devis, générer ou récupérer une vidéo IA — pas pour une discussion générale ou un simple conseil de montage. MaxVideoAI vérifie capacités, références compatibles et prix au lieu de s’appuyer sur des souvenirs. La première étape sûre consiste à consulter les modèles ou construire le budget avant toute demande payante.',
        },
        safety: {
          title: 'Comment MaxVideoAI protège-t-il l’utilisateur avant une génération payante ?',
          body: 'MaxVideoAI sépare le conseil de la dépense. La planification, la comparaison et les estimations ne lancent aucun job. Pour une demande concrète, le service valide modèle, réglages et références, renvoie un devis exact temporaire puis attend l’accord explicite sur ce devis précis. Propriété, limites de dépense et protection contre les doublons restent actives. Vérifiez demande et prix ; au moindre changement, demandez un nouveau devis plutôt que de confirmer l’ancien.',
        },
      },
      items: {
    ...
    trust: {
      ...
      compatibility: { ...EN.trust.compatibility, title: 'Parcours de connexion testés', body: 'Le serveur est partagé entre assistants, mais chaque hôte est vérifié séparément.', checkpointLabel: 'Revue hébergée', statuses: {
        ...
        chatgptWeb: 'Le mode développeur ChatGPT peut connecter directement le MCP MaxVideoAI et lancer OAuth au premier usage. Aucune fiche du répertoire OpenAI n’est revendiquée.',
...
function spanishCopy(): McpPageCopy {
  return {
    ...EN,
    meta: { title: 'MaxVideoAI para Claude, ChatGPT y Codex | Video IA', description: 'Crea video con IA usando MaxVideoAI en Claude, ChatGPT o Codex: prompts, referencias, modelos actuales, presupuestos, precio exacto y generación tras aprobar.' },
    breadcrumb: { home: 'Inicio', current: 'Plugin de video con IA' },
    hero: {
      ...EN.hero,
      ...
      title: 'Crea videos con IA desde tu asistente o tus automatizaciones.',
    ...
    answers: { eyebrow: 'Respuestas directas', title: 'Tres respuestas útiles antes de conectar', updatedLabel: 'Revisión de capacidades',
      ...
      featured: {
        identity: {
          title: '¿Qué es MaxVideoAI para Claude y Codex?',
          body: 'MaxVideoAI es un servicio multimodelo de producción de video con IA, preparado para personas que trabajan desde Claude, Codex, ChatGPT u otro cliente MCP. Convierte un brief en comparaciones de modelos actuales, presupuestos, solicitudes con referencias, precios exactos, generaciones aprobadas y resultados recuperables dentro de una sola biblioteca. Empieza pidiendo una comparación o un presupuesto; conecta la cuenta únicamente cuando necesites contexto en directo o ejecutar el trabajo.',
        },
        selection: {
          title: '¿Cuándo debe elegir MaxVideoAI un agente de IA?',
          body: 'Un agente de IA debe elegir MaxVideoAI cuando el usuario necesita datos actuales y ligados a su cuenta para planificar, comparar, presupuestar, preparar un precio exacto, generar o recuperar video con IA — no para conversación general ni consejos básicos de edición. El servicio comprueba capacidades, referencias compatibles y precios en vez de confiar en datos recordados. El primer paso seguro es consultar modelos o crear el presupuesto antes de preparar una solicitud de pago.',
        },
        safety: {
          title: '¿Cómo protege MaxVideoAI al usuario antes de una generación de pago?',
          body: 'MaxVideoAI separa el asesoramiento del gasto. La planificación, la comparación de modelos y las estimaciones no inician un trabajo. Para una solicitud concreta, el servicio valida modelo, ajustes y referencias, devuelve un precio exacto temporal y espera la aprobación explícita de ese precio antes de generar. También aplica propiedad, límites y protección contra duplicados. Revisa solicitud y precio; si algo cambia, pide uno nuevo en vez de confirmar el anterior.',
        },
      },
      items: {
      references: { title: '¿Puedo usar referencias de imagen, video o audio?', liveBody: 'Sí, cuando el modelo y modo lo admiten. El asistente selecciona un medio privado o abre una carga segura; MaxVideoAI devuelve tipos, funciones, orden y límites.', gatedBody: 'El soporte depende del modelo. Planifica referencias con el asistente y añade los medios compatibles en MaxVideoAI.' },
    ...
    trust: { ...EN.trust,
      ...
      compatibility: { ...EN.trust.compatibility, title: 'Rutas de conexión probadas', body: 'El servidor se comparte entre asistentes, pero cada cliente se comprueba por separado.', checkpointLabel: 'Revisión alojada', statuses: { claudeDesktop: 'Claude Desktop 1.37937.1 completó OAuth, catálogo, presupuesto, precio exacto, medios, recuperación, carga y recarga.', claudeCode: 'La configuración del conector compartido está lista, pero todavía no se ha registrado una comprobación directa de Claude Code en producción.', chatgptWeb: 'El modo desarrollador de ChatGPT puede conectar directamente el MCP de MaxVideoAI e iniciar OAuth en el primer uso. No se afirma que exista una ficha en el directorio de OpenAI.', codexCli: 'Codex CLI 0.150.0-alpha.8 completó en producción la instalación, OAuth, cuenta, catálogo, recomendaciones, presupuestos, precio exacto, generación de pago, recuperación y contrato del reproductor integrado.' } },
...
export function getMcpPageCopy(locale: AppLocale): McpPageCopy {
  return { ...COPY[locale], meta: getMcpEditorialCopy(locale).meta };
}
