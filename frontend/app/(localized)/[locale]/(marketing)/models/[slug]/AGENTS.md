...
- Render `presentationOnly` registry identities through the dedicated prelaunch
  branch. Never synthesize `FalEngineEntry`, `EngineCaps`, provider IDs, modes,
  prices, examples, comparisons, or generation links for that branch.
- Keep prelaunch structured data limited to editorial page and breadcrumb
  schemas. A presentation-only page must not emit commercial or executable
  structured data.
