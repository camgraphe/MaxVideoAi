...
function toFeaturedMedia(entry?: ExampleGalleryVideo | null): FeaturedMedia | null {
  ...
  const posterSrc = entry.rawPosterUrl ?? entry.optimizedPosterUrl ?? null;
  return {
    id: entry.id,
    prompt: entry.prompt,
    videoUrl: entry.videoUrl ?? null,
    posterUrl: posterSrc,
    posterOptimizedUrl: posterSrc ? buildOptimizedPosterUrl(posterSrc, { width: 1200, quality: 65 }) : null,
...
async function renderSoraModelPage({
  ...
  const fallbackMedia: FeaturedMedia = {
    ...
    posterOptimizedUrl: buildOptimizedPosterUrl(engine.media?.imagePath, { width: 1200, quality: 65 }),
    durationSec: null,
...
function Sora2PageLayout({
  ...
  const heroPosterPreload =
    ...
    (heroMedia.posterUrl ? buildOptimizedPosterUrl(heroMedia.posterUrl, { width: 1200, quality: 65 }) : null);
...
export default async function ModelDetailPage({ params }: PageParams) {
  ...
  const heroPosterPreload = heroPosterSrc
    ? buildOptimizedPosterUrl(heroPosterSrc, { width: 1200, quality: 65 }) ?? heroPosterSrc
    : null;
