...
import { resolveProviderInfo } from './model-page-schema';
import { normalizeHeroTitle, type SoraCopy } from './model-page-specs';
...
type LegacyImagePromptExample = Omit<
  NonNullable<ModelPromptingContent['imageExamples']>['items'][number],
  'id'
>;

function getImagePromptExamples(locale: AppLocale, engineSlug: string): LegacyImagePromptExample[] {
...
function resolveLegacyDemoProfile({
  copy,
  engineId,
  locale,
  labels,
  ...
  const isHailuoDraftRoute = engineId === 'minimax-hailuo-02-text';
  const isPikaTextRoute = engineId === 'pika-text-to-video';
  const isLumaRay2Route = engineId === 'luma-ray-2' || engineId === 'lumaRay2';
  const isLumaFlashRoute = engineId === 'luma-ray-2-flash' || engineId === 'lumaRay2_flash';
  const isSora2Route = engineId === 'sora-2';
  const isSora2ProRoute = engineId === 'sora-2-pro';
  const isGeminiOmniRoute = engineId === 'gemini-omni-flash';
  const isVeo31Route = engineId === 'veo-3-1';
  const isVeoFastRoute = engineId === 'veo-3-1-fast';
  const isVeoLiteRoute = engineId === 'veo-3-1-lite';
  const isSeedance20FastRoute = engineId === 'seedance-2-0-fast';
  const isSeedance20MiniRoute = engineId === 'seedance-2-0-mini';
  const isSeedance15ProRoute = engineId === 'seedance-1-5-pro';
  const isLtx23FastRoute = engineId === 'ltx-2-3-fast';
  const isLtx23ProRoute = engineId === 'ltx-2-3-pro' || engineId === 'ltx-2-3';
  const isKling3ProRoute = engineId === 'kling-3-pro';
  const isKling3StandardRoute = engineId === 'kling-3-standard';
  const isKling34kRoute = engineId === 'kling-3-4k';
  const isKlingO3ProRoute = engineId === 'kling-o3-pro';
  const isKlingO3StandardRoute = engineId === 'kling-o3-standard';
  const isKlingO34kRoute = engineId === 'kling-o3-4k';
  const isSilentPromptRoute = isHailuoDraftRoute || isPikaTextRoute || isLumaRay2Route || isLumaFlashRoute;
  const standardDemo = getRouteDemoSummary(locale, engineId);
  ...
  const demoTitle = getDemoTitle(copy, labels.modelName, locale, engineId);
  const prompt =
    isHailuoDraftRoute
  ...
  const duration =
    isHailuoDraftRoute || isSeedance15ProRoute
      ...
      : isPikaTextRoute
        ...
        : isLumaRay2Route
          ...
          : isLumaFlashRoute
            ? (locale === 'fr' || locale === 'es' ? '5 s' : '5s')
            : isSora2ProRoute || isSora2Route
              ? (locale === 'fr' || locale === 'es' ? '8 s' : '8s')
              : isLtx23FastRoute
                ? (locale === 'fr' || locale === 'es' ? '10 s' : '10s')
                : isKling34kRoute
                  ? (locale === 'fr' || locale === 'es' ? '6 s' : '6s')
                  : isVeo31Route || isVeoFastRoute || isVeoLiteRoute
                    ? (locale === 'fr' || locale === 'es' ? '8 s' : '8s')
                    : null;
  const aspectRatio = isSilentPromptRoute ? (isPikaTextRoute ? '1:1' : '16:9') : null;
  const modeLabel = isHailuoDraftRoute
    ? locale === 'fr'
        ...
        : isVeoLiteRoute
          ? locale === 'fr'
            ...
            : locale === 'es'
              ...
              : 'Veo Lite prompt'
        : labels.textToVideo;
  const outputLabel = isSilentPromptRoute
    ? locale === 'fr'
      ? 'Sortie'
      : locale === 'es'
        ? 'Salida'
    ...
    : isSora2Route
      ? labels.demoAudio
    : labels.demoAudio;
  const output = isSilentPromptRoute || isSora2Route || isSora2ProRoute ? demo.output : demo.audio;
  const audioChipMode = isSilentPromptRoute
    ? 'silent'
    : isSora2ProRoute || isKling34kRoute
      ? 'off'
      : isKlingO34kRoute || isKlingO3ProRoute || isKlingO3StandardRoute || isGeminiOmniRoute
        ? 'media'
        : isSeedance20MiniRoute
          ? 'supported'
          : 'on';
  const audioChipLabel = isKling34kRoute ? 'Audio off' : null;
  const altContext = isPikaTextRoute
    ? locale === 'fr'
    ...
    : isLumaFlashRoute
      ...
      : getImageAlt({
          kind: 'renderThumb',
          engine: labels.modelName,
          label: copy.demoTitle ?? `${labels.modelName} demo render`,
          prompt: copy.demoTitle ?? `${labels.modelName} demo render`,
          locale,
        });
...
function resolveLegacyImageExamples({
  engineId,
  locale,
  labels,
}: {
  ...
  return {
    title: labels.imageExamplesTitle,
    intro,
    workspaceLabel: labels.openImageWorkspace,
    items: getImagePromptExamples(locale, engineId).map((example, index) => ({
      id: `${example.kind}-${index + 1}`,
      ...example,
    })),
  };
}
...
function resolveLegacyTabs(
  copy: SoraCopy,
  locale: AppLocale,
  engineId: string,
): ModelPromptingContent['tabs'] {
  const tabs = copy.promptingTabs.length ? copy.promptingTabs : getKlingO3PromptingTabs(locale, engineId);
  return tabs.map((tab) => ({
    ...tab,
    description: tab.description?.trim() ? tab.description : null,
  }));
}

function resolveLegacyGlobalPrinciples(copy: SoraCopy, locale: AppLocale, engineId: string): string[] {
  return copy.promptingGlobalPrinciples.length
    ? copy.promptingGlobalPrinciples
    : getKlingO3PromptingGlobalPrinciples(locale, engineId);
}

function resolveLegacyEngineWhy(copy: SoraCopy, locale: AppLocale, engineId: string): string[] {
  return copy.promptingEngineWhy.length
    ? copy.promptingEngineWhy
    : getKlingO3PromptingEngineWhy(locale, engineId);
}
...
export function buildLegacyModelPromptingContent({
  ...
  return {
    ...
    section: {
      ...
      guide:
        copy.promptingGuideUrl && copy.promptingGuideLabel
