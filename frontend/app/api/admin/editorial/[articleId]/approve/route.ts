...
export async function POST(request: NextRequest, { params }: { params: Promise<{ articleId: string }> }) {
  ...
  let input: { version?: unknown; digest?: unknown };
  ...
  try {
    ...
    if (error instanceof Error && /not found|not the latest|already approved|incomplete|correction requires/.test(error.message)) {
