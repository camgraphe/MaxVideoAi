...
} from '@/server/video-providers/google-vertex-veo/errors';
import {
  GOOGLE_VERTEX_VEO_PROVIDER,
  resolveGoogleVertexVeoSupport,
} from '@/server/video-providers/google-vertex-veo/model-map';
import {
...
type GoogleVertexVeoSubmissionDeps = {
...
export type GoogleVertexVeoGenerateSubmissionResult =
  | {
      ok: true;
      kind: 'accepted';
...
}): Promise<GoogleVertexVeoGenerateSubmissionResult> {
  const falAttempt =
    params.attemptIndex === null
      ...
      : await createProviderAttempt({
          ...
          requestSnapshot: {
            fallbackFrom: GOOGLE_VERTEX_VEO_PROVIDER,
...
export async function submitGoogleVertexVeoGenerateTask(params: {
  jobId: string;
  userId: string;
  engineId: string;
  engineLabel: string;
  ...
  logMetricFn: LogMetricFn;
  deps?: GoogleVertexVeoSubmissionDeps;
}): Promise<GoogleVertexVeoGenerateSubmissionResult> {
  ...
  try {
    ...
    const operation = await getGoogleVertexVeoClientFn().createTask(payload);
    const operationName = typeof operation.name === 'string' && operation.name.trim() ? operation.name.trim() : null;
    if (!operationName) {
      throw new GoogleVertexVeoError('Google Vertex Veo response did not include an operation name.', {
    ...
    await queryFn(
      `UPDATE app_jobs
          SET status = 'queued',
              ...
              updated_at = NOW()
        WHERE job_id = $1`,
      [params.jobId, 'Render submitted.', GOOGLE_VERTEX_VEO_PROVIDER, operationName]
    );
    ...
    const fallbackEligible = shouldFallbackFromGoogleVertexVeoSubmit({
    ...
    if (!fallbackEligible) {
      const message = userSafeGoogleVertexVeoMessage(normalized.errorClass);
      console.warn('[google-vertex-veo] submit failed without fallback', {
        jobId: params.jobId,
        engineId: params.engineId,
      ...
      params.logMetricFn('failed', {
        ...
        meta: { provider: GOOGLE_VERTEX_VEO_PROVIDER, errorClass: normalized.errorClass },
      });
      return {
        ok: false,
        status:
        ...
        body: {
          ok: false,
          error: normalized.code ?? 'GOOGLE_VERTEX_VEO_SUBMIT_FAILED',
          message,
        },
      };
    }
