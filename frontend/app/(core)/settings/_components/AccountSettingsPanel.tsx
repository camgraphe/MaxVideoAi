  ...
  return (
    <section className="rounded-card border border-border bg-surface p-4 shadow-card sm:p-6">
      ...
      <form className="mt-5 space-y-6" onSubmit={(event) => { event.preventDefault(); void form.save(messages); }}>
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-input border border-border bg-bg p-3 sm:justify-between">
        ...
        <fieldset>
          ...
          <div className="mt-3 grid grid-cols-1 gap-2 min-[360px]:grid-cols-3">
            {(['light', 'dark', 'system'] as const).map((preference) => (
              <Button key={preference} type="button" variant={theme.preference === preference ? 'primary' : 'outline'} aria-pressed={theme.preference === preference} onClick={() => theme.setPreference(preference)} className="min-h-11 px-2">
