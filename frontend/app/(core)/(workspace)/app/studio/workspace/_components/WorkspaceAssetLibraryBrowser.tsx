...
export function WorkspaceAssetLibraryBrowser({
  ...
  const hasControlledSearchQuery = controlledSearchQuery !== undefined;
  const searchQuery = controlledSearchQuery ?? localSearchQuery;
  const selectedAssetIdSet = useMemo(() => new Set(selectedAssetIds), [selectedAssetIds]);

  useEffect(() => {
    if (!hasControlledSearchQuery) {
      setLocalSearchQuery('');
    } else {
      onSearchQueryChange?.('');
    }
  }, [hasControlledSearchQuery, mediaKindFilter, onSearchQueryChange, source]);
