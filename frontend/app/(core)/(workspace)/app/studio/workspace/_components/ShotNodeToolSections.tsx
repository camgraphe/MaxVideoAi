...
function renderCharacterBuilderSection(
  ...
  return renderToolSection('Character Builder', (
    <>
      ...
      {has('characterTraits') ? renderBooleanRow('Hair', characterSettings.traits.hairEnabled, (hairEnabled) => patchTraits({ hairEnabled })) : null}
      ...
      {has('characterTraits') ? renderBooleanRow('Outfit', characterSettings.traits.outfitEnabled, (outfitEnabled) => patchTraits({ outfitEnabled })) : null}
      ...
      {has('characterTraits') ? renderBooleanRow('Close-ups', characterSettings.outputOptions.includeCloseUps, (includeCloseUps) => patchOutputOptions({ includeCloseUps })) : null}
      {has('characterTraits') ? renderBooleanRow('Neutral background', characterSettings.outputOptions.neutralStudioBackground, (neutralStudioBackground) => patchOutputOptions({ neutralStudioBackground })) : null}
      {has('characterTraits') ? renderBooleanRow('Preserve face details', characterSettings.outputOptions.preserveFacialDetails, (preserveFacialDetails) => patchOutputOptions({ preserveFacialDetails })) : null}
      {has('characterTraits') ? renderBooleanRow('Avoid 3D render look', characterSettings.outputOptions.avoid3dRenderLook, (avoid3dRenderLook) => patchOutputOptions({ avoid3dRenderLook })) : null}
      {has('characterTraits') ? <FieldLabel>
        Notes
...
function renderAngleSection(shot: WorkspaceShotSettings, onPatchShot: (patch: Partial<WorkspaceShotSettings>) => void, has: (field: WorkspacePolicyControlField) => boolean) {
  ...
  return renderToolSection('Angle controls', (
    <>
      <div className={styles.settingsGrid}>
        {has('angleRotation') ? <FieldLabel>Rotation<NumberControl value={angle.rotation} min={0} max={360} onChange={(rotation) => patchAngle({ rotation })} /></FieldLabel> : null}
        {has('angleTilt') ? <FieldLabel>Tilt<NumberControl value={angle.tilt} min={-30} max={30} onChange={(tilt) => patchAngle({ tilt })} /></FieldLabel> : null}
        {has('angleZoom') ? <FieldLabel>Zoom<NumberControl value={angle.zoom} min={0} max={10} step={0.1} onChange={(zoom) => patchAngle({ zoom })} /></FieldLabel> : null}
      </div>
      {has('angleSafeMode') ? renderBooleanRow('Safe mode', angle.safeMode, (safeMode) => patchAngle({ safeMode })) : null}
      {has('angleBestAngles') ? renderBooleanRow('4 best angles', angle.generateBestAngles, (generateBestAngles) => patchAngle({ generateBestAngles })) : null}
    </>
  ));
}

function renderUpscaleSection(shot: WorkspaceShotSettings, onPatchShot: (patch: Partial<WorkspaceShotSettings>) => void, has: (field: WorkspacePolicyControlField) => boolean) {
...
function renderStoryboardSection(shot: WorkspaceShotSettings, onPatchShot: (patch: Partial<WorkspaceShotSettings>) => void, has: (field: WorkspacePolicyControlField) => boolean) {
  ...
  return renderToolSection('Storyboard', (
    <div className={styles.settingsGrid}>
      {has('tool.storyboard.targetModel') ? <FieldLabel>Target<SelectControl value={storyboard.targetModel} onChange={(value) => patchStoryboard({ targetModel: value === 'kling' ? 'kling' : 'seedance' })}><option value="seedance">Seedance</option><option value="kling">Kling</option></SelectControl></FieldLabel> : null}
      {has('tool.storyboard.durationSec') ? <FieldLabel>Length<SelectControl value={storyboard.lengthPreset} onChange={(value) => patchStoryboard({ lengthPreset: value as typeof storyboard.lengthPreset })}>{['short', 'medium', 'long'].map((value) => <option key={value} value={value}>{value}</option>)}</SelectControl></FieldLabel> : null}
      {has('tool.storyboard.frameCount') ? <FieldLabel>Frames<SelectControl value={storyboard.frameCount} onChange={(value) => patchStoryboard({ frameCount: Number(value) as typeof storyboard.frameCount })}>{[4, 6, 8].map((value) => <option key={value} value={value}>{value}</option>)}</SelectControl></FieldLabel> : null}
      {has('tool.storyboard.orientation') ? <FieldLabel>Orientation<SelectControl value={storyboard.orientation} onChange={(value) => patchStoryboard({ orientation: value === 'portrait' ? 'portrait' : 'landscape' })}><option value="landscape">Landscape</option><option value="portrait">Portrait</option></SelectControl></FieldLabel> : null}
      {has('tool.storyboard.tier') ? <FieldLabel>Tier<SelectControl value={storyboard.tier} onChange={(value) => patchStoryboard({ tier: value as typeof storyboard.tier })}>{['hd', '4k', 'ultra'].map((value) => <option key={value} value={value}>{value.toUpperCase()}</option>)}</SelectControl></FieldLabel> : null}
    </div>
  ));
}
...
function renderAudioSection(
  ...
  return renderToolSection('Audio', (
    <>
      <div className={styles.settingsGrid}>
        {has('audioMood') ? <FieldLabel>Mood<SelectControl value={audio.mood} onChange={(value) => patchAudio({ mood: value as typeof audio.mood })}>{AUDIO_MOOD_VALUES.map((value) => <option key={value} value={value}>{value}</option>)}</SelectControl></FieldLabel> : null}
        {has('audioIntensity') ? <FieldLabel>Intensity<SelectControl value={audio.intensity} onChange={(value) => patchAudio({ intensity: value as typeof audio.intensity })}>{AUDIO_INTENSITY_VALUES.map((value) => <option key={value} value={value}>{value}</option>)}</SelectControl></FieldLabel> : null}
      </div>
      {has('audioMusicEnabled') && (shot.workflowType === 'cinematic_audio' || shot.workflowType === 'cinematic_voiceover') ? renderBooleanRow('Music bed', audio.musicEnabled, (musicEnabled) => patchAudio({ musicEnabled })) : null}
      {includesVoice ? (
        <div className={styles.settingsGrid}>
          {has('voiceGender') ? <FieldLabel>Voice<SelectControl value={audio.voiceGender} onChange={(value) => patchAudio({ voiceGender: value as typeof audio.voiceGender })}>{AUDIO_VOICE_GENDER_VALUES.map((value) => <option key={value} value={value}>{value}</option>)}</SelectControl></FieldLabel> : null}
          {has('voiceProfile') ? <FieldLabel>Profile<SelectControl value={audio.voiceProfile} onChange={(value) => patchAudio({ voiceProfile: value as typeof audio.voiceProfile })}>{AUDIO_VOICE_PROFILE_VALUES.map((value) => <option key={value} value={value}>{value}</option>)}</SelectControl></FieldLabel> : null}
          {has('voiceDelivery') ? <FieldLabel>Delivery<SelectControl value={audio.voiceDelivery} onChange={(value) => patchAudio({ voiceDelivery: value as typeof audio.voiceDelivery })}>{AUDIO_VOICE_DELIVERY_VALUES.map((value) => <option key={value} value={value}>{value}</option>)}</SelectControl></FieldLabel> : null}
          {has('audioLanguage') ? <FieldLabel>Language<SelectControl value={audio.language} onChange={(value) => patchAudio({ language: value as typeof audio.language })}>{AUDIO_LANGUAGE_VALUES.map((value) => <option key={value} value={value}>{value}</option>)}</SelectControl></FieldLabel> : null}
        </div>
      ) : null}
    </>
  ));
...
export function ShotNodeToolSections({ shot, sections, controlFields, onPatchShot }: ShotNodeToolSectionsProps) {
  ...
  return (
    <>
      {sections.includes('character-builder') && hasAny('characterOutputMode', 'characterConsistencyMode', 'characterQualityMode', 'characterFormatMode', 'characterReferenceStrength', 'characterTraits', 'outputCount') ? renderCharacterBuilderSection(shot, onPatchShot, has) : null}
      {sections.includes('angle-controls') && hasAny('angleRotation', 'angleTilt', 'angleZoom', 'angleSafeMode', 'angleBestAngles') ? renderAngleSection(shot, onPatchShot, has) : null}
      {sections.includes('upscale') && hasAny('upscaleMode', 'upscaleFactor', 'outputFormat') ? renderUpscaleSection(shot, onPatchShot, has) : null}
      {sections.includes('storyboard') && hasAny('tool.storyboard.targetModel', 'tool.storyboard.frameCount', 'tool.storyboard.durationSec', 'tool.storyboard.orientation', 'tool.storyboard.tier') ? renderStoryboardSection(shot, onPatchShot, has) : null}
      {sections.includes('audio-pack') && hasAny('audioMood', 'audioIntensity', 'audioMusicEnabled', 'voiceGender', 'voiceProfile', 'voiceDelivery', 'audioLanguage') ? renderAudioSection(shot, onPatchShot, has) : null}
    </>
  );
}
