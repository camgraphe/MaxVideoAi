...
function renderStoryboardSection(shot: WorkspaceShotSettings, onPatchShot: (patch: Partial<WorkspaceShotSettings>) => void, has: (field: WorkspacePolicyControlField) => boolean) {
  ...
  return renderToolSection('Storyboard', (
    <div className={styles.settingsGrid}>
      ...
      {has('tool.storyboard.durationSec') ? <FieldLabel>Duration<SelectControl value={storyboard.durationSec} onChange={(value) => patchStoryboard({ durationSec: Number(value) as typeof storyboard.durationSec })}>{[6, 10, 15].map((value) => <option key={value} value={value}>{value}s</option>)}</SelectControl></FieldLabel> : null}
