...
import { workspaceOutputCountOptions } from '../../_lib/workspace-output-count';
...
const STORYBOARD_LABEL_COPY_KEYS = {
  'tool.storyboard.targetModel': 'storyboardTargetModel',
  'tool.storyboard.frameCount': 'storyboardFrameCount',
  'tool.storyboard.durationSec': 'storyboardDurationSec',
  'tool.storyboard.orientation': 'storyboardOrientation',
  'tool.storyboard.tier': 'storyboardTier',
} as const satisfies Partial<Record<WorkspacePolicyControlField, keyof StudioControlCopy['labels']>>;

function isLocalizedControlId(
  id: WorkspacePolicyControlField,
  copy: StudioControlCopy
): id is WorkspacePolicyControlField & keyof StudioControlCopy['labels'] {
  return id in copy.labels;
}
...
export function WorkspaceControlField({
  ...
  if (id === 'chatProvider') {
    ...
    return wrapChat(labelOverride ?? copy.labels.chatProvider, <select className={selectClassName} disabled={isDisabled} value={chat.provider} onChange={(event) => {
  ...
  if (id === 'chatModel') {
    ...
    return wrapChat(labelOverride ?? copy.labels.chatModel, <select className={selectClassName} disabled={isDisabled} value={chat.modelId} onChange={(event) => onPatchChat({ modelId: event.currentTarget.value })}>{models.map((model) => <option key={model.modelId} value={model.modelId}>{model.label}</option>)}</select>);
  }
  if (id === 'chatSystemPrompt') {
    if (!chat || !onPatchChat) return null;
    return wrapChat(labelOverride ?? copy.labels.chatSystemPrompt, <textarea className={inputClassName} rows={4} disabled={isDisabled} value={chat.systemPrompt} onChange={(event) => onPatchChat({ systemPrompt: event.currentTarget.value })} />);
  }
  if (id === 'chatMessage') {
    if (!chat || !onPatchChat) return null;
    return wrapChat(labelOverride ?? copy.labels.chatMessage, <textarea className={inputClassName} rows={6} disabled={isDisabled} value={chat.draftMessage} onChange={(event) => {
  ...
  const storyboardLabelKey = STORYBOARD_LABEL_COPY_KEYS[id as keyof typeof STORYBOARD_LABEL_COPY_KEYS];
  const localizedLabel = isLocalizedControlId(id, copy)
    ? copy.labels[id]
    : storyboardLabelKey
      ? copy.labels[storyboardLabelKey]
      : undefined;
  const label = labelOverride ?? metadata.label ?? localizedLabel ?? (id === 'audioEnabled' ? audioOption?.label : id === 'lipSyncEnabled' ? lipSyncOption?.label : undefined) ?? id.replace(/^tool\./, '').replaceAll('.', ' ');
  ...
  if (id === 'audioEnabled') {
    if (audioOption?.control === 'included') return wrap(<strong className={styles.staticValue}>{copy.options.included}</strong>);
  ...
  if (id === 'lipSyncEnabled') {
    if (lipSyncOption?.control === 'included') return wrap(<strong className={styles.staticValue}>{copy.options.included}</strong>);
    if (lipSyncOption?.control === 'toggle') return wrap(toggle(shot.lipSyncEnabled, () => onPatchShot({ lipSyncEnabled: !shot.lipSyncEnabled })));
    return null;
  }
  if (id === 'outputCount') {
    if (shot.toolKind === 'character-builder') return wrap(<select className={selectClassName} disabled={isDisabled} value={character.generateCount} onChange={(event) => patchCharacter({ generateCount: Number(event.currentTarget.value) === 4 ? 4 : 1 })}><option value={1}>1</option><option value={4}>4</option></select>);
    if (shot.toolKind === 'angle') {
      return wrap(<select className={selectClassName} disabled={isDisabled} value={angle.generateBestAngles ? 4 : 1} onChange={(event) => patchTool('angle', { generateBestAngles: Number(event.currentTarget.value) === 4 })}><option value={1}>1</option><option value={4}>4</option></select>);
    }
    const values = workspaceOutputCountOptions(capability?.output_count);
    return wrap(<select className={selectClassName} disabled={isDisabled} value={shot.outputCount ?? values[0]} onChange={(event) => onPatchShot({ outputCount: Number(event.currentTarget.value) })}>{values.map((value) => <option key={value} value={value}>{value}</option>)}</select>);
  }
  if (id === 'characterOutputMode') return wrap(<select className={selectClassName} disabled={isDisabled} value={character.outputMode} onChange={(event) => patchCharacter({ outputMode: event.currentTarget.value as typeof character.outputMode })}>{CHARACTER_OUTPUT_OPTIONS.map((option) => <option key={option.id} value={option.id}>{copy.options[CHARACTER_OUTPUT_COPY_KEYS[option.id]]}</option>)}</select>);
  if (id === 'characterConsistencyMode') return wrap(<select className={selectClassName} disabled={isDisabled} value={character.consistencyMode} onChange={(event) => patchCharacter({ consistencyMode: event.currentTarget.value as typeof character.consistencyMode })}>{CHARACTER_CONSISTENCY_OPTIONS.map((option) => <option key={option.id} value={option.id}>{copy.options[CHARACTER_CONSISTENCY_COPY_KEYS[option.id]]}</option>)}</select>);
  if (id === 'characterQualityMode') return wrap(<select className={selectClassName} disabled={isDisabled} value={character.qualityMode} onChange={(event) => patchCharacter({ qualityMode: event.currentTarget.value as typeof character.qualityMode })}>{CHARACTER_QUALITY_OPTIONS.map((option) => <option key={option.id} value={option.id}>{copy.options[CHARACTER_QUALITY_COPY_KEYS[option.id]]}</option>)}</select>);
  ...
  if (id === 'upscaleMode') return wrap(<select className={selectClassName} disabled={isDisabled} value={upscale.mode} onChange={(event) => patchTool('upscale', { mode: event.currentTarget.value === 'factor' ? 'factor' : 'target' })}><option value="target">{copy.options.upscaleTarget}</option><option value="factor">{copy.options.upscaleFactor}</option></select>);
  ...
  if (id === 'audioMood') return wrap(<select className={selectClassName} disabled={isDisabled} value={audio.mood} onChange={(event) => patchTool('audio', { mood: event.currentTarget.value })}>{AUDIO_MOOD_VALUES.map((value) => <option key={value} value={value}>{copy.options[AUDIO_MOOD_COPY_KEYS[value]]}</option>)}</select>);
  if (id === 'audioIntensity') return wrap(<select className={selectClassName} disabled={isDisabled} value={audio.intensity} onChange={(event) => patchTool('audio', { intensity: event.currentTarget.value })}>{AUDIO_INTENSITY_VALUES.map((value) => <option key={value} value={value}>{copy.options[AUDIO_INTENSITY_COPY_KEYS[value]]}</option>)}</select>);
  if (id === 'audioMusicEnabled') return wrap(toggle(audio.musicEnabled, () => patchTool('audio', { musicEnabled: !audio.musicEnabled })));
  if (id === 'voiceGender') return wrap(<select className={selectClassName} disabled={isDisabled} value={audio.voiceGender} onChange={(event) => patchTool('audio', { voiceGender: event.currentTarget.value })}>{AUDIO_VOICE_GENDER_VALUES.map((value) => <option key={value} value={value}>{copy.options[AUDIO_GENDER_COPY_KEYS[value]]}</option>)}</select>);
  if (id === 'voiceProfile') return wrap(<select className={selectClassName} disabled={isDisabled} value={audio.voiceProfile} onChange={(event) => patchTool('audio', { voiceProfile: event.currentTarget.value })}>{AUDIO_VOICE_PROFILE_VALUES.map((value) => <option key={value} value={value}>{copy.options[AUDIO_PROFILE_COPY_KEYS[value]]}</option>)}</select>);
  if (id === 'voiceDelivery') return wrap(<select className={selectClassName} disabled={isDisabled} value={audio.voiceDelivery} onChange={(event) => patchTool('audio', { voiceDelivery: event.currentTarget.value })}>{AUDIO_VOICE_DELIVERY_VALUES.map((value) => <option key={value} value={value}>{copy.options[AUDIO_DELIVERY_COPY_KEYS[value]]}</option>)}</select>);
  if (id === 'audioLanguage') return wrap(<select className={selectClassName} disabled={isDisabled} value={audio.language} onChange={(event) => patchTool('audio', { language: event.currentTarget.value })}>{AUDIO_LANGUAGE_VALUES.map((value) => <option key={value} value={value}>{copy.options[AUDIO_LANGUAGE_COPY_KEYS[value]]}</option>)}</select>);
  ...
  if (typeof field !== 'string') {
    const value = field.kind === 'connection'
      ? formatCopyValue(copy.options.connected, { count: Number(field.value ?? 0) })
      : String(field.value ?? '');
