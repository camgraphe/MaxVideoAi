...
export function mediaSubtitleForGeneratedNode(node: WorkspaceGraphNode): string {
  ...
  const dimensions = output.kind !== 'audio' && output.sourceMetadata?.measurementStatus === 'measured'
    && output.sourceMetadata.width && output.sourceMetadata.height
    ? { width: output.sourceMetadata.width, height: output.sourceMetadata.height }
    : null;
  const dimensionsLabel = dimensions ? `${dimensions.width}x${dimensions.height}` : null;
  const durationSec = output.sourceMetadata?.measurementStatus === 'measured'
    ? output.sourceMetadata.durationSec
    : null;
  return [
    durationSec ? formatProjectMediaDuration(durationSec) : null,
    dimensionsLabel,
    dimensionsLabel ? aspectRatioLabelFromDimensions(dimensionsLabel) : null,
    output.modelLabel,
  ].filter(Boolean).join(MEDIA_DETAIL_SEPARATOR);
}
