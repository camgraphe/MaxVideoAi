  ...
  return useMemo(() => {
    return nodes.map((node) => {
      if (node.data.kind !== 'shot' || !node.data.shot) {
        ...
        return {
          ...
          data: {
            ...
            ...(node.data.kind === 'chat' ? { pricingEstimate: pricingEstimates[node.id], mockGeneration: mockMode } : {}),
            onOpenAssetLibrary,
            onSendOutputToTimeline,
            studioCanvasCopy,
          },
      ...
      const policy = resolveWorkspaceBlockPolicy({
        ...
        connectedInputs,
        policyCopy: studioCanvasCopy.controls.policy,
        edgeLabel: (kind) => localizeStudioEdgeKindLabel(kind, studioCanvasCopy.nodes),
      });
