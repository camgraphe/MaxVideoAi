...
import {
  createEmptyWorkspaceCanvasGuideState,
...
export function useWorkspaceCanvasTemplateActions({
  ...
  const handleApplyCanvasTemplate = useCallback(
    (templateId: WorkspaceTemplateId) => {
      ...
      const nextGuideState = createWorkspaceCanvasGuideState(template);
      commitCanvasState(() => ({
        edges: cloneWorkspaceJson(template.edges),
        guideState: cloneWorkspaceJson(nextGuideState),
    ...
    [commitCanvasState, setActiveEditorSurface, setActiveTemplateId, setActiveUserCanvasTemplateId, setCanvasRevision, setNotice, setSelectedNodeId, studioCanvasCopy.templateSummaries, studioNotices.canvasTemplateApplied]
  );
  ...
  const saveCanvasSnapshot = useCallback(
    (name: string): WorkspaceUserCanvasTemplate => {
      ...
      return {
        ...
        guideState: cloneWorkspaceJson(guideState),
        createdAt: timestamp,
        updatedAt: timestamp,
      };
    },
    [edges, guideState, nodes, studioNotices.defaultCanvasTemplateName, userCanvasTemplates.length]
  );
  ...
  const handleSaveActiveCanvasTemplate = useCallback(
    ...
    [activeUserCanvasTemplateId, edges, guideState, nodes, saveCanvasSnapshot, setActiveEditorSurface, setActiveUserCanvasTemplateId, setNotice, studioNotices.canvasTemplateNotFound, studioNotices.canvasTemplateSaved, studioNotices.canvasTemplateSavedAs, updateUserCanvasTemplates, userCanvasTemplates]
  );
  ...
  const handleApplyUserCanvasTemplate = useCallback(
    (templateId: string) => {
      ...
      const nextGuideState = cloneWorkspaceJson(template.guideState ?? createEmptyWorkspaceCanvasGuideState());
      commitCanvasState(() => ({
        edges: nextEdges,
    ...
    [commitCanvasState, setActiveEditorSurface, setActiveUserCanvasTemplateId, setCanvasRevision, setNotice, setSelectedNodeId, studioNotices.canvasTemplateApplied, studioNotices.canvasTemplateNotFound, userCanvasTemplates]
  );
  ...
  const handleCreateCanvasFromTemplate = useCallback(
    (templateId: WorkspaceTemplateId) => {
      ...
      const nextGuideState = createWorkspaceCanvasGuideState(starterTemplate);
      const timestamp = new Date().toISOString();
    ...
    [commitCanvasState, setActiveEditorSurface, setActiveTemplateId, setActiveUserCanvasTemplateId, setCanvasRevision, setNotice, setSelectedNodeId, studioCanvasCopy.templateSummaries, studioNotices.canvasTemplateCreatedFrom, updateUserCanvasTemplates]
  );
  ...
  const handleDuplicateUserCanvasTemplate = useCallback(
    (templateId: string) => {
      ...
      const duplicate: WorkspaceUserCanvasTemplate = {
        ...
        nodes: cloneWorkspaceJson(template.nodes),
        edges: cloneWorkspaceJson(template.edges),
        guideState: cloneWorkspaceJson(template.guideState ?? createEmptyWorkspaceCanvasGuideState()),
