...
import { fieldSearchKey, fieldsFor, hasFieldId, hasFieldType, hasMode } from './model-engine-fields';
...
function workflowTypesFor(engine: EngineCaps): WorkspaceWorkflowType[] {
  ...
  if (hasMode(engine, ['ref2v', 'r2v']) || engine.id.includes('storyboard')) workflows.push('storyboard_to_video');
...
function outputCountForEngine(engine: EngineCaps): WorkspaceOutputCount {
  const field = fieldsFor(engine).find((candidate) => fieldSearchKey(candidate).includes('numimages'));
  if (!field || field.type !== 'number') return 1;
  const min = Math.max(1, Math.floor(field.min ?? 1));
  const max = Math.max(min, Math.floor(field.max ?? min));
...
function buildCapability(engine: EngineCaps): WorkspaceModelCapability {
  ...
  if (typeof outputCount !== 'number' || outputCount > 1) {
    controlFields.push('outputCount');
    pricingRelevantFields.push('outputCount');
  }
  ...
  const requiredInputs = Array.from(new Set(inputConnectors.filter((connector) => connector.required).map((connector) => connector.kind)));
  const requiredInputSet = new Set(requiredInputs);
  const optionalInputs = Array.from(new Set(
    inputConnectors
      .filter((connector) => !connector.required && !requiredInputSet.has(connector.kind))
