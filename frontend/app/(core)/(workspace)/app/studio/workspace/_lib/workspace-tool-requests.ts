...
function outputCountForImageRequest(
  settings: WorkspaceShotSettings,
  policy: WorkspaceRequestPolicy
): number {
  return resolveWorkspaceSelectedOutputCount(settings, policy?.outputCount);
}
...
export function buildWorkspaceImageGenerationRequest({
  ...
  return {
    ...
    prompt,
    numImages: outputCountForImageRequest(settings, policy),
...
export function buildWorkspaceCharacterBuilderRequest({
  ...
  const identityImages = uniqueUrls(identityImageUrls).slice(0, 2);
  const styleImages = uniqueUrls(styleImageUrls).slice(0, 2);
  const referenceImages: CharacterBuilderRequest['referenceImages'] = [
    ...identityImages.map((url, index) => ({
      id: `studio-character-identity-${index + 1}`,
      url,
      role: 'identity' as const,
      name: `Identity ${index + 1}`,
    })),
    ...styleImages.map((url, index) => ({
