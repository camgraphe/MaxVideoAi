...
import type { StudioCopy, StudioGuideCopyKey } from '../../_lib/studio-copy';
...
export type WorkspaceOutputMetadata = {
  ...
  sourceMetadata?: {
    measurementStatus: 'unknown' | 'measured';
  ...
  outputIndex?: number;
  outputCount?: number;
  /** Legacy request fields retained only for persisted V1 migration. */
  durationSec?: number;
  aspectRatio?: AspectRatio;
  resolution?: Resolution;
...
export type WorkspaceInputConnector = {
  ...
  supportedInModes?: WorkspaceBlockMode[];
  fieldId?: string;
  description?: string;
  minCount?: number;
  maxCount?: number;
...
export type WorkspacePublicTemplateId =
  | 'product-ad'
  | 'dev-blocks'
  | 'character-dialogue'
  | 'storyboard-to-video'
...
export type WorkspaceProjectStarterTemplateId =
  | 'guided-product-ad'
  | 'guided-storyboard-to-video'
  | 'guided-cinematic-scene';

export type WorkspaceGuideStepId = 'reference' | 'prompt' | 'generate' | 'output' | 'timeline';
export type WorkspaceGuideCopyKey = StudioGuideCopyKey;
export type WorkspaceGuidePlacement = 'top' | 'right' | 'bottom' | 'left';

export type WorkspaceGuideAnchor =
  | { kind: 'node'; nodeId: string; target?: 'header' | 'body' | 'action' | 'input-port' | 'output-port' }
  | { kind: 'generated-output'; sourceNodeId: string }
  | { kind: 'surface'; surface: 'viewer-tab' | 'timeline' };

export type WorkspaceGuideAnnotation = {
  id: string;
  order: number;
  copyKey: WorkspaceGuideCopyKey;
  anchor: WorkspaceGuideAnchor;
  preferredPlacement: WorkspaceGuidePlacement;
  manualPosition?: { x: number; y: number };
  originTemplateId: WorkspaceProjectStarterTemplateId;
};

export type WorkspaceCanvasGuideState = {
  annotations: WorkspaceGuideAnnotation[];
  hidden: boolean;
  sourceTemplateId?: WorkspaceProjectStarterTemplateId;
};

export type WorkspaceGuideFocusRequest = {
  annotationId: string;
  requestId: number;
};
...
export type WorkspaceTemplate = {
  ...
  guideAnnotations?: WorkspaceGuideAnnotation[];
  focusNodeId?: string;
};
