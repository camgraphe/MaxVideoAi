...
export function validateShotConnections(params: {
  ...
  const incompatibleInputs = Array.from(connected).filter((kind) => (
    disabledInputs.has(kind) || !inputSupportedBy(kind, supportedInputs)
  ));
  const compatibleInputs = Array.from(connected).filter((kind) => (
    !disabledInputs.has(kind) && inputSupportedBy(kind, supportedInputs)
  ));
