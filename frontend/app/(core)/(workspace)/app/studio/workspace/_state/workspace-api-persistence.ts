...
import { normalizeWorkspaceTimelineItemsSourceMetadata } from '../_lib/timeline/timeline-source-metadata';
import type {
...
export type StudioProjectWorkspaceSnapshotKind = 'missing' | 'empty' | 'persisted' | 'invalid';

export function classifyStudioProjectWorkspaceSnapshot(value: unknown): StudioProjectWorkspaceSnapshotKind {
  if (value === null || value === undefined) return 'missing';
  if (typeof value !== 'object' || Array.isArray(value)) return 'invalid';
  if (Object.keys(value).length === 0) return 'empty';

  const record = value as Partial<PersistedWorkspaceState>;
  return Array.isArray(record.nodes) && Array.isArray(record.edges) && Array.isArray(record.timelineItems)
    ? 'persisted'
    : 'invalid';
}
...
export function shouldApplyStudioProjectWorkspaceState(
  ...
  if (classifyStudioProjectWorkspaceSnapshot(workspaceState) !== 'persisted') return false;
...
export function normalizeUserCanvasTemplate(value: unknown): WorkspaceUserCanvasTemplate | null {
  ...
  return {
    ...
    nodes,
    edges,
    guideState: record.guideState
      ? normalizeWorkspaceCanvasGuideState(record.guideState, nodes)
      : createEmptyWorkspaceCanvasGuideState(),
...
function normalizePersistedProjectAsset(value: unknown): WorkspaceAssetRecord | null {
  ...
  return {
    ...
    audioProvenance:
      record.audioProvenance === 'none' ||
      record.audioProvenance === 'embedded' ||
      record.audioProvenance === 'external' ||
      record.audioProvenance === 'unknown'
        ? record.audioProvenance
        : undefined,
...
export function normalizePersistedWorkspaceState(
  ...
  const guideState = parsed.guideState
    ? normalizeWorkspaceCanvasGuideState(parsed.guideState, nodes)
    : createEmptyWorkspaceCanvasGuideState();
  const timelineItems = migrateStarterGeneratedCopyForTimelineItems(
    activeTemplateId,
    normalizeWorkspaceTimelineIdentities(normalizeWorkspaceTimelineItemsSourceMetadata(
      normalizeTimelineMediaUrls(nodes, parsed.timelineItems)
    ))
  );
