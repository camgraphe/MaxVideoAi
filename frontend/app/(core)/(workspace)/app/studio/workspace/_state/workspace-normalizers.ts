...
function normalizePersistedOutputMetadata(
  output: WorkspaceOutputMetadata,
  sourceShot?: WorkspaceShotSettings
): WorkspaceOutputMetadata {
  const requestedSettings: NonNullable<WorkspaceOutputMetadata['requestedSettings']> = {
    durationSec: positiveNumber(output.requestedSettings?.durationSec)
      ?? positiveNumber(output.durationSec)
      ?? positiveNumber(sourceShot?.durationSec)
      ?? 5,
    aspectRatio: output.requestedSettings?.aspectRatio
      ?? output.aspectRatio
      ?? sourceShot?.aspectRatio
      ?? '16:9',
    resolution: output.requestedSettings?.resolution
      ?? output.resolution
      ?? sourceShot?.resolution
      ?? '1080p',
    fps: positiveNumber(output.requestedSettings?.fps)
      ?? positiveNumber(sourceShot?.fps)
      ?? 30,
    outputCount: Math.max(1, Math.round(
      positiveNumber(output.requestedSettings?.outputCount)
        ?? positiveNumber(output.outputCount)
        ?? positiveNumber(sourceShot?.outputCount)
        ?? 1
    )),
  };
  return {
...
export function normalizeGeneratedOutputNodes(nodes: WorkspaceGraphNode[]): WorkspaceGraphNode[] {
  const shotSettingsById = new Map(nodes.flatMap((node): Array<[string, WorkspaceShotSettings]> => (
    node.data.kind === 'shot' && node.data.shot ? [[node.id, node.data.shot]] : []
  )));
  return nodes.map((node) => {
    if (node.data.kind !== 'output') return node;
    const output = node.data.output
      ? normalizePersistedOutputMetadata(node.data.output, shotSettingsById.get(node.data.output.sourceShotId))
      : node.data.output;
    const sourceHandle = generatedOutputSourceHandle(output);
    if (
...
export function normalizeWorkspaceGenerationNode(node: WorkspaceGraphNode): WorkspaceGraphNode {
  ...
  const normalizedShot: WorkspaceShotSettings = {
    ...
    outputCount: Number.isFinite(shot.outputCount) && (shot.outputCount ?? 0) > 0
      ? Math.floor(shot.outputCount!)
      : 1,
  };
  const normalizedToolShot = normalizeWorkspaceShotToolSettings(normalizedShot);
  if (shot.family && shot.outputKind && shot.outputCount && normalizedToolShot === shot) return node;
  return {
    ...node,
    data: {
      ...node.data,
...
export function normalizeTimelineMediaUrls(nodes: WorkspaceGraphNode[], items: WorkspaceTimelineItem[]): WorkspaceTimelineItem[] {
  return items.map((item) => {
    ...
    const itemWithSourceDimensions = sourceDimensions &&
      ...
      : normalizedItem;
    if (
      itemWithSourceDimensions.mediaKind !== 'audio'
      && itemWithSourceDimensions.mediaUrl
      && itemWithSourceDimensions.mediaUrl !== STALE_EMPTY_DEMO_AUDIO_URL
    ) {
      return itemWithSourceDimensions;
    }
    const output = sourceNode?.data.output;
    const asset = sourceNode?.data.asset;
    const currentAudioUrl = normalizePlayableAudioUrl(itemWithSourceDimensions.mediaUrl);
    const dedicatedAudioUrl = [
      output?.audioUrl,
      asset?.audioUrl,
      output?.kind === 'audio' ? output.url : null,
      asset?.kind === 'audio' ? asset.url : null,
    ].map((url) => normalizePlayableAudioUrl(url)).find((url): url is string => Boolean(url)) ?? null;
    const embeddedAudioUrl = itemWithSourceDimensions.audioProvenance === 'embedded'
      ? [currentAudioUrl, output?.url, asset?.url]
