...
import { defaultTimelineSelection, defaultTimelineSelectionIds, workspaceTimelineCutPoints } from './_lib/workspace-timeline-selection';
import {
...
export default function WorkspacePage({ projectId }: WorkspacePageProps) {
  ...
  const canvasGraph = useWorkspaceCanvasGraphState({
    ...
    defaultSelectedNodeId: defaultTemplate.focusNodeId,
  });
  const { canvasHistoryController, edges, guideState, nodes, selectedNodeId, setEdges, setGuideState, setNodes, setSelectedNodeId } = canvasGraph;
  const [guideFocusRequest, setGuideFocusRequest] = useState<WorkspaceGuideFocusRequest | null>(null);
  ...
  const [activeTemplateId, setActiveTemplateId] = useState<WorkspaceTemplateId>(MINIMAL_START_WORKSPACE_TEMPLATE_ID);
  ...
  useWorkspaceCompletedExportAssets({
    ...
    exportManifest: exportController.submittedExportManifest,
    onNotice: setNotice,
  ...
  const canvasController = useWorkspaceCanvasController({
    ...
    capabilities,
    commitCanvasGuideState: canvasHistoryController.commitCanvasGuideState,
    commitCanvasGraph: canvasHistoryController.commitCanvasGraph,
    commitCanvasState: canvasHistoryController.commitCanvasState,
  ...
  const shellActions = useWorkspaceShellActions({
    ...
    activeTemplateName,
    buildPersistedWorkspaceState: sequenceSnapshots.buildPersistedWorkspaceState,
    connected: persistence.connected, exitReady: persistence.exitReady,
