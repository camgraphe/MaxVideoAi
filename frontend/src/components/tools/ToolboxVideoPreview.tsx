...
export function ToolboxVideoPreview({ locale, sourceUrl, result, viewMode, onViewModeChange, onSave, onDownload }: { locale: string; sourceUrl: string; result: BackgroundRemovalToolResponse | null; viewMode: 'source' | 'result'; onViewModeChange: (mode: 'source' | 'result') => void; onSave: () => void; onDownload: () => void }) {
  const copy = toolboxCopy(locale);
  const [failedUrl, setFailedUrl] = useState<string | null>(null);
  const url = viewMode === 'result' ? result?.output?.url || sourceUrl : sourceUrl;
  if (!url) return <ToolEmptyPreview locale={locale} visual="background-removal" />;
  return <div className="overflow-hidden rounded-2xl border border-border bg-surface">
    ...
    <div className="relative aspect-video bg-[conic-gradient(#b2b4ae_25%,#d6d7d0_0_50%,#b2b4ae_0_75%,#d6d7d0_0)] bg-[length:24px_24px]">
      <video key={url} src={url} preload="none" controls playsInline className="absolute inset-0 h-full w-full object-contain" onError={() => setFailedUrl(url)} />
    </div>
