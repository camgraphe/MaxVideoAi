...
export function ToolboxCatalogue({ locale, mediaKind, onSelect }: { locale: string; mediaKind?: ToolMediaKind; onSelect?: (id: string, kind?: ToolMediaKind) => void }) {
  ...
  return <div className={styles.catalogue}>
    <header className={styles.heading}><span className={styles.eyebrow}>MAXVIDEOAI / TOOLBOX</span><h1>{copy.title}</h1><p>{copy.subtitle}</p></header>
    ...
    {workshops.length > 0 ? <section className={styles.workshopSection} aria-labelledby={`${id}-workshops`}>
      <div className={styles.sectionBar}><h2 id={`${id}-workshops`}>{copy.workshops}</h2><p>{copy.workshopHint}</p></div>
    ...
    <footer className={styles.footer}><span aria-hidden="true">◇</span>{copy.preserved}</footer>
  </div>;
}
