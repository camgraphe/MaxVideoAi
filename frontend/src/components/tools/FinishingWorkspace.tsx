...
export function FinishingSession({ auth, toolId, releasedQualities = [] }: { auth: ReturnType<typeof useRequireAuth>; toolId: FinishingToolId; releasedQualities?: ToolQuality[] }) {
  ...
  return <ToolWorkbench locale={locale} visual={toolId}
    ...
    settings={<section className={styles.finishingSettings}>
      ...
      <button type="button" className="mt-5 min-h-11 w-full rounded-lg bg-brand px-4 py-3 text-sm font-semibold text-on-brand" disabled={!released || !auth.user || running || busy || !pricing.ready} onClick={() => void run()}>{running ? copy.processing : pricing.loading ? copy.priceLoading : !releasedQualities.includes(settings.quality) ? labels.unavailable : labels.run}{pricing.quote && !running ? ` · ${new Intl.NumberFormat(locale, { style: 'currency', currency: pricing.quote.currency }).format(pricing.quote.totalCents / 100)}` : ''}</button>
      {!releasedQualities.includes(settings.quality) ? <p className="mt-2 text-xs text-text-secondary">{labels.unavailableHint}</p> : null}
