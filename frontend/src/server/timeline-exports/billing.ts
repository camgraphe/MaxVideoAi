...
export async function reserveTimelineExportBilling(params: {
  ...
  return withDbTransaction(async (executor) => {
    await lockUserWalletInExecutor(executor, params.userId);
...
export async function createTimelineExportJobWithReservation(params: {
  ...
  return withDbTransaction(async (executor) => {
    ...
    if (existingJob) {
      const sameManifest = timelineExportManifestHash(existingJob.render_manifest)
        === timelineExportManifestHash(params.renderManifest);
      const sameSettings = timelineExportManifestHash(existingJob.export_settings)
        === timelineExportManifestHash(params.exportSettings);
      if (!sameManifest || !sameSettings || existingJob.quality_preset !== params.qualityPreset) {
    ...
    verifyTimelineExportEstimateToken({
      token: params.estimateToken,
      secret: params.estimateSecret,
      now: params.now,
      expected: {
        userId: params.userId,
        manifestHash: timelineExportManifestHash(params.renderManifest),
        qualityPreset: params.qualityPreset,
