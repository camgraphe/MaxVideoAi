...
export function orderSoundDesignProviders(sourceVideoUrl?: string | null): AudioProviderCandidate[] {
  if (sourceVideoUrl) return AUDIO_PROVIDER_ROSTER.soundDesign;
  return AUDIO_PROVIDER_ROSTER.soundDesign.filter((candidate) =>
    candidate.key === 'mmaudio_v2_text'
  );
}
...
export async function generateSoundDesignTrack(input: {
  ...
  prompt?: string | null;
}, options?: { subscribe?: AudioProviderSubscribe }): Promise<AudioProviderResult> {
