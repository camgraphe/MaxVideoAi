...
type MediaRow = { id: string; public_id?: string; job_id?: string; user_id: string; kind: string; url: string; storage_url?: string | null; thumb_url?: string | null; preview_url?: string | null; mime_type?: string | null; metadata?: Record<string, unknown>; status: string; deleted_at?: unknown; hidden?: boolean; job_user_id?: string };
...
export async function resolveStudioMedia(userId: string, input: unknown, execute: StudioMediaExecutor = query) {
    ...
    ? await execute(`SELECT a.*, j.hidden, j.user_id AS job_user_id FROM media_assets a
        ...
        WHERE a.user_id = $1 AND a.public_id = $2 AND a.kind = $3
          ...
          AND (a.source_output_id IS NULL OR (source_output.user_id = $1 AND source_output.status = 'ready' AND j.user_id = $1
            AND (a.source_job_id IS NULL OR a.source_job_id = source_output.job_id))) LIMIT 1`, [userId, ref.assetId, ref.kind])
  ...
  return { id: row.id, ref: ref as ToolAssetRef, kind: ref.kind, url, thumbUrl: row.thumb_url ?? null,
