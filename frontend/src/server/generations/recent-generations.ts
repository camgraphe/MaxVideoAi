...
import { mapRecentGenerationRecordToWeb } from './recent-generation-web-mapper';

export { mapRecentGenerationRecordToWeb };
...
const ECMASCRIPT_TRIM_CHARACTERS_SQL = `CONCAT(
  E'\\t\\n\\f\\r ',
  ...
  const hasValidRenderEntry = `EXISTS (
    SELECT 1
     ...
     WHERE (jsonb_typeof(render_entry.value) = 'string'
       AND BTRIM(COALESCE(render_entry.value #>> '{}', ''), ${ECMASCRIPT_TRIM_CHARACTERS_SQL}) <> '')
        OR (jsonb_typeof(render_entry.value) = 'object'
       AND jsonb_typeof(render_entry.value->'url') = 'string'
       AND BTRIM(COALESCE(render_entry.value->>'url', ''), ${ECMASCRIPT_TRIM_CHARACTERS_SQL}) <> ''
     )
  )`;
