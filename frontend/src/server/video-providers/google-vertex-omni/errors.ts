...
export type GoogleVertexOmniErrorClass =
  | 'timeout'
  | 'rate_limited'
...
export class GoogleVertexOmniError extends Error {
  status: number | null;
  code: string | null;
  errorClass: GoogleVertexOmniErrorClass;
  raw: unknown;
  ...
  constructor(
    ...
    super(message);
    this.name = 'GoogleVertexOmniError';
...
function extractMessage(error: unknown, payload: unknown): string {
  ...
  return 'Gemini Omni Flash request failed.';
}

function classifyByMessage(message: string): GoogleVertexOmniErrorClass | null {
  const normalized = message.toLowerCase();
  if (normalized.includes('abort') || normalized.includes('timeout') || normalized.includes('timed out')) return 'timeout';
  if (normalized.includes('safety') || normalized.includes('policy') || normalized.includes('rai')) return 'moderation';
  ...
  if (normalized.includes('unsupported') || normalized.includes('not support')) return 'unsupported_params';
  return null;
}

function classify(status: number | null, code: string | null, message: string): GoogleVertexOmniErrorClass {
  if (code === 'GOOGLE_VERTEX_OMNI_NO_INTERACTION_ID') return 'invalid_response';
  if (code === 'GOOGLE_VERTEX_OMNI_UNSUPPORTED_PARAMS') return 'unsupported_params';
