...
export function PlaylistsSidebar({
  ...
  const groups = Object.entries(groupedPlaylists).filter(
    ([key]) => key !== "draft" || showDraftCollections,
  );
  return (
    <aside className="min-w-0 border-border xl:border-r xl:pr-5">
      ...
      <div
        className="max-h-[65vh] space-y-5 overflow-y-auto"
        aria-label="Site destinations"
      >
        {groups.map(([key, entries]) => {
          const matches = entries.filter((entry) =>
            `${entry.name} ${entry.drivesRoute ?? ""} ${entry.slug}`
              .toLowerCase()
                ...
                {matches.map((entry) => (
                  ...
                  >
                    <span className="flex justify-between gap-2 text-sm font-medium">
                      ...
                      {!enableCuration ||
                      !["examplesHub", "family", "model"].includes(
                        entry.surfaceRole,
                      ) ? (
