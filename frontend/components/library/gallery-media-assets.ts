...
export function galleryMediaAssets(group: GroupSummary, kind: 'image' | 'video'): AssetBrowserAsset[] {
  return group.members.flatMap((member, index) => {
    if (member.status === 'pending' || member.status === 'failed') return [];
    const imageIndex = Number(member.id.match(/-image-(\d+)$/)?.[1] ?? index + 1) - 1;
    const url = kind === 'image' ? member.originalUrl ?? member.job?.renderIds?.[imageIndex] : member.videoUrl ?? member.job?.videoUrl;
