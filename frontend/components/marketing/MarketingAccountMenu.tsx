  ...
  return (
    <div className="relative">
      ...
      {accountMenuOpen ? (
        <div
          ref={menuRef}
          className="absolute right-0 mt-3 flex max-h-[calc(100dvh-5rem)] w-56 flex-col overflow-hidden rounded-card border border-hairline bg-surface p-3 text-sm text-text-primary shadow-card"
          role="menu"
        >
          <div className="mb-3 shrink-0 rounded-input bg-bg px-3 py-2">
          ...
          <nav className="mb-2 flex min-h-0 flex-col gap-1 overflow-y-auto overscroll-y-contain" aria-label={t('nav.primaryNavigation', 'Primary navigation')}>
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  prefetch={false}
                  className="flex shrink-0 items-center justify-between rounded-input px-3 py-2 text-sm font-medium text-text-primary transition hover:bg-surface-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          ...
          <Button
            ...
            size="sm"
            variant="ghost"
            className="min-h-0 h-auto w-full shrink-0 justify-between rounded-input px-3 py-2 text-sm font-medium text-text-primary hover:bg-surface-2"
            onClick={onSignOut}
          >
