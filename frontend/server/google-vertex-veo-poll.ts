...
import { GOOGLE_VERTEX_VEO_PROVIDER, resolveGoogleVertexVeoModelRoute } from '@/server/video-providers/google-vertex-veo/model-map';
import {
  extractGoogleVertexVeoOutput,
  normalizeGoogleVertexVeoOperation,
  type GoogleVertexVeoOutput,
} from '@/server/video-providers/google-vertex-veo/response';
import {
...
type GoogleVertexVeoPendingJob = {
  job_id: string;
  user_id: string | null;
  engine_id: string;
  engine_label: string;
...
function getResolutionFromSettingsSnapshot(snapshot: unknown): string | null {
  const core = asRecord(asRecord(snapshot)?.core);
  const resolution = core?.resolution;
  return typeof resolution === 'string' && resolution.trim() ? resolution.trim() : null;
}

async function recordWalletRefundOnce(job: GoogleVertexVeoPendingJob, reason: string, queryFn: QueryFn) {
...
async function markJobPollingStalled(job: GoogleVertexVeoPendingJob, queryFn: QueryFn) {
  await queryFn(
    `UPDATE app_jobs
  ...
  if (attempt) {
    await markProviderAttemptFailed({
      attemptId: attempt.id,
      errorCode: 'GOOGLE_VERTEX_VEO_POLLING_STALLED',
...
function buildCostBreakdown(job: GoogleVertexVeoPendingJob) {
  const resolution = getResolutionFromSettingsSnapshot(job.settings_snapshot) ?? '720p';
  const estimate = estimateGoogleVertexVeoCost({
  ...
  return {
    ...
    provider_model: resolveGoogleVertexVeoModelRoute(job.engine_id).providerModel,
...
async function copyGoogleVeoOutputToStorage(params: {
  output: GoogleVertexVeoOutput;
  ...
  const source = params.output.bytesBase64Encoded
    ? {
        data: Buffer.from(params.output.bytesBase64Encoded, 'base64'),
        mime: params.output.mimeType || 'video/mp4',
      }
    : params.output.gcsUri
      ? await params.client.downloadGcsUri(params.output.gcsUri)
      : null;
...
export async function runGoogleVertexVeoPoll(options: { deps?: GoogleVertexVeoPollDeps } = {}) {
  ...
  for (const job of rows) {
    ...
    try {
      ...
      const operation = await client.fetchOperation({
        providerModel: route.providerModel,
        operationName: job.provider_job_id,
      });
      const task = normalizeGoogleVertexVeoOperation(operation, job.provider_job_id);
      const estimate = estimateGoogleVertexVeoCost({
        engineId: job.engine_id,
        durationSec: job.duration_sec,
        audioEnabled: job.has_audio === true,
      ...
      const normalized = classifyGoogleVertexVeoError(error);
      console.warn('[google-vertex-veo-poll] status fetch failed', {
        jobId: job.job_id,
        providerJobId: job.provider_job_id,
