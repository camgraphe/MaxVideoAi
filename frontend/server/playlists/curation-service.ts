...
import { parseCurationDraft, resolveCuration, type CurationDraft, type CurationItem, type CurationPreview } from '@/lib/admin/playlist-curation';
import { CurationError, curationFingerprint, getCurationAliases, lockCuration, readCurationSnapshot } from './curation-store';
...
export async function listCurationCandidates(slug: string, db: QueryExecutor = { query }): Promise<CurationItem[]> {
  ...
  const rows = await db.query<{ job_id:string; engine_id:string; engine_label:string|null; prompt:string|null; thumb_url:string|null; video_url:string; created_at:string }>(
    `SELECT job_id,engine_id,engine_label,prompt,thumb_url,video_url,created_at
     ...
     ORDER BY created_at DESC, job_id ASC`, [aliases.map(alias => alias.toLowerCase())]);
  return rows.map(row => ({ id: row.job_id,engineId: row.engine_id,engineLabel: row.engine_label,prompt:row.prompt??'',thumbUrl:row.thumb_url,videoUrl:row.video_url,createdAt:new Date(row.created_at).toISOString() }));
}

async function buildPreview(playlistId: string, input: unknown, revision: string, db: QueryExecutor): Promise<CurationPreview> {
  ...
  if (draft.orderedIds.some(id => !eligible.has(id))) throw new CurationError('Some selected media are no longer eligible. Reload the destination.');
...
export async function saveCuration(playlistId:string,input:unknown,revision:string,token:string,actor:string|null) {
  return withDbTransaction(async db => {
    ...
    const draft=parseCurationDraft(input);
    await db.query(`INSERT INTO playlist_curations(playlist_id,mode,ordered_ids,excluded_ids,updated_by)
...
export async function resolveCuratedPlaylist(slug:string):Promise<CurationItem[]|null> {
  ...
  try {
    rows=await query(`SELECT p.is_public,c.mode,c.ordered_ids,c.excluded_ids FROM playlists p
      JOIN playlist_curations c ON c.playlist_id=p.id WHERE p.slug=$1`,[slug]);
  } catch(error) {
  ...
  return resolveCuration({mode:saved.mode,orderedIds:saved.ordered_ids,excludedIds:saved.excluded_ids},await listCurationCandidates(slug));
}
...
export async function hasPlaylistCuration(slug: string): Promise<boolean> {
  ...
  try {
    return (await query('SELECT 1 FROM playlist_curations c JOIN playlists p ON p.id=c.playlist_id WHERE p.slug=$1',[slug])).length > 0;
  } catch(error) { if ((error as {code?:string}).code==='42P01') return false; throw error; }
}
