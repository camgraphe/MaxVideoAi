import {connectSection} from './connect-section.mjs';
import {writeFile} from 'node:fs/promises';
import {renderReview} from './rebuild.mjs';
import {library} from './master-data.mjs';
import {modelNetwork,catalogDialog,compareSection,link,arrow} from './master-sections.mjs';
...
const guides=between('<section class="guides','<section class="angle-section')
 ...
 .replace('01 / FIND YOUR STARTING POINT','FIND YOUR STARTING POINT');
...
html=html.replace(/(<header class="header shell">[\s\S]*?<nav[^>]*>)[\s\S]*?(<\/nav>)/,(m,a,b)=>a+nav+b)
 ...
 .replace(/<div class="review-bar"><span>[\s\S]*?<\/span>/,'<div class="review-bar"><span>English master · 05 <i>· LOCAL REVIEW</i></span>')
 .replace('<link rel="stylesheet" href="home/rebuild.css">','<link rel="stylesheet" href="home/rebuild.css"><link rel="stylesheet" href="home/finish.css">')
 ...
 .replace(/(<dialog id="mobile-dialog"[\s\S]*?<nav>)[\s\S]*?(<\/nav>)/,(m,a,b)=>a+[['hero','Results'],['menu','Menu'],['compare','Compare'],['connect','Connect'],['tools','Tools'],['pricing','Pricing']].map(([id,label])=>' <button data-preview-section="'+id+'">'+label+'</button>').join('')+b);
...
html=html.replace('04 / STAY IN CONTROL','STAY IN CONTROL');
...
console.log('Built English master 05 and retained Angle study. FR/ES remain archived proposition 03.');
