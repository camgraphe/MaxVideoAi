...
| --- | --- | --- |
| OpenClaw Gateway | [OpenClaw MCP transports and OAuth](https://docs.openclaw.ai/cli/mcp/transports) documents saved remote Streamable HTTP servers, `auth: "oauth"`, login/logout, shared operator credentials, and optional per-requester identity. | **Partial local branch checkpoint (OpenClaw 2026.9.4 on macOS 26.6.2, 2026-09-13 Europe/Madrid).** A disposable profile connected to the branch-local Streamable HTTP endpoint through OAuth, discovered the filtered tool set, read account and live model data, requested recommendations, compared project budgets, prepared an exact quote without creating a job, waited for explicit approval, confirmed one `$0.07` Seedance 1.5 Pro request exactly once, followed it to completion, and invoked result presentation against the connected MaxVideoAI library. Nullable optional tool inputs were accepted. The CLI exposed the library fallback but no inline media URL. OAuth denial, token refresh, revoke/loss/reconnect, deliberately interrupted-response recovery, channel attachment, the production origin, and ClawHub install/update/uninstall remain unverified; registry evidence therefore remains `not-run` and the preview stays noindex. |
