...
| ChatGPT web custom app / full MCP | Current [OpenAI plugin testing guidance](https://developers.openai.com/plugins/deploy/connect-chatgpt) documents a public HTTPS Streamable HTTP server connected from ChatGPT developer mode. The [OAuth guidance](https://developers.openai.com/plugins/build/auth) prefers CIMD while retaining DCR and requires authorization-code PKCE S256, protected-resource and authorization-server metadata, `resource` propagation, and exact callback metadata from the connection management page. A direct developer connection is separate from public plugin submission and review. | **Documentation preflight only (2026-09-14 Europe/Madrid); remains `not-run`.** No new ChatGPT connection, OAuth denial or approval, tool scan, protected call, inline result, quote, job, charge, recovery, revocation, reconnect, plugin draft, submission, or publication was attempted. OpenAI's current plugin-commerce policy leaves the public MaxVideoAI directory track at the separately recorded `do_not_submit` decision, but that does not demote the existing direct integration page or prove the host. Full lifecycle testing waits for hosted verification of MaxVideoAI's active-grant revocation fix. |
| Claude Code | Current [Claude Code MCP guidance](https://code.claude.com/docs/en/mcp) documents remote Streamable HTTP, OAuth discovery, DCR, CIMD, secure token storage and refresh, localhost callbacks, per-tool permission rules, resources, login, logout, and reconnect behavior. | **Stable preflight only (Claude Code stable 2.1.236; 2026-09-14 Europe/Madrid); remains `not-run`.** The exact stable can be pinned and checked through its signed manifest plus per-platform SHA-256 before execution; no binary was installed or run in this preflight. Stable 2.1.236 supports Streamable HTTP and uses `http://localhost:<random port>/callback` by default, with DCR and CIMD available. Default tool approval remains required and must not be bypassed. Anthropic does not document whether logout calls the authorization server's revocation endpoint, and result-level `resource_link` rendering is not guaranteed, so both require hosted observation with a text fallback. No OAuth, protected tool, quote, job, charge, generation, or marketplace action was attempted. Full lifecycle testing waits for hosted verification of MaxVideoAI's active-grant revocation fix. |
...
| Microsoft Copilot Studio | [Copilot Studio existing-server setup](https://learn.microsoft.com/en-us/microsoft-copilot-studio/mcp-add-existing-server-to-agent) documents Streamable HTTP only, with an onboarding wizard or Power Apps custom connector. OAuth options include dynamic discovery and DCR, DCR with manually supplied endpoints, and a static client. The callback URI is generated during setup and must be copied exactly rather than guessed or wildcarded. The connector boundary also places the server and its tools under Power Platform data policies. | **Documentation preflight only; not run against MaxVideoAI (2026-09-14 Europe/Madrid).** No owner-controlled non-production tenant, eligible Copilot Studio maker licence, distinct administrator, test-user pair, environment policy, generated callback, OAuth connection, protected tool, quote, job, charge, or recovery path was exercised. A future direct-host checkpoint must begin with an unpublished test agent, end-user credentials, dynamic discovery, and a read-only tool subset or non-paying server profile before it can approach the paid `confirm_generation` boundary. Copilot Studio remains registry `not-run`, hidden, non-indexable, and acquisition-disabled. |
| Microsoft Agent 365 | [Microsoft 365 Agent Tools and BYO MCP guidance](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/manage-tools-for-agent) documents tenant registration, approval, runtime blocking, and observability for remote MCP servers. | **Documentation preflight only; not run against MaxVideoAI (2026-09-14 Europe/Madrid).** Although the Agent 365 product is generally available, its BYO remote-MCP path remains preview. It requires Agent 365 CLI 1.1.165-preview or later, a provisioned tenant service principal, and an internet-reachable endpoint. The documented ExternalOAuth registration uses a static client ID and secret rather than DCR, creates Entra application state, then requires an AI Admin or Global Admin to approve the server and grant tenant-wide consent; availability in Copilot Studio can take up to 30 minutes. The preview does not support republishing or deleting a registered BYO MCP server, so a disposable tenant and an explicit manual-cleanup inventory are prerequisites. Server blocking is enforced at runtime and invocation metadata can be inspected through Defender Advanced Hunting, but neither was tested. Agent 365 registration, Copilot Studio reuse, OAuth, governance, observability, removal, and tool filtering all remain unverified. This host remains registry `not-run`, hidden, non-indexable, and acquisition-disabled. |
