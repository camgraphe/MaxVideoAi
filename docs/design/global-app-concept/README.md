...
- Créer → Départ / Fin / Ajouter → choisir/importer/créer une référence dans le rôle disponible.
...
- Changer de modèle → conserver les médias et résoudre les incompatibilités. Aucun scénario de démonstration à choisir avant d’ajouter une référence.
...
La variante `?volume=50` ajoute des copies identifiées comme médias de test pour éprouver la densité. Pour les six modèles vidéo exposés, les rôles, plafonds et modes compatibles sont projetés depuis le catalogue réel ; les autres activités conservent leurs fixtures de présentation. Cette projection ne remplace pas la validation de fichiers, les budgets partagés ni les devis du produit. Aucun rendu, devis personnalisé, paiement, upload serveur, changement de compte réel ou projet Studio n’est produit. Les brouillons vivent en mémoire ; seule l’apparence persiste sous une clé propre à cette maquette.
...
Le lecteur audio est réel ; la forme d’onde est un motif décoratif, pas une analyse du fichier. Les médias copiés n’ont pas été optimisés pour mesurer une performance de production. L’intégration dans le produit réutilise ses contrats de médias, de génération, de références et de pagination ; elle est suivie dans `docs/superpowers/plans/2026-09-07-connected-app-integration.md`.
