...
- Product target: **public flagship launch** with all five modes:
  ...
  extension.
- Proven direct execution: text-to-video, image-to-video,
  reference-to-video, and extension use ModelArk `/api/v3`. Completed
  production jobs prove the image, image-reference, audio-reference,
  video-reference, and extension-source paths. Video-to-video alone targets
  LAS `/api/v1` and remains execution-gated until its LAS-specific quote and
  accounting are approved and an authenticated canary passes.
...
Seedance 2.5 text-to-video, image-to-video, reference-to-video, and extension
submission and polling use the existing server-only `BYTEPLUS_ARK_API_KEY` and
ModelArk `/api/v3`. Video-to-video uses LAS `/api/v1` and requires a separate
server-only `BYTEPLUS_LAS_API_KEY`. The LAS credential alone does not enable
V2V: `SEEDANCE_2_5_LAS_ENABLED=true` is allowed only after the LAS quote,
charge, reconciliation, refund, and canary checks are approved. The
...
- ModelArk text, image, reference, and extension execution is the current proven direct route.
- LAS V2V execution is pending exact pricing/accounting and a bounded
...
and `extend`. The executable ModelArk contract currently exposes `t2v`, `i2v`,
`ref2v`, and `extend`. It supports 4–30 seconds, 480p or 720p, all six explicit
aspect ratios (`21:9`, `16:9`, `4:3`, `1:1`, `3:4`, and `9:16`), 24 FPS,
generated audio, and up to 50 combined references (30 images, 10 videos, 10
audio files). LAS pricing and accounting remain the release gate for `v2v`
only. Polling keeps the transport persisted at submission so ModelArk and LAS
jobs cannot be mixed.
...
to the current ModelArk routes; they are not a valid quote for LAS V2V.
Customer pricing stays owned by
...
4. Verify all five product modes are described. T2V, I2V, Ref2V, and Extend are executable
   under the current environment matrix; V2V remains gated and must
   not be advertised as executable by the MCP catalog.
5. Request read-only live quotes for the four executable modes; each must be
   positive. Do not accept a quote or submit a generation.
