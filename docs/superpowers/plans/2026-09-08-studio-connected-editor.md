...
- Task3 en cours : interfaces partagées `4c986b84e` (refs/faits), `cf525e7df` (transfert compte/projet), `9d7bb92d4` (export du validateur URL existant). Les parcours et la revue finale du runtime médias restent distincts de ces interfaces.
- Task4 préparée, non implémentée : nouveau projet uniquement, commande UI/MCP commune, reçu idempotent, révision/CAS protégeant tous les writers, résolution privée. PostgreSQL17 socket-only et auth ES256 locale sont disponibles et vérifiés ; aucune excuse d'environnement distant ni recours Neon nécessaire.
