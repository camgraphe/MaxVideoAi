...
4. Redesign gallery rail thumbnails and restore contextual reuse from gallery/recent media without breaking preview, grouped outputs, original recreation/settings actions or compatibility graying. For image originals, connect both image creation and video-reference destinations shown in the prototype; video/audio originals use actual supported role destinations. Close and return focus correctly; support keyboard and touch without hover-only essential controls. Keep recent metadata validation and reference roles authoritative. Shared asset modal picker behavior stays intact here; its visual/workflow redesign is Task5.
...
2. Replace inherited dense presentation in those entry points with consistent headers, close/back controls, clear option grouping, concise labels and readable selected/disabled states. Reference asset picker should be a focused selection task: compact search/kind/source filters, visual thumbnails, explicit target role/capacity, clear selection and use action. Remove tool-promotion prose from the primary selection path and separate destructive management from reference selection. Keep existing management actions reachable where appropriate. Preserve existing immediate-vs-staged semantics unless a tested deliberate workflow improvement is justified; never silently change generation settings merely by opening a panel or discard staged values through decoration changes.
