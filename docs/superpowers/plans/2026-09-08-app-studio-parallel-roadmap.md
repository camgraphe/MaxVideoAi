...
## Intégrations effectuées le 8 septembre

- **Toolbox existante :** lot initial intégré jusqu’à `fd009de3e`, puis retrait des badges de catalogue dans `bbdf61de8`. Revue indépendante PASS et six contrôles ciblés de devis, acceptation avant débit et métadonnées. Contrôle visuel root sur le catalogue et le plan de travail d’agrandissement ; les contrôles multi-formats complets et le build isolé sont décrits dans `docs/plans/2026-09-08-toolbox-validation.md`. Le correctif de miniatures privées avait déjà été intégré dans `229ed4961` et n’a pas été dupliqué.
- **Extension Toolbox :** l’utilisateur a ensuite précisé directement dans cette tâche qu’il attend aussi les outils étudiés, avec Standard/Pro. Restore Video, Denoise, Fix Blur et Smooth Motion restent un incrément en cours, avec contrats, prix et qualification avant activation. Le lot initial ne constitue pas la livraison de ces nouveaux outils. Studio doit recevoir les identités et versions additionnelles avant de les proposer dans ses blocs.
- **Activité :** optimisation intégrée dans `b59545f9f`, documentation `b23f37f00`, puis corrections et bootstrap explicite jusqu’à `4e8237778`. La revue indépendante a fermé les deux réserves de fallback média et de cible PostgreSQL. Mesure comparable du propriétaire sur copie Neon : médiane du chemin serveur 15 841,4 → 1 554,0 ms ; ce n’est pas une mesure de rendu navigateur ni de Core Web Vitals. L’initialisation est une commande opérationnelle explicite suivie des migrations, jamais un travail DDL du GET normal. Voir `docs/engineering/activity-feed.md` et le rapport de performance.
- **Créateur :** devis courant et soumission périmée corrigés (`fedc6306f`, revue PASS). Projection commune et préparation de changement de modèle qualifiées (`50c8a20e7`, 62 contrôles ciblés, revue PASS). Le panneau de comparaison/récupération est en cours. La perte de référence après navigation a son plan distinct : `2026-09-08-workspace-active-draft-continuity.md` ; elle n’est pas considérée résolue par la seule sauvegarde au changement de modèle.

Ces intégrations restent dans la branche isolée de refonte. La validation globale combinée et les parcours authentifiés après raccordement restent à effectuer ; aucun déploiement ou paiement réel n’a été réalisé.
