...
- Extend only if required for exact saved-setup recovery: _lib/workspace-model-candidate.ts and its behavior test, keeping workflow derivation shared.
...
- Saved recovery must compare against the live configuration. Do not prepare with `current = saved` and present the resulting `comparable`/changes as relative to the live draft. If needed, add an optional validated `restoreSetup` seed to the pure candidate factory: normalize references under their saved engine/role schema, keep same-engine settings, then derive changes/removed live references/comparability against the actual current setup. Add a regression restoring different duration/audio/reference roles and assert both exact recovery and honest comparison.
...
- [ ] Make storage limits recoverable: allow explicit removal of a saved configuration from the configurations panel, without deleting any original media or the current live draft. Unavailable/retired model snapshots remain visible with a reason instead of disappearing. No automatic eviction or ownerless legacy import.
...
- [ ] Follow the reinforced global visual direction: no slogans or repeated explanatory cards. Use model identity, compact settings and meaningful changes; expose detailed reasons contextually. Preserve visible 44px actions and bounded scrolling. UI text for reason codes is localized; do not dump raw English-only guard messages as the French/Spanish panel's main explanation.
