...
- Search type: Web
- GSC freshness at capture: last update approximately 9.5 hours earlier
- Whole site: 5,638 clicks, 571,753 impressions, 1% CTR, average position 8.6

GSC warns that filtered totals and the query table may be partial. Rounded values and displayed precision below are
preserved as shown in the interface; they must not be treated as exact totals beyond that precision.
...
| --- | --- | --- |
| `/mcp` | Broad product/category intent for a Claude or Codex AI video workflow: clarify a brief, compare models and budgets, review price before generation, and understand separate confirmation. | Do not target broad “best prompts,” model-specific pricing, or technical endpoint/auth queries. |
...
- The static `llms.txt` must remain free of the four future MCP source URLs until the shared gate is enabled. At
  activation, add the hub, both client guides, and the technical guide together with their distinct intent labels;
