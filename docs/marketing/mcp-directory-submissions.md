...
Checked: **2026-09-13**. These conclusions authorize only local preparation in
...
| --- | --- | --- |
| ClawHub | Current first-party [publishing](https://github.com/openclaw/clawhub/blob/main/docs/publishing.md), [CLI](https://github.com/openclaw/clawhub/blob/main/docs/cli.md), and [skill-format](https://github.com/openclaw/clawhub/blob/main/docs/skill-format.md) guidance documents owner-scoped skill publishing, a non-uploading `--dry-run`, post-submission security scans, versioned install/update/uninstall state, and mandatory MIT-0 licensing for published skill files. | **Local candidate preparing; not submitted.** Official `clawhub@0.23.3` was executed ephemerally from npm. The exact `maxvideoai`/`MaxVideoAI`/`1.0.0` dry-run returned `would-publish`, two payload files, and fingerprint `d7cca882cf7561fcc8bc83d3f5c130d060beb132bfab98871ed219ccfbfa79dd`. The third reviewed local file, `.clawhubignore`, is a packaging control and is not uploaded. All files are non-executable text; the audit found no credential, installer, fixed price, or copied model roster. On 2026-09-13, the product owner explicitly accepted MIT-0 for the two payload files, the official CLI verified the owner-controlled publisher `@camgraphe`, and an authenticated dry-run reproduced the fingerprint with `latestVersion: null`. Upload, post-submission scan, and clean install/update/uninstall evidence remain pending. |
| n8n workflow library | [n8n's workflow library](https://n8n.io/workflows/) accepts creator-submitted templates, while the [MCP Client node](https://docs.n8n.io/integrations/builtin/core-nodes/n8n-nodes-langchain.mcpclient) provides the direct client surface. | **Local candidates preparing; not submitted.** Three credential-free JSON candidates pass repository approval/idempotency contracts. Clean import on a recorded n8n version and owner authorization are still required. |
