...
| --- | --- | --- |
| ClawHub | [ClawHub](https://docs.openclaw.ai/clawhub) hosts versioned skills and plugins, exposes scan state, and provides an authenticated publisher CLI with `--dry-run`. Published skills use MIT-0, so licensing and the exact file set require owner review. | **Local candidate preparing; not submitted.** The text-only candidate guides the direct OAuth-backed MCP connection and does not duplicate prices, models, billing rules, or credentials. Clean-host and CLI dry-run evidence remain pending. |
