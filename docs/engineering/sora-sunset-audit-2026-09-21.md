...
Validation sur un checkout isolé des changements de connexion présents en parallèle : **5 688 tests de la suite principale réussis**, build de production réussi, 539 tests Studio réussis, registre/projections, lint, exposition, i18n, SEO et baseline publique des prix conformes. Après une dernière correction de formulation des FAQ comparatives, 34 tests comparateurs réussis. Les **11 tests d’intégration Studio supplémentaires réussissent** sur le commit local `ac18ec90a`, dans un environnement sans fichier de secrets et avec PostgreSQL 17 jetable. Ils couvrent notamment les routes authentifiées, MCP, la persistance et la lecture privée dans le navigateur. Soit **5 699 tests réussis** pour les deux phases de validation, sans compter les relances ciblées.
...
Livrable : branche locale `codex/sora-sunset-verified-20260921`, checkout `/Users/adrienmillot/.codex/worktrees/sora-sunset-validation/MaxVideoAi V2`. L’aperçu local de l’archive française est disponible sur `http://localhost:3107/fr/modeles/sora-2`. Le snapshot Sora est commité séparément des changements de connexion présents dans le checkout initial.
