...
`827241cf4` ajoute la récupération du focus sur le formulaire stable au retry et un test DOM contrôlé RED→GREEN. La racine rejoue le navigateur connecté sur ce SHA. Elle vérifie en parallèle que la nouvelle garde ne piège pas les anciens projets locaux : le contre-test `editor-local-draft-exit.spec.ts` crée réellement un projet via l’UI sans serveur (API401 ou503 contrôlée), confirme son stockage local et ses deux blocs, puis trouve Projects désactivé indéfiniment (**RED4017,3s ;50314,2s**). Un premier essai avait échoué dans un import helper de test non exporté ; corrigé avant ces preuves causales.
...
`bdc14614f` conserve le marqueur `persistenceMode` dans le cache local et distingue la sortie locale sans writer. Les projets historiques sans marqueur ayant un état local existant peuvent également sortir localement après erreur de lecture ; cela ne les autorise pas à écrire comme projets legacy serveur. Quatre E2E racine sur ce SHA passent (**4/4,32,2s**) : UI locale marquée et ancien format exact sans marqueur, chacun avec401 et503 ; saisie Prompt, sortie, absence de mutation serveur à la sortie, reprise depuis Projects et texte intact. La re-revue corrective et le rejeu des six tests connectés sur ce même SHA sont en cours.
