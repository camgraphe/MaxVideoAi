...
`827241cf4` ajoute la récupération du focus sur le formulaire stable au retry et un test DOM contrôlé RED→GREEN. La racine rejoue le navigateur connecté sur ce SHA. Elle vérifie en parallèle que la nouvelle garde ne piège pas les anciens projets locaux : le contre-test `editor-local-draft-exit.spec.ts` crée réellement un projet via l’UI sans serveur, confirme son stockage local et ses deux blocs, puis trouve Projects désactivé indéfiniment (**RED7,3s et14,2s**). Rectification QA : ces deux premiers passages utilisaient le raccourci anonyme sans session hint, pas les réponses HTTP401/503 prévues. Ils prouvent le blocage local anonyme seulement. Un premier essai avait aussi échoué dans un import helper de test non exporté, corrigé avant cette reproduction.

La revue confirme la branche `!serverProject` sans résolution de sortie locale. Le cache Projects perd aussi le marqueur `persistenceMode`, donc la seule présence d’un snapshot ne constitue pas une preuve qu’un writer legacy serait sûr. Correction bornée demandée : sortie locale honnête conservant le brouillon et sans writer serveur lorsque le mode est indisponible, tout en maintenant les protections connected/unresolved et en préservant les projets historiques sans marqueur. Le test racine vérifie également édition du prompt, réouverture et absence de mutation serveur lors de cette sortie ; ces dernières assertions attendent le correctif.

Navigateur connecté complet sur `827241cf4` : **6/6 GREEN,63,73s**. Focus contenu dans le dialogue après retry, Échap et retour au déclencheur passent sans refocus du test ; nouvelle preuve également du thème applicatif sombre. Les protections de sortie et l’ensemble lecture privée/CAS/brouillons/rejeu restent verts. Revue du correctif focus **Approved**, aucun nouveau défaut concret. Le défaut legacy offline reste distinct et ouvert ; Task4 n’est pas encore clôturée. Les34 empreintes de la copie source et son HEAD sont de nouveau vérifiés identiques au manifeste.

`bdc14614f` conserve le marqueur `persistenceMode` dans le cache local et distingue la sortie locale sans writer. Les projets historiques sans marqueur ayant un état local existant peuvent également sortir localement après erreur de lecture ; cela ne les autorise pas à écrire comme projets legacy serveur. Quatre passages racine sur ce SHA passent (**4/4,32,2s**) : UI locale marquée et ancien format exact sans marqueur, saisie Prompt, sortie, reprise depuis Projects et texte intact. Même rectification QA : le raccourci anonyme rendait alors les mocks401/503 inatteints ; ces passages ne prouvent pas leurs réponses HTTP. Les six tests connectés authentifiés sur ce SHA passent séparément (**6/6,64,69s**), sans cette limite de harness.

Audit Git de remise indépendant : les paires `8a74ab753↔1830bf652`, `4c986b84e↔f55401ad7`, `746a0cf49↔9c9bc9ea4` ont des patch-id stables identiques ; leurs SKIP sur app sont confirmés. `cf525e7df` ne contient que le handoff compte/projet et son test ; `9d7bb92d4` exporte le validateur URL existant ; `00c0cfa3b` ajoute le fait mesuré `hasAudio` au probe/upload et deux tests. Le bundle partagé `b736bd6b8` comporte24 chemins à résoudre auprès des propriétaires actuels, pas à remplacer globalement. L’ordre partagé MCP et ses gates false sont confirmés, aucun nouveau défaut d’interface identifié. `3bb6188e0` reste un merge de composition propre à cette worktree, à ne pas rejouer.

## Préservation de la liste locale et preuve HTTP corrigée

`eb0c7755c` garde les projets explicitement local-only absents du serveur et traite leur404 comme une absence attendue, pas une erreur propriétaire. La revue détecte deux frontières supplémentaires : troncature à20 alors que le serveur retourne40, et réconciliation depuis le tableau capturé au montage malgré une création pendant le GET. `112ee3b28` retire la troncature (RED pur20≠41 puis GREEN) ; `18bf21cad` relit le cache courant à la réception du GET. Les collisions d’ID restent gagnées par le serveur ; aucun projet ambigu n’est promu local-only.

Le premier test navigateur40+local ne voyait qu’une carte et le test GET retenu ne démarrait jamais sa requête. Cela a révélé le raccourci `hasAuthFetchSessionHint=false`. Le harness ajoute maintenant un simple indice de session au seul contexte possédé, **sans authentification réelle**, pour atteindre les routes explicitement mockées ; des assertions exigent les réponses réseau401/503 et404. Les anciennes preuves locales sont rectifiées ci-dessus, pas conservées comme qualification HTTP.

Sur `18bf21cad`, la suite locale renforcée passe **6/6,43,0s** : marqué/historique ×401/503 effectivement reçus, texte modifié/retrouvé et aucune mutation serveur à la sortie ; retour d’un serveur40+local=41 cartes ; projet local404 encore ouvrable ; GET initial et POST création retenus, création locale postérieure au départ du GET conservée dans les41 cartes avant navigation et après reprise. Ceci qualifie le client avec API contrôlée, pas la persistance serveur. La re-revue finale de cet additif et le rejeu connecté exact-head sont en cours.

Le rejeu connecté exact `18bf21cad` termine à son tour **6/6 GREEN,63,36s**, avec authentification signée réelle, PostgreSQL jetable vérifié, lecture privée et tous les scénarios de conflit/reprise/création. Suite générale au HEAD documentaire `16aa22d75` (même produit) : **483/483, aucun skip,23,01s**. Le premier essai en environnement `env -i` avait13 erreurs de démarrage PostgreSQL et470 tests verts : le journal isolé indique `postmaster became multithreaded during startup`, avec recommandation de fixer `LC_ALL`. Ajouter `LC_ALL=C` à l’allowlist, sans aucune DB héritée, résout ces erreurs et le rejeu intégral. Log conservé `/private/tmp/studio-editor-suite.pUGxrb` ; diagnostic local sans données utilisateur `/private/tmp/stpg-probe.IYBCh0/start.log`. Aucun correctif produit n’a été nécessaire pour cet environnement de test.
