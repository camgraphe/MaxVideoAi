...
Après `704c9d974`, `909e37f60`, `4a72b3d04` et `139bf2e01`, le parcours complet est **GREEN1/1 (36,31s)** sur le snapshot Git `db6560345`. Il vérifie tous les critères ci-dessus, puis l’édition du premier onglet ACK en SQL revision1, le vrai conflit du second, son brouillon local intact, le clic Recharger réellement actionnable, une réouverture en troisième contexte sans localStorage et le refus404 en contexte neuf ownerB sans aucun accès aux médias. Le snapshot et les URL sans query figurent dans le diagnostic ; aucune signature/cookie/session n’est enregistrée dans les preuves. La capture `output/playwright/studio-connected/persisted-private-viewer.png` a été inspectée : posters vidéo non traités comme images, clip et titre édité présents. Sa petite taille naturelle est réservée au polish des nouveaux montages, pas confondue avec un défaut de décodage.

La preuve audio privée utilise le compteur AAC du Chromium installé, dont la présence est explicitement vérifiée ; elle ne remplace pas le RMS de la fixture native séparée ni une mesure de haut-parleur. L’expiration est une injection d’horloge uniquement dans le validateur privé pour un GET ; la nouvelle autorisation/signature et son téléchargement sont réels dans l’environnement jetable. Le test ne prétend pas attendre cinq minutes d’expiration naturelle. Le serveur/correctifs ont une re-revue indépendante Approved ; le client a sa revue distincte en cours. L’entrée de création par l’interface Projects est encore à qualifier.

## Contrôles restant ouverts

État actuel après correctifs `bccd76266`, `77d05b569`, `fa48e19b4`, `adfe2f29c` : navigateur renforcé **5/5 (61,36s)** sur `adfe2f29c`. Les critères client ci-dessous ne sont plus rouges : deux réouvertures du brouillon en conflit conservent sa base0 face au serveur1, aucune écriture implicite ; panne503 visible et brouillon relu puis sortie seulement après vrai200 ; A→B et SIGNED_OUT purgent durablement l’ancien contenu sans nouvelle lecture privée ; retrait du bin relu en SQL et source B exacte réellement décodée après réouverture. La validation UI montre son motif et le trim de fin reste valide au passage24→25fps. La re-revue corrective client/UI/QA reste ouverte ; le détail causal, y compris le RED intermédiaire de baseRevision, est dans `studio-task4-integration-review.md`.

La création mobile/clavier est qualifiée séparément sur `fa48e19b4` : sous-test réel5,69s, route et reçu SQL, ordre/trims, perte de la première réponse après commit, gel de toute l’intention et rejeu identique sans deuxième projet. La capture a été inspectée. Le listing bibliothèque est une fixture explicitement hors propriétaire Studio ; les commandes, Auth, résolution des originaux et écritures restent réels. Ce résultat ne qualifie pas les nouveaux critères client encore RED, ni tous les états de validation/FPS du builder.

La revue client suivante a identifié quatre chemins non couverts par le passage nominal : référence privée conservée en timeline après retrait du bin/réouverture, annonce de sauvegarde et sortie malgré échec, récupération d’un brouillon connecté après remount, purge de l’ancien compte lors d’un changement Auth sans démontage. Le premier est désormais **RED causal dans le navigateur** sur `7caa7cea2` (44,84s) : aucun lecteur ne monte pour le clip B conservé. Les correctifs client sont en cours ; le passage nominal antérieur n’est pas présenté comme preuve de ces critères. Le builder UI `89af69248` fait l’objet d’une revue séparée et d’un sous-test mobile/clavier avec vraie route de commande et reçu SQL ; aucune qualification finale de ce parcours à ce stade.
