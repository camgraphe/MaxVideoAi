...
Le nouveau test `connected-studio-montage-http-integration.test.ts` est au stade **RED attendu** contre le snapshot committé `a940929ef` : découverte authentifiée réelle réussie, mais `create_studio_montage` encore absent malgré l'override local. Il prépare les assertions SQL ordre/trims, propriétaire, reçu unique, sauvegarde CAS, refus des anciens writers, rejeu préservant une édition et refus des faits non mesurés. Ces assertions post-création ne sont pas encore passées : elles attendent le commit produit Task4.
