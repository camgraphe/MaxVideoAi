...
Use Streamable HTTP. Do not append a token, password, query parameter or API key. Follow the guide for your client: it opens the MaxVideoAI sign-in and OAuth consent screen in your browser.
...
Connecting and planning add no MaxVideoAI subscription. Paid generations use your MaxVideoAI credits; your assistant provider may have its own plan requirements. Remove the integration from the client and revoke its authorization in MaxVideoAI to disconnect.
