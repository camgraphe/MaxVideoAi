---
title: "MaxVideoAI app for ChatGPT, Claude connector, and MCP guide"
...
Do not add a token, query string, password, or API key. The client opens MaxVideoAI in the browser for OAuth 2.1 sign-in and consent with `openid,email,profile`. MCP is the technical connection. In product language, MaxVideoAI is an app for ChatGPT, a connector for Claude, and a plugin for Codex.
...
2. Sign in or create your MaxVideoAI account in the browser, then confirm your email if needed.
3. Review and approve the connection; MaxVideoAI is free to connect and adds no separate subscription.
4. Return to the assistant and ask it to check your MaxVideoAI account.

OAuth never gives the assistant your password, payment details, or database access. The same user wallet and private MaxVideoAI Library are used on the website and through the connection.

Hosted compatibility is recorded per client rather than inferred from the protocol. Claude Desktop `1.37937.1` has completed controlled OAuth, account, catalogue, budgeting, exact quote, media, recovery, upload, and top-up checks. Codex CLI `0.150.0-alpha.8` has completed production installation, OAuth, account, catalogue, recommendation, budgeting, exact quote, paid generation, recovery, and inline-player contract checks. Direct ChatGPT desktop and Claude Code production checks have not yet been recorded.
