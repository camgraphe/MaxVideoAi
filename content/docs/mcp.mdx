...
An accepted job is not yet a completed video. Poll `get_generation_status` for a known job or call `list_recent_generations` to recover it. Do not submit the same paid generation again merely because the first call timed out.
