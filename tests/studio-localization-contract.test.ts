...
test('Studio localization dictionaries expose required copy in every locale', () => {
  const enginePickerKeys = [
    'enginePickerSearch',
    'enginePickerFamilies',
    'enginePickerModels',
    'enginePickerNoResults',
    'enginePickerUnavailable',
    'enginePickerCurrentInputsIncompatible',
    'enginePickerPaused',
    'enginePickerWaitlist',
    'enginePickerFamilyCount',
    'enginePickerModelCount',
    'enginePickerOpen',
  ];
  ...
  locales.forEach((locale) => {
    ...
    enginePickerKeys.forEach((key) => assert.ok(key in studio.canvas.nodes, `${locale} studio.canvas.nodes should include ${key}`));
...
test('Studio guide controls and steps stay aligned across locales', () => {
  ...
  locales.forEach((locale) => {
    const dictionary = JSON.parse(fs.readFileSync(path.join(root, `frontend/messages/${locale}.json`), 'utf8'));
    const guide = dictionary.workspace?.studio?.canvas?.guide;

    assert.deepEqual(Object.keys(guide?.controls ?? {}).sort(), expectedControlKeys.sort());
    assert.deepEqual(Object.keys(guide?.steps ?? {}).sort(), expectedStepKeys.sort());
    Object.values(guide?.steps ?? {}).forEach((step) => {
      assert.equal(typeof (step as { title?: unknown }).title, 'string');
      assert.equal(typeof (step as { body?: unknown }).body, 'string');
      assert.equal(typeof (step as { ariaLabel?: unknown }).ariaLabel, 'string');
    });
  });
});

test('Studio control labels and option text are typed and localized in French and Spanish', () => {
  ...
  const fr = resolveStudioCopy(frDictionary).canvas.controls;
  const es = resolveStudioCopy(esDictionary).canvas.controls;

  assert.equal(fr.labels.durationSec, 'Durée');
  assert.equal(fr.labels.outputCount, 'Nombre');
  assert.equal(fr.labels.angleSafeMode, 'Mode sûr');
  assert.equal(fr.options.on, 'Activé');
  assert.equal(fr.options.off, 'Désactivé');
  assert.equal(fr.options.included, 'Inclus');
  assert.equal(fr.options.characterExploratory, 'Exploratoire');
  assert.equal(fr.options.moodDreamy, 'Onirique');
  assert.equal(es.labels.durationSec, 'Duración');
  assert.equal(es.labels.outputCount, 'Cantidad');
  assert.equal(es.options.on, 'Activado');
  assert.equal(es.options.off, 'Desactivado');
  assert.equal(es.options.characterExploratory, 'Exploratorio');
  assert.equal(es.options.moodDreamy, 'Soñador');
});

async function renderSpecializedInspector(dictionary: Dictionary): Promise<string> {
  const { ShotNodeToolSections } = await import(
    '../frontend/app/(core)/(workspace)/app/studio/workspace/_components/ShotNodeToolSections'
  );
  const copy = resolveStudioCopy(dictionary).canvas.controls;
  const characterPreset = getWorkspaceBlockPreset('character-builder');
  assert.ok(characterPreset?.defaultShot?.toolSettings?.characterBuilder);
  const characterSettings = characterPreset.defaultShot.toolSettings.characterBuilder;
  const characterShot = {
    ...characterPreset.defaultShot,
    toolSettings: {
      ...characterPreset.defaultShot.toolSettings,
      characterBuilder: {
        ...characterSettings,
        traits: {
          ...characterSettings.traits,
          hairEnabled: true,
          outfitEnabled: true,
        },
      },
  ...
  const cases = [
    { shot: characterShot, section: 'character-builder' },
    { shot: getWorkspaceBlockPreset('angle')?.defaultShot, section: 'angle-controls' },
    { shot: getWorkspaceBlockPreset('upscale-image')?.defaultShot, section: 'upscale' },
    { shot: getWorkspaceBlockPreset('storyboard')?.defaultShot, section: 'storyboard' },
    { shot: getWorkspaceBlockPreset('audio-sound-design-voice')?.defaultShot, section: 'audio-pack' },
  ] as const;

  return cases.map(({ shot, section }) => {
    assert.ok(shot);
    const capability = getWorkspaceModelCapability(shot.modelId);
    const policy = resolveWorkspaceBlockPolicy({ settings: shot, capability, connectedInputs: [] });
    return renderToStaticMarkup(React.createElement(ShotNodeToolSections, {
      copy,
      shot,
      sections: [section],
      controlFields: policy.controlFields,
      onPatchShot: () => undefined,
    }));
  }).join('');
}

test('specialized inspectors render localized French and Spanish sections, fields, and options', async () => {
  const frDictionary = JSON.parse(fs.readFileSync(path.join(root, 'frontend/messages/fr.json'), 'utf8')) as Dictionary;
  const esDictionary = JSON.parse(fs.readFileSync(path.join(root, 'frontend/messages/es.json'), 'utf8')) as Dictionary;
  const frMarkup = await renderSpecializedInspector(frDictionary);
  const esMarkup = await renderSpecializedInspector(esDictionary);
  ...
  for (const expected of [
    ...
    'Noir',
    'Brun foncé',
  ]) assert.match(frMarkup, new RegExp(expected));
  for (const expected of [
    ...
    'Negro',
    'Castaño oscuro',
  ]) assert.match(esMarkup, new RegExp(expected));
  ...
  for (const forbidden of [
    ...
    assert.doesNotMatch(frMarkup, new RegExp(forbidden));
    assert.doesNotMatch(esMarkup, new RegExp(forbidden));
  }
});

test('Studio policy presentation localizes mutual-exclusion and missing-input reasons', () => {
  const frDictionary = JSON.parse(fs.readFileSync(path.join(root, 'frontend/messages/fr.json'), 'utf8')) as Dictionary;
  const frCopy = resolveStudioCopy(frDictionary);
  const settings = getWorkspaceBlockPreset('generate-video')?.defaultShot;
  assert.ok(settings);
  const capability = getWorkspaceModelCapability(settings.modelId);
  assert.ok(capability);
  const edgeLabel = (kind: string) => localizeStudioEdgeKindLabel(kind, frCopy.canvas.nodes);
  const exclusivePolicy = resolveWorkspaceBlockPolicy({
    settings,
    capability,
    connectedInputs: ['prompt', 'start_image'],
    policyCopy: frCopy.canvas.controls.policy,
    edgeLabel,
  });
  const missingPolicy = resolveWorkspaceBlockPolicy({
    settings,
    capability,
    connectedInputs: [],
    policyCopy: frCopy.canvas.controls.policy,
  ...
  assert.equal(
    exclusivePolicy.inputConnectors.find((connector) => connector.kind === 'reference')?.disabledReason,
    'Le mode image de départ désactive les images de référence.'
  );
  assert.equal(missingPolicy.disabledReason, 'Connectez l’entrée Prompt requise.');
});

test('Studio control renderer consumes localized copy instead of visible English literals', () => {
  const source = fs.readFileSync(path.join(
    root,
    'frontend/app/(core)/(workspace)/app/studio/workspace/_components/nodes/WorkspaceControlField.tsx'
  ), 'utf8');

  assert.match(source, /copy:\s*StudioControlCopy/);
  assert.match(source, /copy\.labels\[id\]/);
  assert.match(source, /copy\.options\.on/);
  assert.match(source, /copy\.options\.included/);
  assert.doesNotMatch(source, />On</);
  assert.doesNotMatch(source, />Off</);
  assert.doesNotMatch(source, />Included</);
});
