...
test('video generation request preserves render controls and excludes media unsupported by the resolved mode', () => {
  const settings = {
    ...shot('generate-video'),
    durationSec: 7,
...
test('character builder request includes traits and reference image roles', () => {
  ...
  const request = buildWorkspaceCharacterBuilderRequest({
    ...
    identityImageUrls: [
      ...
      'https://example.com/identity.png',
      'https://example.com/identity-two.png',
      'https://example.com/identity-three.png',
    ],
    styleImageUrls: [
      'https://example.com/style.png',
      'https://example.com/style.png',
      'https://example.com/style-two.png',
      'https://example.com/style-three.png',
    ],
  ...
  assert.deepEqual(request.referenceImages.map((image) => [image.url, image.role]), [
    ...
    ['https://example.com/identity-two.png', 'identity'],
    ['https://example.com/style.png', 'style'],
    ['https://example.com/style-two.png', 'style'],
  ]);
