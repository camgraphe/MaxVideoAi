...
test('jobs page delegates copy, library helpers, and compact rail UI', () => {
  ...
  assert.match(pageSource, /from '\.\/_hooks\/useJobsCopy'/);
...
test('jobs page does not regain extracted local ownership', () => {
  ...
  assert.ok(lineCount <= 470, `jobs page should stay below 470 lines after shell and copy extraction, got ${lineCount}`);
});
