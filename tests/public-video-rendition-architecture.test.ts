...
test('critical homepage coverage is an offline prebuild guard with a pure injected boundary', async () => {
  ...
  assert.equal(
    ...
    'pnpm --dir .. model:registry:check && node ../scripts/check-production-git.mjs && pnpm --dir .. seo:machine:check && pnpm media:public-renditions:check && pnpm media:home-posters:check',
  );
  assert.equal(
