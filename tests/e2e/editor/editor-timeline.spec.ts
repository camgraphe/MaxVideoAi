...
test('canvas video output drop inside a clip resolves to a timeline edit point by default', async ({ page }) => {
  ...
  await expect.poll(async () => {
    const states = await timelineClipStates(page);
    const inserted = states.find((clip) => clip.id.startsWith('timeline-output-01-') && clip.id !== 'timeline-output-01' && clip.track === 'video');
    const shifted = states.find((clip) => clip.id === 'timeline-output-02');
    if (!inserted || !shifted) return false;
    return Number((inserted.start + inserted.duration).toFixed(6)) === Number(shifted.start.toFixed(6));
  }).toBe(true);
  await expect.poll(async () => hasTimelineOverlap(page, 'video')).toBe(false);
