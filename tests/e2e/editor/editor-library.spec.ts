...
test('asset library modal scrolls through a full app media library', async ({ page }) => {
  ...
  const firstAsset = dialog.getByRole('button', { name: 'Select asset-00.png' });
  const lastAsset = dialog.getByRole('button', { name: 'Select asset-53.png' });
  await expect(firstAsset).toBeVisible();

  const gridMetrics = await library.evaluate((element) => {
    const firstAssetButton = element.querySelector('button[aria-label^="Select "]');
...
test('Project media import can add multiple library assets at once', async ({ page }) => {
  ...
  await dialog.getByRole('button', { name: 'Select batch-video-1.mp4' }).click({ modifiers: ['Meta'] });
...
test('Project media sidebar filters imported media by kind', async ({ page }) => {
  ...
  await dialog.getByRole('button', { name: 'Select filter-video-1.mp4' }).click({ modifiers: ['Meta'] });
  await dialog.getByRole('button', { name: 'Select filter-audio-2.mp3' }).click({ modifiers: ['Meta'] });
