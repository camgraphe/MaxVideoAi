...
const productAdRegressionFixture = createStarterWorkspaceTemplate('product-ad');
const PRODUCT_AD_REGRESSION_WORKSPACE_STATE = {
  nodes: productAdRegressionFixture.nodes,
  edges: productAdRegressionFixture.edges,
  timelineItems: productAdRegressionFixture.timelineItems,
  activeTemplateId: productAdRegressionFixture.id,
...
async function mockEditorStudioPersistenceApi(page: Page): Promise<void> {
  await page.route('**/api/studio/timeline-exports/estimate', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ok: true,
        estimateToken: 'e2e-signed-export-estimate-token',
        quota: {
          billingKind: 'free',
