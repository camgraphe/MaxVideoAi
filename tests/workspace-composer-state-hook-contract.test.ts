...
test('workspace composer mode and settings state is owned by a route-local hook', () => {
  ...
  const projectionSource = fs.readFileSync('frontend/app/(core)/(workspace)/app/_lib/workspace-workflow-projection.ts', 'utf8');
  ...
  assert.match(hookSource, /resolveWorkspaceComposerFacts/);
  assert.match(projectionSource, /getWorkspaceMultiPromptState/);
  ...
  assert.match(engineModeHookSource, /resolveWorkspaceWorkflow/);
  assert.match(projectionSource, /const implicitMode = /);
  assert.match(projectionSource, /const audioWorkflowUnsupported =/);
