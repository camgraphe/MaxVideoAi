...
import type {
  PlaylistSummary,
  PlaylistItemRecord,
} from "../frontend/components/admin/playlists/playlist-types";
...
test("failed destination load retains selection and dirty order; pending requests prevent a second switch", async () => {
  ...
  const button = (name: string) =>
    [...dom.window.document.querySelectorAll("button")].find((el) =>
      el.textContent?.includes(name),
    )!;
  try {
    const { PlaylistsManager } = await import(
      "../frontend/components/admin/PlaylistsManager"
    );
    await act(async () =>
      root.render(
    ...
    const beforeDrag = [
      ...dom.window.document.querySelectorAll("article[draggable]"),
    ...
    assert.notDeepEqual(
      [...dom.window.document.querySelectorAll("article[draggable]")].map(
        (row) => row.textContent,
      ),
      beforeDrag,
    );
    ...
    await act(async () =>
      (
        dom.window.document.querySelector(
          '[aria-label="Move item 2 up"]',
        ) as HTMLButtonElement
      ).click(),
    );
    assert.equal(
      button("Seed from current order").disabled,
      true,
      "maintenance must not discard an unsaved order",
    );
    ...
    assert.equal(
      requests.length,
      1,
      "no parallel request while a destination is loading",
    );
    await act(async () =>
    ...
    assert.equal(
      requests[1].url,
      "/api/admin/playlists/a/items",
      "old items must never be saved to destination b",
    );
    ...
    await act(async () =>
      requests[3].resolve(new Response("{}", { status: 503 })),
    );
    assert.doesNotMatch(dom.window.document.body.textContent!, /Unsaved order/);
    assert.equal(
      button("Cancel").disabled,
      true,
      "confirmed save must not offer rollback to an obsolete local order",
    );

    await act(async () => button("Destination b").click());
    await act(async () =>
      requests[4].resolve(
        Response.json({ ok: true, playlist: playlists[1], items: [] }),
      ),
    );
...
test("curation destination switches guard staged changes and pending preview", async () => {
  ...
  try {
    ...
    assert.equal(
      button("Destination b").disabled,
      true,
      "pending preview locks destination",
    );
    await act(async () =>
      requests[1].resolve(
        Response.json(
    ...
    assert.equal(
      button("Cancel").disabled,
      false,
      "failed preview preserves draft",
    );
    dom.window.confirm = () => true;
    await act(async () => button("Destination b").click());
    await act(async () =>
    ...
    assert.equal(
      button("Cancel").disabled,
      true,
      "destination starts with a clean draft",
    );
  } finally {
    await act(async () => root.unmount());
    dom.window.close();
