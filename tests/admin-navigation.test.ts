...
test('five operator work areas, secondary settings and no obsolete site map', () => {
  assert.deepEqual(ADMIN_NAV_GROUPS.filter(group => !group.secondary).map(group => group.label), ['Overview', 'Users', 'Transactions', 'Generations', 'Content']);
  const paths = ADMIN_NAV_GROUPS.flatMap(group => group.items.map(item => item.href));
  for (const retired of ['/admin/theme', '/admin/pricing', '/admin/membership', '/admin/seo/cockpit']) assert.ok(!paths.includes(retired));
  assert.equal(paths.every(path => path.startsWith('/admin')), true);
