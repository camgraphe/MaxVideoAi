...
test('AI Strategist playground admin route and page stay internal-only', () => {
  ...
  const brandingSource = readFileSync(join(root, 'frontend/lib/ai-strategist/branding.ts'), 'utf8');
  ...
  assert.match(chatSource, /AI_STRATEGIST_WIDGET_TITLE/);
  assert.match(chatSource, /AI_STRATEGIST_LAUNCHER_LABEL/);
  assert.match(chatSource, /AI_STRATEGIST_ASK_CTA/);
