...
test('historical recovery and automatic completion feed exact means once, including new models', { timeout: 90_000 }, async (t) => {
  ...
  await db.pool.query(`
    ...
    CREATE TABLE provider_attempts (job_id integer, provider text, status text, finished_at timestamptz);
    ...
    INSERT INTO provider_attempts SELECT id, provider, 'completed', created_at + INTERVAL '4 minutes' FROM app_jobs WHERE job_id = 'old-direct';
  `);
