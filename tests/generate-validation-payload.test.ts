...
test('generate route evaluates validation rejection before billing, job reservation, and provider submission', () => {
  const validationRejectionIndex = serviceSource.indexOf(
    'if (!validationPayloadResult.ok)'
  );
  const billingPreflightIndex = serviceSource.indexOf(
    'const billingPreflight = trustedIncludedTrialBilling'
  );
  const preparedExecutionIndex = serviceSource.indexOf(
    'return executePreparedVideoGeneration'
  );
  const preparedServiceSource = readFileSync(
    join(root, 'frontend/src/server/video-generation/execute-prepared-video-generation.ts'),
    'utf8'
  );
  const initialJobIndex = preparedServiceSource.indexOf(
    'const initialJobState = await createAtomicInitialVideoJob'
  );
  const bytePlusSubmissionIndex = preparedServiceSource.indexOf(
    'const submission = await adapters.submitBytePlusGenerateTask'
  );
  const providerSubmissionIndex = preparedServiceSource.indexOf(
  ...
  assert.ok(billingPreflightIndex < preparedExecutionIndex);
  assert.ok(initialJobIndex >= 0);
