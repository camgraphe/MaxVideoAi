...
test('audio pricing enforces music-only minimum charge for short renders', () => {
  ...
  assert.equal(pricing.totalCents, 79);
  assert.equal(pricing.base.amountCents, 24);
  assert.equal(pricing.margin.amountCents, 55);
...
test('audio pricing adds clone surcharge for voice-only renders', () => {
  const pricing = buildAudioPricingSnapshot({
    pack: 'voice_only',
    durationSec: 20,
    voiceMode: 'clone',
  ...
  assert.equal(pricing.totalCents, 360);
  assert.equal(pricing.base.amountCents, 200);
