...
test("curation stages drag order, previews before save, locks requests and preserves confirmed state", async () => {
  ...
  for (const [key, value] of Object.entries({
    ...
    fetch: (_url: string, init?: RequestInit) =>
      new Promise<Response>((resolve) => requests.push({ init, resolve })),
  })) {
  ...
  try {
    const { PlacementEditor } = await import(
      "../frontend/components/admin/playlists/PlacementEditor"
    );
    await act(async () =>
      root.render(React.createElement(PlacementEditor, { playlistId: "p" })),
    );
    await act(async () =>
      requests[0].resolve(
        Response.json({
    ...
    const order = () =>
      [...dom.window.document.querySelectorAll("[data-curation-item]")].map(
        (el) => el.getAttribute("data-curation-item"),
      );
    assert.deepEqual(order(), ["b", "a"]);
    assert.equal(
      button("Save changes"),
      undefined,
      "saving requires an explicit preview",
    );
    ...
    await act(async () =>
      requests[2].resolve(
        Response.json(
          { ok: false, error: "This destination changed. Reload it." },
          { status: 409 },
        ),
      ),
    );
    ...
    assert.equal(
      button("Save changes"),
      undefined,
      "failed save invalidates preview",
    );
    await act(async () => button("Preview changes").click());
    await act(async () =>
      requests[3].resolve(
