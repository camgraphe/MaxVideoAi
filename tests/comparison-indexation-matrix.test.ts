...
test('the generated inventory covers all 918 localized published comparison URLs', () => {
  ...
  assert.equal(artifacts.document.summary.publishedSlugs, 306);
  assert.equal(artifacts.document.summary.totalUrls, 918);
  ...
  assert.equal(artifacts.document.rows.length, 918);
  assert.equal(
...
test('the generator CLI writes the JSON and Markdown audit artifacts', () => {
  ...
  assert.match(result.stdout, /"totalUrls": 918/);
});
