...
test('timeline ripple resize reverts when linked audio synchronization hits a secondary-track blocker', () => {
  const items = [
    videoClip({ id: 'video-a', startSec: 0, durationSec: 5, sourceDurationSec: 8 }),
    videoClip({ id: 'video-b', startSec: 5, durationSec: 2, linkedGroupId: 'group-b', linkedGroupKind: 'video-audio' }),
    audioClip({ id: 'audio-b', startSec: 5, durationSec: 2, linkedGroupId: 'group-b', linkedGroupKind: 'video-audio' }),
    audioClip({ id: 'audio-blocker', startSec: 4, durationSec: 1 }),
  ];
  ...
  assert.equal(timelineTrackHasOverlap(items), false);
  assert.equal(updated, items);
  assert.equal(timelineTrackHasOverlap(updated), false);
});

test('timeline roll resize reverts when linked audio synchronization hits a secondary-track blocker', () => {
  const items = [
    videoClip({ id: 'video-a', startSec: 0, durationSec: 5, sourceDurationSec: 8 }),
    videoClip({ id: 'video-b', startSec: 5, durationSec: 5, sourceStartSec: 2, sourceDurationSec: 10, linkedGroupId: 'group-b', linkedGroupKind: 'video-audio' }),
    audioClip({ id: 'audio-b', startSec: 5, durationSec: 5, sourceStartSec: 2, sourceDurationSec: 10, linkedGroupId: 'group-b', linkedGroupKind: 'video-audio' }),
    audioClip({ id: 'audio-blocker', startSec: 3, durationSec: 2 }),
  ];
