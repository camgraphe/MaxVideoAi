...
test('MaxVideoAI editor owns graph, node, generation, and capability contracts', () => {
  ...
  assert.match(canvasNodeStyleSource, /\.graphHandle[\s\S]*width:\s*7px/, 'canvas connector handles should stay visually compact next to dense node controls');
  assert.match(canvasNodeStyleSource, /\.graphHandle[\s\S]*height:\s*7px/, 'canvas connector handles should stay visually compact next to dense node controls');
  assert.match(canvasNodeStyleSource, /\.graphHandle[\s\S]*border-radius:\s*999px/, 'canvas connector handles should render as small full-color dots instead of oversized squares');
  ...
  assert.match(shotInputDockSource, /left:\s*-10/, 'generate block input handles should be centered on the node edge like output handles');
  assert.match(shotInputDockSource, /right:\s*-10/, 'generate block output handles should be centered on the node edge like input handles');
