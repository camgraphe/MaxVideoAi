...
test('MaxVideoAI editor workspace is an isolated authenticated app route', () => {
  ...
  assert.match(canvasSource, /CanvasFloatingToolbar/, 'canvas surface should compose the floating block and template toolbar');
...
test('MaxVideoAI editor owns graph, node, generation, and capability contracts', () => {
  ...
  assert.match(canvasFloatingToolbarSource, /template\.thumbnailUrl/, 'canvas toolbar should render image-backed template cards');
  ...
  assert.match(canvasFloatingToolbarSource, /MEDIA_BLOCKS/, 'canvas toolbar should expose reusable media block templates');
  ...
  assert.doesNotMatch(canvasFloatingToolbarSource, /onAddNode/, 'canvas toolbar block templates should be drag-and-drop only, not click-to-add');
  ...
  assert.match(canvasFloatingToolbarSource, /onDuplicateUserTemplate/, 'canvas toolbar should let users duplicate a saved canvas template');
  assert.match(canvasFloatingToolbarSource, /onDeleteUserTemplate/, 'canvas toolbar should let users delete a saved canvas template');
  ...
  assert.match(workspaceEditorLayoutSource, /onSaveCanvasTemplate:\s*canvas\.handleSaveCanvasTemplate/, 'workspace should wire saving the current graph as a personal canvas template');
  assert.match(workspaceEditorLayoutSource, /onApplyUserTemplate:\s*canvas\.handleApplyUserCanvasTemplate/, 'workspace should wire applying personal templates without touching timeline state');
  ...
  assert.match(canvasToolbarStyleSource, /\.toolbarPopover[\s\S]*overflow:\s*auto/, 'canvas template menus should scroll inside the floating toolbar when vertical space is tight');
