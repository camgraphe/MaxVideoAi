...
const canvasLibraryStylesPath = join(workspaceDir, '_styles/canvas-library.module.css');
...
test('MaxVideoAI editor workspace is an isolated authenticated app route', () => {
  ...
  assert.ok(existsSync(canvasLibraryStylesPath), 'canvas library sidebar styles should live in a focused route-local CSS module');
  ...
  const canvasLibraryStyleSource = source(canvasLibraryStylesPath);
  ...
  assert.match(workspaceEditorLayoutSource, /NodeLibrarySidebar/, 'editor layout should compose the block template library');
  ...
  assert.match(librarySource, /from '\.\.\/_styles\/canvas-library\.module\.css'/, 'canvas sidebar should import focused canvas library CSS');
  assert.match(shellStyleSource, /\.editorShell/, 'shell CSS should own the editor shell layout');
  assert.match(canvasLibraryStyleSource, /\.blockTemplateList/, 'canvas library CSS should own block template list styling');
...
test('MaxVideoAI editor owns graph, node, generation, and capability contracts', () => {
  ...
  assert.match(canvasLibraryStyleSource, /\.blockTemplateList/, 'sidebar block template list should be styled with focused canvas library CSS');
  assert.match(canvasLibraryStyleSource, /\.blockTemplateList[\s\S]*flex:\s*0 0 auto/, 'sidebar block templates should keep a bounded height when Viewer mode reduces available body space');
  assert.match(canvasLibraryStyleSource, /\.blockTemplateList[\s\S]*user-select:\s*none/, 'block template labels should not be selectable during block drags');
  assert.match(canvasLibraryStyleSource, /\.blockTemplateCard/, 'sidebar block template cards should be styled with focused canvas library CSS');
  ...
  assert.match(workspaceEditorLayoutSource, /onSaveCanvasTemplate=\{canvas\.handleSaveCanvasTemplate\}/, 'workspace should wire saving the current graph as a personal canvas template');
  assert.match(workspaceEditorLayoutSource, /onApplyUserTemplate=\{canvas\.handleApplyUserCanvasTemplate\}/, 'workspace should wire applying personal templates without touching timeline state');
  ...
  assert.match(canvasLibraryStyleSource, /\.templateSection[\s\S]*flex:\s*1 1 auto[\s\S]*overflow:\s*hidden/, 'starter templates should own the flexible sidebar height instead of overlapping block templates');
  assert.match(canvasLibraryStyleSource, /\.templateList[\s\S]*overflow:\s*auto/, 'starter templates should scroll inside the left sidebar when vertical space is tight');
  ...
  assert.match(librarySource, /from '\.\.\/_styles\/canvas-library\.module\.css'/, 'canvas sidebar should import the focused canvas library CSS module');
  ...
  assert.doesNotMatch(canvasStyleSource, /\.blockTemplateList/, 'canvas surface CSS should no longer own canvas library list styles');
  ...
  assert.ok(lineCount(canvasLibraryStyleSource) <= 420, 'canvas library CSS module should stay under the focused sidebar threshold');
