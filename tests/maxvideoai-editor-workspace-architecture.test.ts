...
test('Studio projects use real canvas captures for starters and personal project thumbnails', () => {
  assert.ok(existsSync(studioProjectPreviewStoragePath), 'Studio should own project canvas preview storage helpers');
  const projectsSource = readFileSync(projectsClientPath, 'utf8');
  const canvasSource = readFileSync(canvasPath, 'utf8');

  assert.match(projectsSource, /STUDIO_PROJECT_STARTER_CANVAS_IMAGES/, 'starter cards should use captured Studio canvas assets');
  assert.match(projectsSource, /readStudioProjectCanvasPreview/, 'personal project cards should prefer their saved canvas capture');
  assert.doesNotMatch(projectsSource, /StudioCanvasPreview/, 'project cards should not approximate the Studio canvas with a custom SVG');
  assert.match(canvasSource, /toJpeg/, 'the real React Flow canvas should be captured as a project thumbnail');
  assert.match(canvasSource, /writeStudioProjectCanvasPreview/, 'canvas captures should be persisted for the project picker');
  assert.match(canvasSource, /data-studio-theme[\s\S]*#f7f9fc/, 'light canvas captures should encode an explicit light background instead of JPEG black');
});
...
const studioProjectPreviewStoragePath = join(studioDir, 'projects/studio-project-preview-storage.ts');
...
const canvasGuideCalloutPath = join(workspaceDir, '_components/canvas/CanvasGuideCallout.tsx');
const canvasGuideLayerPath = join(workspaceDir, '_components/canvas/CanvasGuideLayer.tsx');
const workspaceGuideSurfaceLayerPath = join(workspaceDir, '_components/WorkspaceGuideSurfaceLayer.tsx');
...
const workspaceMobilePanelsHookPath = join(workspaceDir, '_hooks/useWorkspaceMobilePanels.ts');
const workspaceMobilePanelFramePath = join(workspaceDir, '_components/WorkspaceMobilePanelFrame.tsx');
const workspaceMobilePanelControlsPath = join(workspaceDir, '_components/WorkspaceMobilePanelControls.tsx');
const workspaceProjectMediaPanelPath = join(workspaceDir, '_components/WorkspaceProjectMediaPanel.tsx');
const workspaceCanvasInspectorPanelPath = join(workspaceDir, '_components/WorkspaceCanvasInspectorPanel.tsx');
...
const workspaceEnginePickerPath = join(workspaceDir, '_components/WorkspaceEnginePicker.tsx');
...
const guidedStartersPath = join(workspaceDir, '_lib/templates/guided-project-starters.ts');
...
const canvasGuideControllerHookPath = join(workspaceDir, '_hooks/useWorkspaceCanvasGuideController.ts');
const workspaceGuidePresentationControllerHookPath = join(workspaceDir, '_hooks/useWorkspaceGuidePresentationController.ts');
const canvasGuideViewportHookPath = join(workspaceDir, '_hooks/useWorkspaceCanvasGuideViewport.ts');
const canvasGuideAnalyticsPath = join(workspaceDir, '_lib/workspace-guide-analytics.ts');
const canvasGuideLayoutPath = join(workspaceDir, '_lib/workspace-guide-layout.ts');
...
test('guide analytics stay consent-aware, sanitized, and controller-owned', () => {
  assert.ok(existsSync(canvasGuideAnalyticsPath), 'guide analytics should live in a focused route-local helper');
  const analyticsSource = source(canvasGuideAnalyticsPath);
  const guideControllerSource = source(canvasGuideControllerHookPath);
  const canvasControllerSource = source(canvasControllerHookPath);
  const workspaceSource = source(workspacePagePath);
  const canvasSource = source(canvasPath);

  assert.match(analyticsSource, /dispatchAnalyticsEvent/, 'guide analytics should use the shared consent-aware dispatcher');
  assert.doesNotMatch(analyticsSource, /\bgtag\b|localStorage|sessionStorage/, 'guide analytics should not own transport or consent persistence');
  assert.match(analyticsSource, /templateId:\s*WorkspaceProjectStarterTemplateId/);
  assert.match(analyticsSource, /stepId:\s*WorkspaceGuideStepId\s*\|\s*'all'/);
  assert.match(analyticsSource, /elapsedMs:\s*number/);
  assert.match(analyticsSource, /locale:\s*AppLocale/);
  assert.match(
    analyticsSource,
    /export type WorkspaceGuideViewportClass\s*=\s*'mobile'\s*\|\s*'compact'\s*\|\s*'desktop'/
  );
  const analyticsInput = analyticsSource.match(
    /export type WorkspaceGuideAnalyticsInput\s*=\s*\{[\s\S]*?\n\};/
  )?.[0] ?? '';
  assert.match(analyticsInput, /viewportClass:\s*WorkspaceGuideViewportClass/);
  assert.doesNotMatch(analyticsInput, /viewportWidth/, 'analytics input must not accept raw viewport width');
  assert.doesNotMatch(
    analyticsSource,
    /promptText|mediaData|fileName|filename|mediaUrl|generatedContent|nodeTitle|\burl:/,
    'analytics input must not accept content-bearing fields'
  );
  assert.match(guideControllerSource, /dispatchWorkspaceGuideAnalytics/, 'guide interactions and milestones should dispatch inside the guide controller');
  assert.match(guideControllerSource, /sourceShotId/, 'generation milestones should match the guided source shot');
  assert.match(guideControllerSource, /status\s*===\s*'ready'/, 'generation milestones should ignore pending, placeholder, and failed outputs');
  assert.match(guideControllerSource, /outputNodeId/, 'timeline milestones should match the generated output identity');
  assert.match(canvasControllerSource, /activeCanvasId/, 'canvas controller should provide the active canvas analytics key');
  assert.match(canvasControllerSource, /timelineItems:\s*timelineItemsRef\.current/, 'canvas controller should expose current timeline items to milestone observation');
  assert.doesNotMatch(workspaceSource, /dispatchWorkspaceGuideAnalytics|studio_first_generation_completed/, 'workspace page should remain orchestration-only');
  assert.doesNotMatch(canvasSource, /dispatchWorkspaceGuideAnalytics|studio_first_generation_completed/, 'canvas presentation should not own analytics or milestone rules');
});

test('Studio owns one accessible family-first engine picker surface', () => {
  const workspaceEnginePickerSource = source(workspaceEnginePickerPath);
  const workspaceEnginePickerStyles = source(workspaceEnginePickerStylesPath);
  const shotNodeControlsSource = source(shotNodeControlsPath);
  const shotNodeInspectorSource = source(shotNodeInspectorPath);

  assert.match(workspaceEnginePickerSource, /createPortal/);
  assert.match(workspaceEnginePickerSource, /role="listbox"/);
  assert.match(workspaceEnginePickerSource, /aria-activedescendant/);
  assert.match(workspaceEnginePickerSource, /role="option"/);
  assert.match(workspaceEnginePickerSource, /aria-disabled/);
  assert.match(workspaceEnginePickerSource, /title=\{item\.disabledReason\}/);
  assert.match(workspaceEnginePickerSource, /if \(item\.disabled\) return;/);
  assert.match(workspaceEnginePickerSource, /EngineIcon/);
  assert.match(workspaceEnginePickerSource, /replaceAll\('\{count\}', String\(count\)\)/);
  assert.match(workspaceEnginePickerSource, /event\.key === 'Escape'/);
  assert.match(workspaceEnginePickerSource, /document\.addEventListener\('focusin'/);
  assert.match(workspaceEnginePickerSource, /aria-controls=\{open && activeGroup \? listboxId : undefined\}/);
  assert.match(workspaceEnginePickerSource, /aria-controls=\{activeGroup \? listboxId : undefined\}/);
  assert.match(workspaceEnginePickerSource, /scrollIntoView\(\{ block: 'nearest' \}\)/);
  assert.match(workspaceEnginePickerSource, /getComputedStyle/);
  assert.match(workspaceEnginePickerSource, /data-theme=\{portalThemeMode\}/);
  assert.match(workspaceEnginePickerSource, /new MutationObserver\(updateGeometry\)/);
  assert.match(workspaceEnginePickerSource, /attributeFilter: \['data-studio-theme'\]/);
  assert.match(workspaceEnginePickerSource, /window\.innerWidth - VIEWPORT_PADDING \* 2/);
  assert.match(workspaceEnginePickerSource, /window\.innerHeight - VIEWPORT_PADDING - panelHeight/);
  assert.match(workspaceEnginePickerSource, /workspaceEnginePickerTriggerLabel/);
  assert.match(workspaceEnginePickerSource, /unavailableLabel: props\.copy\.enginePickerUnavailable/);
  assert.match(workspaceEnginePickerSource, /disabled=\{!hasOptions\}/);
  assert.doesNotMatch(workspaceEnginePickerSource, /WORKSPACE_V1_BLOCK_MATRIX|compatibleModelIds/);
  assert.match(workspaceEnginePickerStyles, /\.triggerNode[\s\S]*height:\s*32px/);
  assert.match(workspaceEnginePickerStyles, /\.triggerInspector[\s\S]*height:\s*38px/);
  assert.match(workspaceEnginePickerStyles, /grid-template-columns:\s*148px minmax\(0, 1fr\)/);
  assert.match(workspaceEnginePickerStyles, /@media \(max-width:\s*560px\)[\s\S]*overflow-x:\s*auto/);

  for (const sourceText of [shotNodeControlsSource, shotNodeInspectorSource]) {
    assert.match(sourceText, /WorkspaceEnginePicker/);
    assert.match(sourceText, /workspaceShotPatchForModelSelection/);
    assert.match(sourceText, /buildWorkspaceEnginePickerGroups/);
    assert.doesNotMatch(sourceText, /<select[\s>]/);
    assert.match(
      sourceText,
      /isToolOnlyPreset\([^)]+\)[\s\S]*compatibleCapabilities\.length <= 1/,
      'single-engine tools should keep a static model summary'
    );
  }
  assert.equal(
    (shotNodeControlsSource.match(/function patchForModelSelection/g) ?? []).length,
    0
  );
});
...
test('Studio responsive shell keeps Project media and inspector accessible without compressing the primary surface', () => {
  ...
  assert.doesNotMatch(
    source(workspaceMobilePanelsHookPath),
    /FOCUSABLE_DRAWER_SELECTOR|handlePanelKeyDown|event\.key !== 'Tab'/,
    'responsive panel state should not retain an unused modal focus trap'
  );
...
test('MaxVideoAI editor workspace is an isolated authenticated app route', () => {
  ...
  assert.equal(existsSync(studioCanvasTemplatesApiPath), false, 'saved canvases should not expose a global collection API');
  ...
  const workspaceMobilePanelsHookSource = source(workspaceMobilePanelsHookPath);
  const workspaceMobilePanelFrameSource = source(workspaceMobilePanelFramePath);
  const workspaceMobilePanelControlsSource = source(workspaceMobilePanelControlsPath);
  const workspaceProjectMediaPanelSource = source(workspaceProjectMediaPanelPath);
  const workspaceCanvasInspectorPanelSource = source(workspaceCanvasInspectorPanelPath);
  const workspaceTimelineInspectorPanelSource = source(workspaceTimelineInspectorPanelPath);
  ...
  assert.match(
    studioArchitectureGuideSource,
    /Project-page guided starters initialize a new project[\s\S]*canonical guide state/,
    'studio architecture guide should distinguish guided project bootstrap from advanced Canvas templates',
  );
  assert.match(
    studioArchitectureGuideSource,
    /advanced Canvas navigator[\s\S]*New and Replace[\s\S]*clear the active guide state[\s\S]*Add[\s\S]*preserves the active guide state unchanged/,
    'studio architecture guide should document guide behavior for every advanced template apply mode',
  );
  ...
  assert.match(
    studioAgentsSource,
    /Project-page guided starters initialize a new project[\s\S]*canonical guide state/,
    'studio AGENTS guide should distinguish guided project bootstrap from advanced Canvas templates',
  );
  assert.match(
    studioAgentsSource,
    /advanced Canvas navigator[\s\S]*New and Replace[\s\S]*clear the active guide state[\s\S]*Add[\s\S]*preserves the active guide state unchanged/,
    'studio AGENTS guide should document guide behavior for every advanced template apply mode',
  );
  ...
  assert.match(projectsClientSource, /createProject\s*=\s*async\s*\(templateId:\s*WorkspaceTemplateId\s*=\s*DEFAULT_STUDIO_PROJECT_TEMPLATE_ID\)/, 'project creation should accept a real canvas template while preserving the blank default');
  assert.match(projectsClientSource, /canvasTemplateId:\s*templateId/, 'starter selection should persist the selected canvas template on the project');
  assert.match(projectsClientSource, /styles\.starterGrid/, 'all users should see a compact starter workflow grid');
  assert.match(projectsClientSource, /styles\.blankProjectAction/, 'starter onboarding should retain a secondary blank-project action');
  assert.match(projectsClientSource, /styles\.projectGrid/, 'returning users should see their projects as the primary collection');
  assert.doesNotMatch(projectsClientSource, /openLatestProject|styles\.openProjectButton|<FolderOpen/, 'project navigation should live on project cards instead of a duplicate Open Project action');
  ...
  assert.doesNotMatch(projectsClientSource, /showStarterChooser|newProjectButtonRef|closeStarterButtonRef|projectsHeroCopy/, 'starter workflows should stay visible without a disclosure control or redundant hero copy');
  assert.doesNotMatch(projectsStylesSource, /\.projectsHeaderBar|\.projectsHeroCopy|\.newProjectButton/, 'studio projects should start directly with workflow choices');
  assert.match(projectsStylesSource, /\.starterGrid/, 'studio projects should own responsive starter workflow styling');
  assert.doesNotMatch(projectsStylesSource, /radial-gradient|\.newProjectLaunchCard[\s\S]*min-height:\s*356px/, 'studio projects should avoid decorative gradients and oversized blank launch cards');
  ...
  assert.match(workspaceEditorLayoutSource, /useWorkspaceCanvasGuideViewport/, 'editor layout should retain guide viewport lifecycle state above the conditional Canvas mount');
  ...
  assert.match(workspaceEditorLayoutSource, /WorkspaceProjectMediaPanel/, 'editor layout should compose a focused project media panel for Viewer mode');
  assert.match(workspaceProjectMediaPanelSource, /TimelineProjectSidebar/, 'project media panel should own the route-local project media sidebar composition');
  ...
  assert.match(workspaceEditorLayoutSource, /useWorkspaceMobilePanels/, 'editor layout should delegate responsive side panel state to a focused hook');
  assert.match(workspaceMobilePanelsHookSource, /export type MobileWorkspacePanel = 'media' \| 'inspector' \| null/, 'mobile panel hook should model the responsive side panels as explicit mobile-only state');
  assert.match(workspaceEditorLayoutSource, /data-mobile-panel=\{mobilePanels\.activePanel \?\? 'closed'\}/, 'editor body should expose the active mobile panel state for responsive styling');
  assert.match(workspaceMobilePanelFrameSource, /data-mobile-panel-close/, 'mobile panel frame should provide a stable close target for focus restoration');
  ...
  assert.match(persistenceEffectsHookSource, /autosaveReadiness/, 'workspace persistence hook should keep autosave authority separate from render hydration');
  assert.match(persistenceEffectsHookSource, /!autosaveReadiness\.local/, 'local fallback writes should wait for authoritative hydration state');
  assert.match(persistenceEffectsHookSource, /!autosaveReadiness\.api/, 'API writes should wait for complete API hydration or authoritative local state');
  assert.match(
    persistenceEffectsHookSource,
    /autosaveReadiness\.workspaceStorageKey !== workspaceStorageKey/,
    'autosave readiness must be scoped to the project being persisted'
  );
  ...
  assert.match(
    workspaceSource,
    /createStarterWorkspaceTemplate\(MINIMAL_START_WORKSPACE_TEMPLATE_ID\)/,
    'workspace defaults should use the internal minimal bootstrap'
  );
  ...
  assert.match(
    persistenceEffectsHookSource,
    /createStarterWorkspaceTemplate\(project\.canvasTemplateId\)/,
    'clean project hydration should use the normalized stored template id directly'
  );
  assert.match(workspacePersistenceSource, /normalizeStudioProjectStorageRecord/, 'local project persistence should normalize legacy records before workspace hydration');
  assert.match(
    projectsClientSource,
    /DEFAULT_STUDIO_PROJECT_TEMPLATE_ID: WorkspaceTemplateId = MINIMAL_START_WORKSPACE_TEMPLATE_ID/,
    'project creation should persist the internal bootstrap id'
  );
  assert.match(
    persistenceEffectsHookSource,
    /if \(persisted\) \{\s*applyPersistedWorkspace\(persisted\);\s*\} else if \(storedProject\) \{\s*applyStoredProjectWorkspace\(storedProject\);\s*\}/,
    'persisted workspace state should remain the priority before clean project hydration'
  );
  ...
  assert.match(workspaceProjectMediaPanelSource, /onNewSequence=\{sequence\.handleCreateSequence\}/, 'viewer project sidebar should create real empty sequences');
  assert.match(workspaceProjectMediaPanelSource, /onSelectSequence=\{sequence\.handleSelectSequence\}/, 'viewer project sidebar should switch between stored sequences');
  ...
  assert.match(workspaceProjectMediaPanelSource, /onDeleteSequence=\{sequence\.handleDeleteSequence\}/, 'viewer project sidebar should delete sequences through sequence actions');
  assert.match(workspaceProjectMediaPanelSource, /onDeleteSequences=\{sequence\.handleDeleteSequences\}/, 'viewer project sidebar should bulk-delete sequences through sequence actions');
  assert.match(workspaceProjectMediaPanelSource, /onDuplicateSequence=\{sequence\.handleDuplicateSequence\}/, 'viewer project sidebar should duplicate sequences through sequence actions');
  ...
  assert.match(canvasTemplateActionsHookSource, /handleApplyCanvasTemplate/, 'canvas template action hook should own canvas-only template application');
  assert.doesNotMatch(canvasTemplateActionsHookSource, /setTimelineItems|setSequences/, 'canvas template actions should not own montage timeline or sequence state');
  ...
  assert.match(workspaceEditorLayoutSource, /WorkspaceCanvasInspectorPanel/, 'editor layout should compose the focused canvas settings inspector panel');
  assert.match(workspaceCanvasInspectorPanelSource, /NodeSettingsPanel/, 'canvas inspector panel should own node settings composition');
  assert.match(workspaceEditorLayoutSource, /WorkspaceTimelineInspectorPanel/, 'editor layout should compose a focused timeline clip inspector panel for Viewer mode');
  assert.match(workspaceTimelineInspectorPanelSource, /TimelineClipInspector/, 'timeline inspector panel should own clip inspector composition');
  assert.match(workspaceEditorLayoutSource, /focusMode === 'canvas'[\s\S]*WorkspaceCanvasInspectorPanel[\s\S]*WorkspaceTimelineInspectorPanel/, 'right inspector should switch from node settings in Canvas mode to clip settings in Viewer mode');
  ...
  assert.match(canvasGraphStateHookSource, /const \[selectedNodeId, setSelectedNodeId\] = useState<string \| null>\(defaultSelectedNodeId \?\? null\)/, 'Canvas graph state should accept an optional bootstrap focus without opening the inspector');
...
test('Studio sequence snapshots own live guide serialization for the workspace', () => {
  const sequenceSnapshotsSource = source(sequenceSnapshotsHookPath);
  const workspaceSource = source(workspacePagePath);

  assert.match(
    sequenceSnapshotsSource,
    /buildPersistedWorkspaceSequenceSnapshot/,
    'sequence snapshot hook should own the persisted workspace serializer'
  );
  assert.match(
    workspaceSource,
    /useWorkspaceSequenceSnapshots\(\{[\s\S]*?guideState,/,
    'workspace should pass the active guide state into persistence snapshots'
  );
});
...
test('MaxVideoAI editor owns authenticated Studio persistence contracts', () => {
  ...
  assert.doesNotMatch(schemaSource, /studio_canvas_templates/, 'runtime schema should not recreate the removed global canvas-template owner');
  ...
  assert.doesNotMatch(repositorySource, /StudioCanvasTemplate|listStudioCanvasTemplates|upsertStudioCanvasTemplate|deleteStudioCanvasTemplate/, 'server persistence should not expose an unused graph-only canvas-template repository');
...
test('MaxVideoAI editor owns graph, node, generation, and capability contracts', () => {
  ...
  assert.ok(existsSync(canvasGuideViewportHookPath), 'guide fit completion and viewport memory should live above the conditional Canvas mount');
  ...
  const canvasSource = source(canvasPath);
  const canvasGuideCalloutSource = source(canvasGuideCalloutPath);
  const canvasGuideLayerSource = source(canvasGuideLayerPath);
  const workspaceGuideSurfaceLayerSource = source(workspaceGuideSurfaceLayerPath);
  ...
  const generationRoutingSource = source(generationRoutingPath);
  ...
  const guidedStartersSource = source(guidedStartersPath);
  const workspaceStateSource = source(workspaceStatePath);
  ...
  const workspaceTypesSource = typesSource;
  const styleSource = source(stylesPath);
  ...
  assert.match(canvasNodeStyleSource, /\.graphHandle[\s\S]*border:\s*0/, 'canvas connector handles should render as solid colored dots without inherited white outlines');
  assert.match(canvasNodeStyleSource, /\.graphHandle[\s\S]*box-shadow:\s*none/, 'canvas connector handles should not keep a light halo around selection dots');
  assert.match(canvasNodeStyleSource, /\.graphHandle[\s\S]*width:\s*20px/, 'canvas connector handles should expose a generous invisible pointer target');
  assert.match(canvasNodeStyleSource, /\.graphHandle[\s\S]*height:\s*20px/, 'canvas connector handles should expose a generous invisible pointer target');
  assert.match(canvasNodeStyleSource, /\.graphHandle[\s\S]*touch-action:\s*none/, 'canvas connector handles should keep pointer drags reliable on touch and trackpad input');
  assert.match(canvasNodeStyleSource, /\.graphHandle::after[\s\S]*width:\s*7px/, 'canvas connector handles should keep the visible dot compact next to dense node controls');
  assert.match(canvasNodeStyleSource, /\.graphHandle::after[\s\S]*height:\s*7px/, 'canvas connector handles should keep the visible dot compact next to dense node controls');
  assert.match(canvasNodeStyleSource, /\.graphHandle::after[\s\S]*background:\s*var\(--workspace-handle-color/, 'canvas connector handles should draw their visible dot from a handle color variable');
  assert.match(canvasNodeStyleSource, /\.graphHandle:hover::after[\s\S]*width:\s*10px/, 'canvas connector handles should expose a clearer hover affordance without permanently enlarging dots');
  assert.match(canvasNodeStyleSource, /\.graphHandle:hover::after[\s\S]*height:\s*10px/, 'canvas connector handles should expose a clearer hover affordance without permanently enlarging dots');
  assert.match(nodeFrameSource, /'--workspace-handle-color': color/, 'node frame handles should pass handle colors through CSS variables instead of coloring the whole hit target');
  assert.match(nodeFrameSource, /const sourceHandles = data\.kind === 'shot' \? \[\] : outputHandles\(data\)/, 'shot node outputs should be row-aligned in the compact connector dock instead of the frame stack');
  ...
  assert.match(shotNodeControlsSource, /shotGeneratePrice/, 'shot generate buttons should keep pricing in a separate constrained price slot');
  assert.match(shotNodeControlsSource, /aria-busy=\{shot\.status === 'generating'\}/, 'shot generate buttons should expose loading state without relying on oversized text');
  ...
  assert.match(canvasShotControlsStyleSource, /\.shotGenerateButton[\s\S]*grid-template-columns:\s*minmax\(0, 1fr\) max-content/, 'shot generate buttons should reserve separate columns for label and price');
  assert.match(canvasShotControlsStyleSource, /\.shotGeneratePrice[\s\S]*max-width:/, 'shot generate price labels should be constrained instead of cropping the action label');
  ...
  assert.doesNotMatch(workspaceApiPersistenceSource, /\/api\/studio\/canvas-templates/, 'saved canvases should not use the stale global template API');
  assert.match(workspaceApiPersistenceSource, /savedCanvases/, 'saved canvases should round-trip inside the project workspace snapshot');
  ...
  assert.doesNotMatch(workspacePersistenceSource, /readUserCanvasTemplates|writeUserCanvasTemplates|USER_CANVAS_TEMPLATES_STORAGE_KEY/, 'saved canvases should have no global local-storage owner');
  assert.match(workspaceStateSource, /savedCanvases\?: WorkspaceUserCanvasTemplate\[\]/, 'project snapshots should remain the only saved-canvas persistence owner');
  ...
  assert.match(generationActionsHookSource, /createPendingWorkspaceOutput/, 'generation action hook should create a submission-scoped output placeholder as soon as generation starts');
  assert.match(generationActionsHookSource, /createWorkspaceGenerationSubmissionId/, 'generation action hook should isolate concurrent paid submissions');
  assert.match(generationActionsHookSource, /mergeWorkspaceGenerationOutputNodes/, 'generation action hook should append provider outputs through the deterministic merger');
  assert.doesNotMatch(generationActionsHookSource, /findGeneratedOutputNodeForShot/, 'generation action hook must not overwrite an earlier run by reusing its output block');
  ...
  assert.match(shotInputDockSource, /data-shot-connector-row/, 'generate block connector rows should be directly testable in browser smoke tests');
  assert.match(shotInputDockSource, /type="source"/, 'generate block output handles should stay connectable from the compact connector dock');
  ...
  assert.match(canvasNodeStyleSource, /\.shotInputRowHandle/, 'generate block connector row handles should have row-specific alignment styling');
  assert.match(canvasNodeStyleSource, /\.shotOutputRow/, 'generate block output rows should be styled alongside input rows');
  assert.match(shotInputDockSource, /CONNECTOR_HANDLE_EDGE_OFFSET = -14/, 'generate block input handles should compensate for dock inset and sit on the node border');
  assert.match(shotInputDockSource, /left:\s*CONNECTOR_HANDLE_EDGE_OFFSET/, 'generate block input handles should be centered on the node border like output handles');
  assert.match(shotInputDockSource, /right:\s*CONNECTOR_HANDLE_EDGE_OFFSET/, 'generate block output handles should be centered on the node border like input handles');
  assert.match(shotInputDockSource, /translate\(-50%, -50%\)/, 'input handle dots should keep their center aligned to the node border');
  assert.match(shotInputDockSource, /translate\(50%, -50%\)/, 'output handle dots should keep their center aligned to the node border');
  ...
  assert.match(shotNodeInspectorSource, /WorkspaceEnginePicker/, 'shot inspector should use the shared engine picker');
  assert.match(shotNodeInspectorSource, /buildWorkspaceEnginePickerGroups/, 'shot inspector should build model choices through shared policy');
  assert.match(shotNodeInspectorSource, /workspaceShotPatchForModelSelection/, 'shot inspector should delegate model-derived patches to the shared helper');
  ...
  assert.doesNotMatch(shotNodeInspectorSource, /<SelectControl/, 'shot inspector model selector should not retain the native selector wrapper');
  assert.doesNotMatch(shotNodeInspectorSource, /recommendedModels|remainingCapabilities|<optgroup/, 'shot inspector should not recreate legacy recommendation groups');
  ...
  assert.match(workspaceTimelineInspectorPanelSource, /selectedSequence=\{exportState\.selectedSequenceForInspector\}/, 'viewer mode inspector should receive selected sequence settings');
  ...
  assert.match(canvasHistoryHookSource, /guideStateRef/, 'canvas history should snapshot guide state with graph state');
  assert.match(canvasHistoryHookSource, /commitCanvasState/, 'canvas history should expose a composite commit operation');
  assert.match(canvasHistoryHookSource, /commitCanvasGuideState/, 'canvas history should expose a guide-state commit operation');
  assert.match(workspaceTypesSource, /WorkspaceCanvasGuideState/);
  assert.match(guidedStartersSource, /guideAnnotations/);
  assert.match(canvasGuideLayerSource, /resolveWorkspaceGuidePlacement/);
  assert.doesNotMatch(workspaceSource, /WORKSPACE_GUIDE_DESKTOP_GAP|protectedSelectors|deleteAllConfirm/);
  assert.doesNotMatch(nodeSource, /CanvasGuide|guideAnnotations/);
  const forbiddenGuideDataPattern = /\b(?:guideState|guideAnnotations?|WorkspaceCanvasGuideState|WorkspaceGuideAnnotation|CanvasGuide)\b|(?:^|[,{;\s])annotations\s*[:=]/m;
  for (const [owner, downstreamSource] of [
    ['Canvas map', canvasMapSource],
    ['graph clipboard', graphClipboardSource],
    ['model pricing adapter', modelPricingAdapterSource],
    ['pricing resolver', pricingSource],
    ['pricing hook', pricingHookSource],
    ['generation request builder', generationSource],
    ['generation routing', generationRoutingSource],
    ['generation actions', generationActionsHookSource],
    ['render-node adapter', renderNodesHookSource],
    ['render-edge adapter', renderEdgesSource],
    ['timeline owner', timelineSource],
    ['timeline editing', timelineEditingSource],
    ['timeline render manifest', timelineRenderSource],
    ['timeline export manifest', timelineExportSource],
    ['export state', exportStateHookSource],
    ['export controller', exportControllerSource],
  ]) {
    assert.doesNotMatch(
      downstreamSource,
      forbiddenGuideDataPattern,
      `guide presentation state must stay out of ${owner} ownership`,
    );
  }
  assert.ok(existsSync(canvasGuideControllerHookPath), 'guide commands should live in their own canvas controller hook');
  assert.match(canvasControllerHookSource, /useWorkspaceCanvasGuideController/, 'canvas controller should compose focused guide commands');
  assert.match(canvasControllerHookSource, /guide:\s*\{/, 'canvas controller should expose guide commands as a grouped surface');
  assert.ok(existsSync(canvasGuideLayerPath), 'canvas guide rendering should live in a focused route-local layer');
  assert.ok(existsSync(canvasGuideCalloutPath), 'canvas guide callouts should live outside React Flow nodes');
  assert.ok(existsSync(workspaceGuidePresentationControllerHookPath), 'shared guide presentation state should live above both visual layers');
  assert.match(workspaceEditorLayoutSource, /useWorkspaceGuidePresentationController/, 'the persistent editor layout should own guide presentation state');
  assert.match(canvasGuideLayerSource, /useWorkspaceGuidePresentationController/, 'the Canvas guide layer should consume the shared presentation contract');
  assert.match(workspaceGuideSurfaceLayerSource, /useWorkspaceGuidePresentationController/, 'the surface guide layer should consume the shared presentation contract');
  assert.doesNotMatch(workspaceGuideSurfaceLayerSource, /from ['"]\.\/canvas\/CanvasGuideLayer['"]/, 'the surface layer must not import its controller contract from the Canvas presentation layer');
  assert.ok(existsSync(canvasGuideLayoutPath), 'canvas guide geometry should live in the pure layout helper');
  assert.match(canvasSource, /CanvasGuideLayer/, 'workspace canvas should render the standalone guide layer');
  assert.match(canvasGuideLayerSource, /workspace-guide-layout/, 'canvas guide layer should delegate placement geometry to the pure layout helper');
  assert.match(canvasGuideLayerSource, /useOnViewportChange/, 'canvas guide placement should react to React Flow viewport changes');
  assert.match(canvasGuideLayerSource, /ResizeObserver/, 'canvas guide placement should remeasure responsive callouts');
  assert.match(canvasGuideLayerSource, /requestAnimationFrame/, 'canvas guide placement should batch screen-space measurement');
  assert.match(canvasGuideLayerSource, /resolveWorkspaceGuidePlacement/, 'canvas guide layer should use the pure placement engine');
  assert.match(canvasGuideLayerSource, /expandWorkspaceGuideFitBounds/, 'initial guide framing should use the pure fit-bounds helper');
  assert.match(canvasGuideLayerSource, /data-canvas-guide-layer/, 'canvas guide layer should expose its stable browser selector');
  assert.match(canvasGuideCalloutSource, /data-canvas-guide-annotation/, 'guide callouts should expose their stable annotation selector');
  assert.match(canvasGuideCalloutSource, /data-guide-step/, 'guide callouts should expose their stable step selector');
  assert.match(canvasGuideCalloutSource, /data-guide-collapsed/, 'guide callouts should expose their compact-state selector');
  assert.match(canvasFloatingToolbarSource, /data-canvas-floating-toolbar/, 'guide placement should be able to protect the floating toolbar');
  assert.match(workspaceEditorLayoutSource, /data-studio-canvas-inspector/, 'guide placement should be able to protect the responsive Canvas inspector');
  assert.match(shotNodeControlsSource, /data-shot-generation-action/, 'guide anchors should target the stable generate action selector');
  assert.match(shotNodeControlsSource, /data-shot-generation-status/, 'blocked guide anchors should target the stable generation status selector');
  assert.match(nodeSource, /data-generated-output-status/, 'guide anchors should expose failed generated output status');
  assert.doesNotMatch(canvasSource, /resolveWorkspaceGuidePlacement|expandWorkspaceGuideFitBounds|guide\.steps/, 'workspace canvas should not own guide geometry or copy resolution');
  assert.doesNotMatch(nodeSource, /CanvasGuide|data-canvas-guide/, 'guide components must stay out of the React Flow node registry');
  assert.doesNotMatch(canvasMapSource, /CanvasGuide|data-canvas-guide/, 'guide components must stay out of the Canvas map');
  assert.doesNotMatch(workspaceSource, /WORKSPACE_GUIDE_PLACEMENTS|GUIDE_PLACEMENT|WORKSPACE_GUIDE_COPY_KEYS|guideCopyTable/, 'workspace page should not own guide placement or copy tables');
  ...
  assert.match(workspaceTimelineInspectorPanelSource, /onPatchItem=\{timelineClip\.handlePatchTimelineItem\}/, 'timeline inspector panel should expose timeline clip patching for the clip inspector');
  ...
  assert.match(exportControllerSource, /\/api\/studio\/timeline-exports\/\$\{activeExportJobId\}/, 'export controller should poll server render export jobs by stable identity');
  ...
  assert.match(exportControllerSource, /setTimeout/, 'export controller should schedule queued and rendering polls after each delay');
  ...
  assert.match(workspaceTimelineInspectorPanelSource, /<TimelineClipInspector[\s\S]*selectedItem=\{exportState\.selectedTimelineItem\}/, 'viewer mode inspector should edit the selected timeline item');
  assert.match(workspaceTimelineInspectorPanelSource, /<TimelineClipInspector[\s\S]*selectedAsset=\{exportState\.selectedProjectAssetForInspector\}/, 'viewer mode inspector should inspect the selected project media asset');
  assert.match(workspaceTimelineInspectorPanelSource, /<TimelineClipInspector[\s\S]*onPatchItem=\{timelineClip\.handlePatchTimelineItem\}/, 'viewer mode inspector should patch timeline clip edit properties');
  assert.match(workspaceTimelineInspectorPanelSource, /<TimelineClipInspector[\s\S]*onRenameProjectAsset=\{projectMedia\.handleRenameProjectAsset\}/, 'viewer mode inspector should rename selected project media assets through project media actions');
  ...
  assert.match(persistenceEffectsHookSource, /applyPersistedWorkspaceCanvasHydration/, 'persistence effects should own persisted canvas hydration');
  assert.match(persistenceEffectsHookSource, /applyStoredProjectCanvasHydration/, 'persistence effects should own clean-project canvas hydration');
  assert.match(sequenceSnapshotsHookSource, /buildPersistedWorkspaceSequenceSnapshot/, 'sequence snapshot hook should own workspace serialization');
  assert.match(workspaceSource, /commitCanvasState:\s*canvasHistoryController\.commitCanvasState/, 'workspace should wire composite canvas commits without owning mutations');
  assert.match(workspaceSource, /setGuideState,/, 'workspace should only wire the guide setter into persistence');
  ...
  assert.match(workspaceProjectMediaPanelSource, /onInsertProjectAsset=\{projectMedia\.handleInsertProjectAssetToTimeline\}/, 'project media panel should insert imported project media into the timeline');
  ...
  assert.match(cssBlock(viewerStyleSource, '.videoViewerShell'), /min-width:\s*0/, 'central montage viewer should be allowed to shrink inside the three-column shell grid');
  assert.match(cssBlock(viewerStyleSource, '.viewerStage'), /min-width:\s*0/, 'viewer stage should not push the program monitor underneath Project media in Chrome');
  assert.match(cssBlock(viewerStyleSource, '.programMonitor'), /min-width:\s*0/, 'program monitor should shrink before fitting media inside the center column');
...
test('minimal Studio bootstrap is internal, focused, and timeline-empty', () => {
  const template = createStarterWorkspaceTemplate(MINIMAL_START_WORKSPACE_TEMPLATE_ID);

  assert.equal(template.id, 'minimal-start');
  assert.equal(template.focusNodeId, 'minimal-start-video');
  assert.deepEqual(template.timelineItems, []);
  assert.equal(template.nodes.length, 2);
  assert.deepEqual(template.nodes.map((node) => node.type), ['text-prompt', 'shot']);
  assert.equal(template.nodes[0]?.data.promptText, '');
  assert.equal(template.nodes[1]?.data.shot?.presetId, 'generate-video');
  assert.equal(template.nodes[1]?.data.shot?.workflowType, 'text_to_video');
  assert.equal(template.edges.length, 1);
  assert.equal(template.edges[0]?.data?.kind, 'prompt');
  assert.equal(
    WORKSPACE_TEMPLATE_SUMMARIES.some((summary) => summary.id === 'minimal-start'),
    false
  );
});
