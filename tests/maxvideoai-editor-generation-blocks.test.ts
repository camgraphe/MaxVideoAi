...
test('Generate Video only offers native connectors for the selected engine', () => {
  const settings = defaultShotForPreset('generate-video');
  const capability = getWorkspaceModelCapabilities().find((item) => item.id === 'luma-ray-3-2');
  assert.ok(capability);
  const policy = resolveWorkspaceBlockPolicy({
    settings: { ...settings, modelId: capability.id },
    capability,
    connectedInputs: ['prompt'],
  });
  const kinds = policy.inputConnectors.map((connector) => connector.kind);
  assert.equal(kinds.includes('style'), false);
  assert.equal(kinds.includes('camera'), false);
  assert.equal(kinds.includes('voiceover'), false);
  assert.equal(kinds.includes('music'), false);
  assert.equal(kinds.includes('sfx'), false);
});

test('Generate Video intersects native fields with its workflows and preserves mode-specific connector identity', () => {
  const settings = defaultShotForPreset('generate-video');
  const capabilities = getWorkspaceModelCapabilities();
  const seedance = capabilities.find((item) => item.id === 'seedance-2-0');
  const veo = capabilities.find((item) => item.id === 'veo-3-1');
  assert.ok(seedance);
  assert.ok(veo);

  assert.deepEqual(
    seedance.input_connectors
      .filter((connector) => connector.kind === 'video_reference')
      .map((connector) => connector.fieldId),
    ['video_url', 'video_urls', 'extension_source_videos'],
    'capability normalization must retain same-kind fields until block policy selects a workflow'
  );

  const seedancePolicy = resolveWorkspaceBlockPolicy({
    settings: { ...settings, modelId: seedance.id },
    capability: seedance,
    connectedInputs: ['prompt'],
  });
  const seedanceVideo = seedancePolicy.inputConnectors.find((connector) => connector.kind === 'video_reference');
  assert.equal(seedanceVideo?.fieldId, 'video_urls');
  assert.equal(seedanceVideo?.maxCount, 3);
  assert.equal(seedancePolicy.inputConnectors.some((connector) => connector.fieldId === 'video_url'), false);
  assert.equal(seedancePolicy.inputConnectors.some((connector) => connector.fieldId === 'extension_source_videos'), false);

  const veoPolicy = resolveWorkspaceBlockPolicy({
    settings: { ...settings, modelId: veo.id },
    capability: veo,
    connectedInputs: ['prompt'],
  });
  assert.equal(
    veoPolicy.inputConnectors.some((connector) => connector.kind === 'video_reference'),
    false,
    'Veo extend-only video_url must not appear on Generate Video'
  );
});

test('Generate Video enforces the resolved Seedance ref2v video capacity', () => {
  const settings = defaultShotForPreset('generate-video');
  const capabilities = getWorkspaceModelCapabilities();
  const seedance = capabilities.find((item) => item.id === 'seedance-2-0');
  assert.ok(seedance);
  const shotNode: WorkspaceGraphNode = {
    ...
    data: {
      ...
      shot: { ...settings, modelId: seedance.id },
    },
  };
  const sourceNodes: WorkspaceGraphNode[] = Array.from({ length: 4 }, (_, index) => ({
    id: `video-source-${index + 1}`,
    type: 'asset',
    position: { x: 0, y: 0 },
    data: {
      kind: 'asset-video',
  ...
  const existingEdges = (count: number): WorkspaceGraphEdge[] => Array.from({ length: count }, (_, index) => ({
    id: `video-edge-${index + 1}`,
    source: sourceNodes[index].id,
    target: shotNode.id,
    sourceHandle: 'video_reference',
    targetHandle: 'video_reference',
    data: { kind: 'video_reference' },
  }));
  const connection = {
    source: sourceNodes[3].id,
    target: shotNode.id,
    sourceHandle: 'video_reference',
    targetHandle: 'video_reference',
  };
  ...
  assert.equal(workspaceConnectionRejectionReason({
    ...
    edges: existingEdges(2),
    capabilities,
  }), null);
  assert.equal(workspaceConnectionRejectionReason({
    connection,
    nodes: [shotNode, ...sourceNodes],
    edges: existingEdges(3),
    capabilities,
  })?.code, 'connector_full');
});

test('Generate Video rejects Veo video_reference admission when policy omits the connector', () => {
  const settings = defaultShotForPreset('generate-video');
  const capabilities = getWorkspaceModelCapabilities();
  const shotNode: WorkspaceGraphNode = {
    id: 'veo-shot',
...
test('Generate Video rejects new connections to a disabled compatibility anchor', () => {
  ...
  const capabilities = getWorkspaceModelCapabilities();
  const shotNode: WorkspaceGraphNode = {
    id: 'compatibility-shot',
    type: 'shot',
    position: { x: 0, y: 0 },
    data: {
      kind: 'shot',
      title: 'Compatibility shot',
      subtitle: '',
      sourceHandles: ['video_reference'],
      targetHandles: ['prompt', 'continuity', 'previous_shot'],
      shot: { ...settings, modelId: 'luma-ray-3-2' },
    },
  };
  const sourceNodes: WorkspaceGraphNode[] = ['persisted-video', 'new-video'].map((id) => ({
    id,
    type: 'asset',
    position: { x: 0, y: 0 },
    data: {
  ...
  const edges: WorkspaceGraphEdge[] = [{
    id: 'persisted-compatibility-edge',
    source: sourceNodes[0].id,
    target: shotNode.id,
    sourceHandle: 'video_reference',
    targetHandle: 'previous_shot',
    data: { kind: 'continuity' },
  }];
...
test('Generate Video rejects a mutually excluded reference after start_image is connected', () => {
  ...
  const capabilities = getWorkspaceModelCapabilities();
  const shotNode: WorkspaceGraphNode = {
    id: 'exclusive-input-shot',
    type: 'shot',
    position: { x: 0, y: 0 },
    data: {
      kind: 'shot',
      title: 'Exclusive input shot',
      subtitle: '',
      sourceHandles: ['video_reference'],
  ...
  const sourceNodes: WorkspaceGraphNode[] = ['start-image-source', 'reference-source'].map((id) => ({
    id,
    type: 'asset',
    position: { x: 0, y: 0 },
    data: {
...
test('Generate Video does not send inactive mode media fields in a text-to-video request', () => {
  const settings = defaultShotForPreset('generate-video');
  const capability = getWorkspaceModelCapabilities().find((item) => item.id === 'seedance-2-0');
  assert.ok(capability);

  const request = buildWorkspaceShotGenerateRequest({
    settings: { ...settings, modelId: capability.id },
    capability,
    prompt: 'A prompt-only shot.',
    connectedInputs: ['prompt'],
    referenceImages: [],
    videoReferences: ['https://cdn.example.com/stale-v2v-reference.mp4'],
    audioReferences: ['https://cdn.example.com/stale-v2v-reference.wav'],
    shotNodeId: 'prompt-only-shot',
    outputName: 'Prompt only shot',
  });
...
test('Generate Video never synthesizes absent non-prompt required connectors as supported', () => {
  const settings = defaultShotForPreset('generate-video');
  const sourceCapability = getWorkspaceModelCapabilities().find((item) => item.id === 'seedance-2-0');
  assert.ok(sourceCapability);
  const promptConnector = sourceCapability.input_connectors.find((connector) => connector.kind === 'prompt');
  assert.ok(promptConnector);
  const incompleteReferenceCapability: WorkspaceModelCapability = {
    ...sourceCapability,
    id: 'test-incomplete-reference-capability',
    modes: ['t2v', 'ref2v'],
    workflows: ['text_to_video', 'storyboard_to_video'],
    input_connectors: [promptConnector],
    required_inputs: ['prompt'],
    optional_inputs: [],
  };

  const policy = resolveWorkspaceBlockPolicy({
    settings: { ...settings, modelId: incompleteReferenceCapability.id },
    capability: incompleteReferenceCapability,
    connectedInputs: ['prompt', 'reference'],
  });
  const validation = validateShotConnections({
    settings: { ...settings, modelId: incompleteReferenceCapability.id },
    connectedInputs: ['prompt', 'reference'],
    capabilities: [incompleteReferenceCapability],
  });

  const reference = policy.inputConnectors.find((connector) => connector.kind === 'reference');
  assert.match(reference?.disabledReason ?? '', /does not support/i);
  assert.deepEqual(policy.missingInputs, ['reference']);
  assert.deepEqual(validation.incompatibleInputs, ['reference']);
  assert.equal(validation.canGenerate, false);
});

test('Generate Video preserves a connected unsupported input as disabled', () => {
  const settings = defaultShotForPreset('generate-video');
  const capability = getWorkspaceModelCapabilities().find((item) => item.id === 'luma-ray-3-2');
  assert.ok(capability);
  const policy = resolveWorkspaceBlockPolicy({
    settings: { ...settings, modelId: capability.id },
    capability,
    connectedInputs: ['prompt', 'music'],
  });
  const music = policy.inputConnectors.find((connector) => connector.kind === 'music');
  assert.ok(music?.disabledReason);
  assert.equal(music?.connectedCount, 1);
});

test('Generate Video rejects persisted compatibility-only semantic and audio inputs', () => {
  const settings = defaultShotForPreset('generate-video');
  const capabilities = getWorkspaceModelCapabilities();
  const capability = capabilities.find((item) => item.id === 'luma-ray-3-2');
  assert.ok(capability);

  for (const kind of ['camera', 'style', 'music', 'voiceover', 'sfx'] as const) {
    const policy = resolveWorkspaceBlockPolicy({
      settings: { ...settings, modelId: capability.id },
      capability,
      connectedInputs: [kind],
    });
    const validation = validateShotConnections({
      settings: { ...settings, modelId: capability.id },
      connectedInputs: [kind],
    ...
    assert.ok(policy.inputConnectors.find((connector) => connector.kind === kind)?.disabledReason, `${kind} should remain disabled`);
    assert.deepEqual(validation.missingInputs, ['prompt'], `${kind} must not satisfy prompt`);
    assert.deepEqual(validation.incompatibleInputs, [kind], `${kind} should be incompatible`);
    assert.equal(validation.canGenerate, false, `${kind} should block generation`);
  }
});

test('Generate Video accepts native connectors', () => {
  const settings = defaultShotForPreset('generate-video');
  const capabilities = getWorkspaceModelCapabilities();
  const capability = capabilities.find((item) => item.id === 'luma-ray-3-2');
  assert.ok(capability);
...
test('Studio shot input dock disables compatibility-only connector handles independently of capacity', () => {
  const dockSource = readFileSync(
    join(root, 'frontend/app/(core)/(workspace)/app/studio/workspace/_components/nodes/workspace-shot-input-dock.tsx'),
    'utf8'
  );
  ...
  assert.match(dockSource, /opacity: isDisabled \? 0\.35 : 1/);
  assert.match(dockSource, /isConnectable=\{!isDisabled\}/);
});

test('Studio preserves persisted semantic input kinds and their actual target handle anchors', () => {
  const edge: WorkspaceGraphEdge = {
    id: 'legacy-continuity-edge',
    source: 'previous-output',
    target: 'generate-video-shot',
    sourceHandle: 'video_reference',
    targetHandle: 'previous_shot',
    data: { kind: 'continuity' },
  };

  assert.deepEqual(connectedInputKinds('generate-video-shot', [edge]), ['continuity', 'previous_shot']);
  assert.equal(connectedInputCounts('generate-video-shot', [edge]).get('continuity'), 1);
  assert.equal(connectedInputCounts('generate-video-shot', [edge]).get('previous_shot'), 1);
  ...
  const policy = resolveWorkspaceBlockPolicy({
    settings: { ...settings, modelId: capability.id },
    capability,
    connectedInputs: connectedInputKinds('generate-video-shot', [edge]),
  });
  const compatibilityKinds = policy.inputConnectors
    .filter((connector) => connector.disabledReason)
    .map((connector) => connector.kind);
  assert.deepEqual(compatibilityKinds, ['continuity', 'previous_shot']);
});

test('Studio inspector separates available connectors from disabled compatibility rows', () => {
  const inspectorSource = readFileSync(
    join(root, 'frontend/app/(core)/(workspace)/app/studio/workspace/_components/ShotNodeInspector.tsx'),
    'utf8'
  );

  assert.match(inspectorSource, /availableInputConnectors\s*=\s*inputConnectors\.filter\(\(connector\) => !connector\.disabledReason\)/);
  assert.match(inspectorSource, /incompatibleInputConnectors\s*=\s*inputConnectors\.filter\(\(connector\) => Boolean\(connector\.disabledReason\)\)/);
  assert.match(inspectorSource, /data-inspector-connector-state="incompatible"/);
  assert.match(inspectorSource, /<small>\{connector\.disabledReason\}<\/small>/);
});
...
test('Studio chat keeps the shared model registry and delegates its closed API to the authenticated handler', () => {
  ...
  assert.match(handlerSource, /resolveStudioApiAccess/);
  assert.doesNotMatch(handlerSource, /getRouteAuthContext|requireAdmin/);
...
test('tool-aware pricing blocks missing inputs and estimates ready Character and Audio blocks', () => {
  ...
  const characterSettings: WorkspaceShotSettings = {
    ...character,
    toolSettings: {
      characterBuilder: {
        ...character.toolSettings!.characterBuilder!,
  ...
  assert.equal(characterReady, null);
  assert.deepEqual(buildWorkspaceToolBillingProductRequest(characterSettings), {
...
test('Angle and Upscale Studio routing preserves product API pricing in output metadata', () => {
  ...
  const angleMapper = routingSource.match(/export function mapWorkspaceAngleGenerationOutputs[\s\S]*?async function submitAngleGeneration/)?.[0] ?? '';
  ...
  assert.match(angleMapper, /pricing:\s*pricingSnapshotFromToolPricing\(result\.pricing\)/);
  assert.doesNotMatch(angleMapper, /pricing:\s*null/);
});
