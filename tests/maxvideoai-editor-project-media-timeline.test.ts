...
} from '../frontend/app/(core)/(workspace)/app/studio/workspace/_lib/workspace-audio-provenance';
...
test('generated video project assets preserve their dedicated linked audio on the timeline', () => {
  ...
  if (!result.ok) return;
  assert.deepEqual(
    result.items.map((item) => [item.track, item.mediaKind, item.mediaUrl, item.audioProvenance, item.hasEmbeddedAudio]),
    [
...
test('upscale routing preserves unknown audio provenance instead of inventing audio', () => {
  const source = readFileSync(join(
    process.cwd(),
    'frontend/app/(core)/(workspace)/app/studio/workspace/_lib/workspace-generation-routing.ts'
  ), 'utf8');
  const upscale = source.match(/async function submitUpscaleGeneration[\s\S]*?export function audioPackForWorkflow/)?.[0] ?? '';

  assert.match(upscale, /audioProvenance:\s*mediaType === 'video' \? 'unknown' : 'none'/);
  assert.doesNotMatch(upscale, /hasAudio:\s*mediaType === 'video'/);
});
...
test('project media insertion rejects a linked audio collision when the video target is free', () => {
  ...
  const result = resolveProjectAssetTimelineInsert({
    ...
    projectAssets: [{
      ...
      audioUrl: '/media/linked-video-audio.m4a',
      durationSec: 5,
      hasAudio: true,
      audioProvenance: 'external',
    }],
...
test('unknown imported duration stays unknown through insert, normalization, and hydration', () => {
  const asset: WorkspaceAssetRecord = {
    ...
    audioUrl: '/media/unknown-duration.m4a',
    hasAudio: true,
    audioProvenance: 'external',
  };
...
test('project media subtitles do not hide missing source dimensions', () => {
  ...
  assert.equal(
    ...
    'generated video cards should expose explicitly measured source metadata'
  );
});
