...
test('export controller fails closed before the reservation POST without a current estimate', () => {
  ...
  const estimateGuardIndex = source.indexOf('if (!hasCurrentExportEstimate || !estimateToken)');
...
test('timeline export creation carries the authenticated project and signed estimate token', () => {
  const request = buildWorkspaceTimelineVideoExportRequest(readyManifest, {
  ...
  const source = readFileSync(exportControllerPath, 'utf8');
  assert.match(source, /estimateToken/);
  assert.match(source, /JSON\.stringify\(\{ request, estimateToken/);
});

test('timeline export sessions preserve nonterminal jobs and immutable submitted manifests', () => {
  const submittedManifest = {
    ...readyManifest,
    projectName: 'Submitted project name',
  };
  const session = snapshotWorkspaceTimelineExportSubmission({
    current: {
      activeJob: null,
      idempotencyKey: 'export-idempotency-123456',
      submittedManifests: {},
  ...
  submittedManifest.projectName = 'Current project changed later';
  const restored = parseWorkspaceTimelineExportSession(JSON.stringify(session));
  assert.equal(restored?.activeJob?.id, 'export-job-1');
  assert.equal(restored?.idempotencyKey, 'export-idempotency-123456');
  assert.equal(
    workspaceTimelineExportSubmittedManifest(restored, 'export-job-1')?.projectName,
    'Submitted project name'
  );
});

test('export polling is keyed by stable job identity and schedules after each delay', () => {
  const source = readFileSync(exportControllerPath, 'utf8');
  assert.doesNotMatch(source, /setInterval\(/);
  assert.match(source, /setTimeout\(/);
  assert.match(source, /activeExportJob\?\.id/);
  assert.match(source, /activeExportJob\?\.status/);
  assert.doesNotMatch(source, /\}, \[activeExportJob, copy, notices, onNotice\]\)/);
});
...
test('timeline render manifest uses explicitly measured output source metadata', () => {
  const timelineItems = [
    {
      id: 'generated-720p',
