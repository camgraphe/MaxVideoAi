...
test('curation is opt-in, eligible, stable, local and protected against stale saves', async () => {
  ...
  try {
    await db.pool.query(`CREATE TABLE playlists(id uuid PRIMARY KEY, slug text, is_public boolean, updated_at timestamptz DEFAULT now());
    ...
    const { appendPlaylistItem, reorderPlaylistItems, removePlaylistItem } = await import('../frontend/server/playlists/mutations');
    const { getCurationSnapshot, previewCuration, saveCuration, resolveCuratedPlaylist } = await import('../frontend/server/playlists/curation-service');
    ...
    const add = async (job:string, values:Record<string,unknown>={}) => {
      const row={engine_id:'wan-3',engine_label:'Wan 3',prompt:'Fixture',thumb_url:'/thumb.png',video_url:'/video.mp4',status:'completed',surface:'video',visibility:'public',indexable:true,created_at:'2026-09-20T12:00:00Z',...values};
      await db.pool.query(`INSERT INTO app_jobs(job_id,${Object.keys(row).join(',')}) VALUES (${[job,...Object.values(row)].map((_,i)=>'$'+(i+1)).join(',')})`,[job,...Object.values(row)]);
    };
    for(const job of ['a','b','c']) await add(job);
    await add('private',{visibility:'private'}); await add('hidden',{indexable:false}); await add('running',{status:'running'});
    await add('image',{surface:'image'}); await add('missing',{video_url:null}); await add('other-model',{engine_id:'kling-3-pro'});
    assert.deepEqual((await listPlaylistVideos('examples-wan-3',10)).map(v=>v.id),['a']);
    await add('deleted-output');
    await db.pool.query("INSERT INTO job_outputs(job_id,kind,status,url) VALUES ('deleted-output','video','deleted','/video.mp4')");
    ...
    assert.deepEqual(preview.items.map(v=>v.id),['a','c']);
    ...
    assert.deepEqual((await listPlaylistVideos('examples-wan-3',1)).map(v=>v.id),['a']);
    ...
    assert.deepEqual((await listExampleFamilyPage('wan',{sort:'playlist',limit:1,offset:0})).items.map(v=>v.id),['a']);
    assert.equal((await listExampleFamilyPage('wan',{sort:'playlist',limit:1,offset:1})).items.length,0,'excluded model clips cannot reappear via inheritance');

    assert.deepEqual((await resolveCuratedPlaylist('examples-wan-3'))!.map(v=>v.id),['a','c']);
    await assert.rejects(saveCuration(id,draft,initial.revision,preview.token,null),/changed/i);
    await add('new',{created_at:'2026-09-22T12:00:00Z'});
    assert.deepEqual((await resolveCuratedPlaylist('examples-wan-3'))!.map(v=>v.id),['a','new','c']);
    await db.pool.query("UPDATE app_jobs SET visibility='private' WHERE job_id='a'");
    assert.deepEqual((await resolveCuratedPlaylist('examples-wan-3'))!.map(v=>v.id),['new','c']);
    ...
    assert.deepEqual(await resolveCuratedPlaylist('examples-wan-3'),[],'explicitly empty never becomes legacy fallback');
