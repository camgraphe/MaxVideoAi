...
test('admin playlists manager delegates contracts, helper logic, and card UI', () => {
  ...
  assert.match(managerSource, /usePlaylistAction/);
});
...
test('admin playlists manager does not regain extracted ownership', () => {
  ...
  assert.doesNotMatch(
    managerSource,
    /function buildFamilyHelpers\(/,
    'family helper derivation belongs in playlist-helpers.ts'
  );
  assert.doesNotMatch(
    managerSource,
    /getExampleFamilyIds/,
    'model family catalog access belongs in playlist-helpers.ts'
  );
  assert.doesNotMatch(
    managerSource,
    /action: 'seed-family-playlist'/,
    'playlist helper API orchestration belongs in usePlaylistHelperActions.ts'
  );
  ...
  assert.doesNotMatch(
    ...
    'missing family helper UI belongs in MissingFamilyCard.tsx'
  );
  assert.doesNotMatch(
    managerSource,
    /function MissingModelCard\(/,
    'missing model helper UI belongs in MissingModelCard.tsx'
  );
  ...
  assert.doesNotMatch(
    ...
    'model rail disclosure belongs in PlaylistsSidebar.tsx'
  );
  assert.doesNotMatch(
    managerSource,
    /placeholder="Homepage holiday edits"/,
    'new collection form UI belongs in PlaylistCreateForm.tsx'
  );
  assert.doesNotMatch(
    managerSource,
    /Fallback model playlists/,
    'collection details UI belongs in PlaylistDetailsPanel.tsx'
  );
  ...
  assert.ok(
    lineCount <= 500,
    `PlaylistsManager should stay below 500 lines after surface extraction, got ${lineCount}`
  );
});
...
test('admin playlist helper modules expose the expected contract', () => {
  ...
  assert.match(
    createFormSource,
    /placeholder="Homepage holiday edits"/,
    'PlaylistCreateForm should own new collection form fields'
  );
  assert.match(detailsPanelSource, /export function PlaylistDetailsPanel/, 'PlaylistDetailsPanel should be exported');
  assert.match(
    detailsPanelSource,
    /Fallback model playlists/,
    'PlaylistDetailsPanel should own collection metadata details'
  );
  assert.match(detailsPanelSource, /getSurfaceRoleLabel/, 'PlaylistDetailsPanel should own surface labels');
  assert.match(
    feedbackBannersSource,
    /export function PlaylistFeedbackBanners/,
    'PlaylistFeedbackBanners should be exported'
  );
  ...
  assert.match(
    ...
    'PlaylistsManagerSelectionPanel should be exported'
  );
  assert.match(selectionPanelSource, /PlaylistDetailsPanel/, 'PlaylistsManagerSelectionPanel should compose details');
  assert.match(
    helperActionsHookSource,
    /export function usePlaylistHelperActions/,
    'helper API actions should live in a dedicated hook'
  );
  assert.match(
    helperActionsHookSource,
    /seed-model-playlist/,
    'model playlist helper action should stay in the helper hook'
  );
  assert.match(
    dragHookSource,
    /export function usePlaylistDragReorder/,
    'drag/drop behavior should live in a dedicated hook'
  );
